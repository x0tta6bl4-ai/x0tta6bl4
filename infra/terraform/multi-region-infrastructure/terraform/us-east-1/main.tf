# Terraform конфигурация для региона us-east-1 (Северная Америка)
...
module "eks" {
  ...
  eks_managed_node_groups = {
    main = {
      ...
      instance_types = [var.regional_config.us_east_1.instance_types.eks]
      min_size       = 2
      max_size       = 10
      desired_size   = 3
...
module "rds" {
  ...
  instance_class = var.regional_config.us_east_1.instance_types.rds
