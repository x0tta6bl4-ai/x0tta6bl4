    ...
    with get_db_connection() as conn:
        ...
        cursor.execute(
            ...
            INSERT INTO users (
                ...
                entry_node, client_family, stealth_mode_enabled
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
            (
...
def get_broadcast_user_ids(audience: str = "all") -> list[int]:
    ...
    with get_db_connection() as conn:
        if audience == "active":
            rows = conn.execute(
                "SELECT user_id FROM users WHERE expires_at > datetime('now')"
            ).fetchall()
        elif audience == "paid":
            rows = conn.execute(
                "SELECT user_id FROM users WHERE expires_at > datetime('now') AND plan != 'trial'"
            ).fetchall()
        elif audience == "trial":
            rows = conn.execute(
                "SELECT user_id FROM users WHERE expires_at > datetime('now') AND plan = 'trial'"
            ...
            rows = conn.execute(
                "SELECT user_id FROM users WHERE expires_at IS NOT NULL AND expires_at <= datetime('now')"
            ).fetchall()
        else:  # all
