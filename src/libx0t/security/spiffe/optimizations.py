...
class MultiRegionFailover:
    ...
    async def start_health_monitoring(self) -> None:
        ...
        logger.info(
            f"Started multi-region health monitoring (primary: {self.current_region})"
        )
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                try:
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                if response_time < 1.0:
                                    logger.debug(
                                        f"Region {region} health check passed (endpoint: {endpoint}, time: {response_time:.3f}s)"
                                    )
                                    return True
                                else:
                                    logger.warning(
                                        f"Region {region} health check slow: {response_time:.3f}s"
                                    )
                    ...
                    logger.warning(
                        f"Region {region} health endpoints not responding, checking basic connectivity"
                    )
    ...
    def _get_region_config(self, region: str) -> Optional[Dict[str, Any]]:
        ...
        default_configs = {
            "us-east": {
                "spire_server_url": os.getenv(
                    "SPIRE_SERVER_US_EAST", "https://spire-us-east.x0tta6bl4.mesh:8080"
                ),
                "priority": 1,
            },
            "eu-west": {
                "spire_server_url": os.getenv(
                    "SPIRE_SERVER_EU_WEST", "https://spire-eu-west.x0tta6bl4.mesh:8080"
                ),
                "priority": 2,
            },
            "asia-pacific": {
                "spire_server_url": os.getenv(
                    "SPIRE_SERVER_ASIA_PACIFIC",
                    "https://spire-asia-pacific.x0tta6bl4.mesh:8080",
                ),
                "priority": 3,
            },
        }
        ...
        async with self._lock:
            ...
            for region in self.config.fallback_regions:
                if self.region_health.get(region, False):
                    logger.warning(
                        f"Failing over from {self.current_region} to {region}"
                    )
