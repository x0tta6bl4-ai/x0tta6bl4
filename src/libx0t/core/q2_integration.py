...
class Q2Integration:
    ...
    def __init__(
        ...
        self.enable_enhanced_aggregators = (
            enable_enhanced_aggregators and ENHANCED_AGGREGATORS_AVAILABLE
        )
        ...
        if self.enable_rag:
            try:
                self.rag_pipeline = RAGPipeline(
                    top_k=10, rerank_top_k=5, similarity_threshold=0.7
                )
                if rag_data_path:
        ...
        logger.info(
            f"Q2 Integration initialized: RAG={self.enable_rag}, LoRA={self.enable_lora}, Cilium={self.enable_cilium}, EnhancedAgg={self.enable_enhanced_aggregators}"
        )
    ...
    def add_knowledge(
        ...
        try:
            ...
            logger.debug(
                f"Added knowledge document {document_id} ({len(chunk_ids)} chunks)"
            )
            return True
        except Exception as e:
    ...
    def initialize_lora_trainer(
        ...
        try:
            self.lora_trainer = LoRATrainer(
                base_model_name=base_model_name, config=config or LoRAConfig()
            )
    ...
    def train_lora_adapter(
        ...
        if not self.lora_trainer:
            logger.warning(
                "LoRA Trainer not initialized. Call initialize_lora_trainer() first."
            )
            return None

        try:
            result = self.lora_trainer.train(
                train_dataset=train_dataset, adapter_id=adapter_id, **training_kwargs
            )
