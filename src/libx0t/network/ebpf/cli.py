...
class EBPFCLI:
    ...
    def cmd_status(self, args):
        ...
        if programs:
            ...
            for prog in programs:
                ...
                status_color = (
                    Colors.GREEN if attached != "not attached" else Colors.YELLOW
                )
        ...
        if total > 0:
            ...
            drop_color = (
                Colors.GREEN
                if drop_rate < 1
                else (Colors.YELLOW if drop_rate < 5 else Colors.RED)
            )
    ...
    def cmd_load(self, args):
        ...
        try:
            ...
            print_info(
                f"Use 'attach {program_id} <interface>' to attach to an interface"
            )
    ...
    def cmd_attach(self, args):
        ...
        try:
            ...
            success = self.loader.attach_to_interface(
                program_id, interface, attach_mode
            )
    ...
    def cmd_flows(self, args):
        ...
        try:
            cilium = CiliumLikeIntegration(
                interface=self.interface, enable_flow_observability=True
            )
            ...
            for flow in flows:
                ...
                print(
                    f"{time_str:<20} {source:<22} {dest:<22} {proto:<8} {colorize(verdict, verdict_color):<10}"
                )
...
Examples:
  ...
  %(prog)s health                    Perform health check
    ...
    attach_parser.add_argument(
        "-m", "--mode", choices=["skb", "drv", "hw"], help="Attach mode"
    )

    # detach command
    detach_parser = subparsers.add_parser(
        "detach", help="Detach program from interface"
    )
