...
try:
    from .pqc_tunnel import PQC_AVAILABLE, PQCTunnelManager
except ImportError:
...
class MeshRouter:
        ...
        if bootstrap_env:
            ...
            for node_str in bootstrap_env.split(","):
                ...
                if len(parts) >= 2:
                    ...
                    node_id = (
                        parts[2] if len(parts) > 2 else f"node-{host.replace('.', '-')}"
                    )
    ...
    def __init__(self, node_id: str, local_port: int = 10809):
        ...
        if PQCTunnelManager:
            ...
            logger.info(
                f"🔐 PQC encryption enabled ({'ML-KEM-768 (NIST FIPS 203)' if PQC_AVAILABLE else 'simulated'})"
            )
        else:
            self.pqc = None
            ...
            for peer_id, peer in list(self.peers.items()):
                try:
                    ...
                    if latency >= 0:
                        ...
                        if self.shield:
                            self.shield.receive_beacon(
                                peer_id, latency * 1000
                            )  # Convert to ms
                ...
                if peer.is_alive():
                    try:
                        ...
                        for new_peer in new_peers:
                            if (
                                new_peer.node_id not in self.peers
                                and new_peer.node_id != self.node_id
                            ):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    for peer_data in data.get("peers", []):
                        try:
                            new_peer = MeshPeer(
                                ...
                                host=(
                                    peer_data.get("address", "").split(":")[0]
                                    if ":" in peer_data.get("address", "")
                                    else peer_data.get("host", "")
                                ),
                                port=(
                                    int(peer_data.get("address", "").split(":")[1])
                                    if ":" in peer_data.get("address", "")
                                    else peer_data.get("port", 0)
                                ),
                    ...
                    logger.info(
                        f"Discovered {len(peers_list)} peers from {peer.node_id}"
                    )
                    return peers_list
                else:
                    logger.warning(
                        f"Failed to get peers from {peer.node_id}: HTTP {response.status_code}"
                    )
                    return []
        except ImportError:
    ...
    def get_route(self, destination: str, hops: int = 2) -> List[MeshPeer]:
        ...
        for peer in self.peers.values():
            if peer.host == dest_host:
                logger.debug(
                    f"Destination {dest_host} is a peer, using direct connection"
                )
                return []
        ...
        if len(alive_peers) <= hops:
            ...
            relays = random.sample(
                relay_candidates, min(hops - 1, len(relay_candidates))
            )
...
class MeshConnection:
    ...
    async def connect_multi_hop(self, target_host: str, target_port: int) -> bool:
        ...
        logger.info(
            f"🔀 Multi-hop route: {' → '.join([p.node_id for p in self.route])} → target"
        )
        ...
        try:
            ...
            self.reader, self.writer = await asyncio.wait_for(
                asyncio.open_connection(first_hop.host, first_hop.port), timeout=10.0
            )
            ...
            if self.router.pqc and not self.router.pqc.has_tunnel(first_hop.node_id):
                try:
                    if await self.router.pqc.establish_tunnel(
                        self.reader, self.writer, first_hop.node_id
                    ):
            ...
            if not await self._socks5_connect(target_host, target_port):
                raise Exception(
                    f"Failed to connect to target {target_host}:{target_port}"
                )
            ...
            logger.error(
                f"Multi-hop connection failed at hop {self.hops_completed}: {e}"
            )
            await self.close()
            return False
    ...
    async def _socks5_connect(self, host: str, port: int) -> bool:
        ...
        try:
            ...
            if len(response) >= 2 and response[1] == 0x00:
                ...
                logger.debug(
                    f"SOCKS5 connect failed: rep={response[1] if len(response) > 1 else 'N/A'}"
                )
                return False
