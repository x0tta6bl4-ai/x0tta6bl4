...
class BoundaryValidator:
    ...
    def check_string_bounds(
        self, value: str, min_len: int = 0, max_len: int = None
    ) -> List[EdgeCaseViolation]:
    ...
    def check_collection_bounds(
        self, collection: Any, min_size: int = 0, max_size: int = None
    ) -> List[EdgeCaseViolation]:
...
class StateTransitionValidator:
    ...
    def validate_transition(
        self, current_state: str, next_state: str
    ) -> Optional[EdgeCaseViolation]:
...
class ConcurrencyValidator:
    ...
    def check_thread_safety(
        self, operation_name: str, max_concurrent: int = 1
    ) -> Optional[EdgeCaseViolation]:
    ...
    def start_operation(self, operation_name: str) -> None:
        ...
        with self.lock:
            self.active_threads[operation_name] = (
                self.active_threads.get(operation_name, 0) + 1
            )

    def end_operation(self, operation_name: str) -> None:
        """Mark operation end"""
        with self.lock:
            self.active_threads[operation_name] = max(
                0, self.active_threads.get(operation_name, 0) - 1
            )
...
class TimingValidator:
    ...
    def check_timeout(
        self, start_time: datetime, timeout_seconds: float
    ) -> Optional[EdgeCaseViolation]:
...
class EdgeCaseValidator:
    ...
    def validate_input(
        ...
        if isinstance(value, (int, float)):
            violations.extend(
                self.boundary.check_numeric_bounds(
                    value, constraints.get("min"), constraints.get("max")
                )
            )
        elif isinstance(value, str):
            violations.extend(
                self.boundary.check_string_bounds(
                    value, constraints.get("min_len", 0), constraints.get("max_len")
                )
            )
        elif isinstance(value, (list, dict, set)):
            violations.extend(
                self.boundary.check_collection_bounds(
                    value, constraints.get("min_size", 0), constraints.get("max_size")
                )
            )
    ...
    def check_numeric_bounds(
        ...
        """Backward-compatible proxy for numeric boundary checks."""
        violations = self.boundary.check_numeric_bounds(value, min_val, max_val)
        self.all_violations.extend(violations)
        return violations
    ...
    def check_string_bounds(
        ...
        """Backward-compatible proxy for string boundary checks."""
        violations = self.boundary.check_string_bounds(value, min_length, max_length)
        self.all_violations.extend(violations)
        return violations
    ...
    def check_collection_bounds(
        ...
        """Backward-compatible proxy for collection boundary checks."""
        violations = self.boundary.check_collection_bounds(
    ...
    def check_state_transition(
        ...
        """Backward-compatible proxy for state transition validation."""
        violations: List[EdgeCaseViolation] = []
        violation = self.state.validate_transition(current_state, next_state)
        if violation:
    ...
    def check_concurrency_limits(
        ...
        """Backward-compatible check for concurrency limits."""
        violations: List[EdgeCaseViolation] = []
        if active_count > max_concurrent:
            ...
            self.concurrency.violations.append(violation)
            violations.append(violation)
    ...
    def check_timing_bounds(
        ...
        """Backward-compatible timing bounds check."""
        violations: List[EdgeCaseViolation] = []
        if operation_duration > timeout_duration:
            violation = EdgeCaseViolation(
                case_type=EdgeCaseType.TIMING,
                description="Operation timeout",
            ...
            self.timing.violations.append(violation)
            violations.append(violation)
