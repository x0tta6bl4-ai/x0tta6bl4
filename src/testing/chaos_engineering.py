...
class ChaosInjector:
    ...
    async def inject_network_latency(
        self, base_latency_ms: float, max_latency_ms: float, duration_seconds: float
    ) -> None:
        """
        ...
        while time.time() - start_time < duration_seconds:
            ...
            latency = base_latency_ms + random.random() * (
                max_latency_ms - base_latency_ms
            )
    ...
    async def inject_memory_leak(
        self, leak_rate_mb_per_sec: float, max_memory_mb: float, duration_seconds: float
    ) -> None:
        """
        ...
        while (
            time.time() - start_time < duration_seconds and total_leaked < max_memory_mb
        ):
    ...
    async def inject_cpu_spike(
        self, cpu_percent: float, duration_seconds: float  # 0-100
    ) -> None:
        """
        Simulate CPU spike.
    ...
    async def inject_data_corruption(
        self, corruption_rate: float, duration_seconds: float  # 0-1
    ) -> None:
        """
    ...
    async def inject_byzantine_fault(
        ...
        """
        logger.error(
            f"👹 BYZANTINE FAULT: {service_name} ({fault_rate*100:.1f}% corruption)"
        )
    ...
    async def inject_network_partition(
        ...
        logger.critical(
            f"🔌 NETWORK PARTITION: {partition_nodes} isolated for {partition_duration_seconds}s"
        )
    ...
    def record_chaos_event(
        self, scenario: ChaosScenario, result: Dict[str, Any]
    ) -> None:
