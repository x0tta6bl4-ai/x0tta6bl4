...
class ZeroTrustValidator:
    ...
    def _check_policy(self, peer_spiffe_id: str, resource: Optional[str] = None) -> bool:
        """
        ...
        try:
            ...
            if decision.allowed:
                logger.debug(f"Policy check passed for {peer_spiffe_id}: {decision.reason}")
            else:
                logger.warning(f"Policy check failed for {peer_spiffe_id}: {decision.reason}")
