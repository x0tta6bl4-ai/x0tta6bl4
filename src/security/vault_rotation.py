    ...
    async def _on_failure(self):
        ...
        async with self._lock:
            ...
            if self._state == CircuitBreakerState.HALF_OPEN:
                ...
                logger.warning(
                    f"Circuit breaker {self.name} opened (half-open failure)"
                )
...
class RotationValidator:
    ...
    async def validate(
        self, creds: Dict[str, Any], validators: Optional[List[str]] = None
    ) -> Dict[str, bool]:
...
class EnhancedDatabaseCredentialRotator:
    ...
    def __init__(
        ...
        self._circuit_breaker = CircuitBreaker(
            circuit_breaker_config or CircuitBreakerConfig(), name="db-rotation"
        )
        self._validator = RotationValidator()
        self._pg_pool = None

        # Register default validators
        self._validator.register_validator("connectivity", self._validate_connectivity)
        self._validator.register_validator("permissions", self._validate_permissions)
            ...
            async with self._circuit_breaker:
                ...
                if self.strategy == RotationStrategy.SHADOW:
                    ...
                    async with pool.acquire() as conn:
                        await conn.execute(
                            f"CREATE USER {new_username} WITH PASSWORD $1", new_password
                        )
                ...
                async with pool.acquire() as conn:
                    ...
                    for db in databases:
                        await conn.execute(
                            f"GRANT CONNECT ON DATABASE {db} TO {new_username}"
                        )

                        async with pool.acquire() as db_conn:
                            await db_conn.execute(
                                f"GRANT USAGE ON SCHEMA public TO {new_username}"
                            )
                            await db_conn.execute(
                                f"GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO {new_username}"
                            )
                            await db_conn.execute(
                                f"ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO {new_username}"
                            )
                ...
                if self.strategy in (
                    ...
                    logger.info(
                        f"[Rotation] Entering grace period ({self.grace_period}s)"
                    )
                ...
                if old_creds and old_creds.get("username"):
                    logger.info(
                        f"[Rotation] Revoking old user: {old_creds['username']}"
                    )
                    async with pool.acquire() as conn:
                        await conn.execute(
                            """
                        ...
                        await conn.execute(
                            f"DROP USER IF EXISTS {old_creds['username']}"
                        )

                result.status = RotationStatus.COMPLETED
                result.completed_at = datetime.now()
                logger.info(
                    f"[Rotation] Successfully rotated credentials for {db_name}"
                )
...
class RotationScheduler:
    ...
    def __init__(
        ...
        self._circuit_breaker = CircuitBreaker(
            circuit_breaker_config or CircuitBreakerConfig(), name="scheduler"
        )
    ...
    def schedule_database_rotation(
        ...
        logger.info(
            f"[Scheduler] Scheduled {db_name} rotation every {days} days ({strategy.name})"
        )
    ...
    def schedule_api_key_rotation(
        ...
        logger.info(
            f"[Scheduler] Scheduled {api_name} API key rotation every {days} days"
        )
            ...
            try:
                ...
                for schedule_id, schedule in self._schedules.items():
                    ...
                    should_rotate = (
                        ...
                        or (now - last_rotation) >= schedule["interval"]
                    )

                    if should_rotate and schedule.get("failure_count", 0) < 3:
                        logger.info(
                            f"[Scheduler] Triggering rotation for {schedule_id}"
                        )
