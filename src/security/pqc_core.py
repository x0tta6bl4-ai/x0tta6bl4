...
class PQCKeyExchange:
    ...
    def generate_keypair(self, key_id: str = "", validity_days: int = 365) -> PQCKeyPair:
        """
...
class PQCDigitalSignature:
    ...
    async def sign(self, message: bytes, secret_key: bytes, key_id: str = "") -> PQCSignature:
        """
    ...
    async def verify(self, message: bytes, signature_bytes: bytes, public_key: bytes) -> bool:
        """
...
class PQCHybridScheme:
    ...
    async def setup_secure_channel(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ciphertext, client_secret = await self.kem.encapsulate(server_keypair.public_key)
            server_secret = await self.kem.decapsulate(server_keypair.secret_key, ciphertext)
    ...
    async def verify_certificate(self, cert_data: bytes, signature_bytes: bytes, public_key: bytes) -> bool:
        """
