...
try:
    ...
    if os.getenv("X0TTA6BL4_PRODUCTION", "false").lower() != "true":
        ...
        logger.warning(
            f"⚠️ liboqs-python not available ({type(e).__name__}: {e}). Install with: pip install liboqs-python"
        )
...
class HybridPQEncryption:
    ...
    def hybrid_encapsulate(
        ...
        # Replacing simple hash: derive classical secret
        classical_secret = HKDF(
            algorithm=hashes.SHA256(),
            length=32,
            salt=None,
            info=b"x0tta6bl4-classical-secret",
        ).derive(classical_random + classical_public_key)
