...
class IntrusionDetectionSystem:
    ...
    def detect_intrusion(
        ...
        if confidence < self.confidence_threshold:
            logger.debug(
                f"Intrusion {intrusion_type.value} confidence too low: {confidence}"
            )
            return None
    ...
    def _compare_to_baseline(
        self, indicators: Dict[str, Any], baseline: Dict[str, Any]
    ) -> float:
    ...
    def _determine_severity(
        ...
        # High severity
        if intrusion_type in [
            IntrusionType.UNAUTHORIZED_ACCESS,
            IntrusionType.MALWARE_INFECTION,
        ]:
            if confidence > 0.7:
    ...
    def update_behavioral_baseline(self, node_id: str, metrics: Dict[str, Any]):
        """
    ...
    def respond_to_intrusion(self, alert_id: str, action: str) -> bool:
        """
