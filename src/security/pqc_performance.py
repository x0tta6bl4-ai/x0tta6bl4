...
class PQCKeyCache:
    ...
    def __init__(self, max_size: int = 1000, ttl_seconds: int = 3600):
        ...
        logger.info(f"✅ PQC Key Cache initialized (max_size={max_size}, ttl={ttl_seconds}s)")
    ...
    def clear_expired(self):
        ...
        with self._lock:
            ...
            expired_keys = [
                key_id for key_id, (_, cached_time) in self._cache.items()
                if now - cached_time >= self.ttl_seconds
            ]
            ...
            expired_sessions = [
                key for key, (_, cached_time) in self._session_keys.items()
                if now - cached_time >= self.ttl_seconds
            ]
...
class PQCPerformanceOptimizer:
    ...
    def __init__(
        ...
        # Key cache
        self.cache = PQCKeyCache(max_size=cache_size, ttl_seconds=cache_ttl) if enable_cache else None
    ...
    def _init_ebpf(self):
        ...
        try:
            ...
            import sys
            if 'ebpf' in sys.modules or hasattr(sys.modules.get('src.network.ebpf', None), 'XDPProgram'):
    ...
    def optimized_handshake(
        ...
        if total_time > 0.5:
            ...
            logger.debug(
                f"✅ Fast handshake: {total_time:.3f}ms for {peer_id}"
            )
...
class OptimizedPQMeshSecurity(PQMeshSecurityLibOQS):
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        stats.update({
            ...
            'cache_size': len(self.optimizer.cache._cache) if self.optimizer.cache else 0
        })
        return stats
