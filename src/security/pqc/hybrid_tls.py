...
class HybridTLSContext:
    ...
    def __init__(self, role: str = "peer", kem_alg: str = "Kyber768", sig_alg: str = "ML-DSA-65"):
        ...
        # Ключи для KEM (Kyber)
        self.kem_public_key, self.kem_private_key = self.pqc_adapter.kem_generate_keypair()

        # Ключи для подписи (Dilithium)
        self.sig_public_key, self.sig_private_key = self.pqc_adapter.sig_generate_keypair()
    ...
    def set_peer_public_bundle(self, bundle: Dict[str, bytes]):
        ...
        self.peer_ecc_public_key = serialization.load_pem_public_key(bundle["ecc_public"], default_backend())
    ...
    def compute_session_key(self, encapsulation: Optional[bytes] = None) -> bytes:
        ...
        if self.role == "server" and encapsulation:
            ...
            kyber_secret = self.pqc_adapter.kem_decapsulate(self.kem_private_key, encapsulation)
        else:
...
def hybrid_handshake(client_ctx: HybridTLSContext, server_ctx: HybridTLSContext) -> Tuple[bytes, bytes]:
    ...
    # Клиент инкапсулирует секрет для сервера
    ciphertext, client_kyber_secret = client_ctx.pqc_adapter.kem_encapsulate(server_ctx.kem_public_key)
    ...
    client_session_key = HKDF(algorithm=hashes.SHA256(), length=32, salt=None, info=b"x0tta6bl4-hybrid-tls-v1").derive(client_combined)
    ...
    server_kyber_secret = server_ctx.pqc_adapter.kem_decapsulate(server_ctx.kem_private_key, ciphertext)
    server_combined = server_ecdhe + server_kyber_secret
    server_session_key = HKDF(algorithm=hashes.SHA256(), length=32, salt=None, info=b"x0tta6bl4-hybrid-tls-v1").derive(server_combined)
