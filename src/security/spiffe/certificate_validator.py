...
class CertificateValidator:
    ...
    def __init__(
        ...
        self._ocsp_cache: Dict[str, tuple[bool, datetime]] = (
            {}
        )  # cert_fingerprint -> (is_revoked, cached_at)
        self._ocsp_cache_ttl = timedelta(hours=1)  # Cache OCSP responses for 1 hour

        # Cache for CRL data
        self._crl_cache: Dict[str, tuple[Set[bytes], datetime]] = (
            {}
        )  # crl_url -> (revoked_serials, cached_at)
        ...
        try:
            ...
            if now > cert.not_valid_after:
                ...
                return (
                    False,
                    None,
                    f"Certificate expired (expired: {cert.not_valid_after})",
                )
            ...
            if cert_age > self.max_cert_age:
                return (
                    False,
                    None,
                    f"Certificate too old (age: {cert_age}, max: {self.max_cert_age})",
                )
            ...
            if not spiffe_id.startswith(f"spiffe://{self.trust_domain}/"):
                return (
                    False,
                    spiffe_id,
                    f"Invalid trust domain (expected: {self.trust_domain})",
                )
            ...
            if expected_spiffe_id and spiffe_id != expected_spiffe_id:
                return (
                    False,
                    spiffe_id,
                    f"SPIFFE ID mismatch (expected: {expected_spiffe_id})",
                )
            ...
            if self.enable_pinning:
                ...
                if self.pinned_certs and cert_fingerprint not in self.pinned_certs:
                    return (
                        False,
                        spiffe_id,
                        f"Certificate not in pinned set (fingerprint: {cert_fingerprint})",
                    )
    ...
    def _validate_certificate_chain(
        self, cert: x509.Certificate, trust_bundle: List[bytes]
    ) -> bool:
        ...
        try:
            ...
            # Check if certificate is issued by any trusted CA using cryptographic verification
    ...
    def _download_and_parse_crl(self, crl_url: str) -> Set[int]:
        ...
        try:
            ...
            with httpx.Client(timeout=10.0) as client:
                ...
                logger.debug(
                    f"Downloaded CRL from {crl_url}: {len(revoked_serials)} revoked certificates"
                )
