...
class SPIFFEAutoRenew:
    ...
    def __init__(
        self, client: "WorkloadAPIClient", config: Optional[AutoRenewConfig] = None
    ):
        """
    ...
    async def stop(self):
        ...
        if self._task:
            ...
            try:
                ...
                logger.warning("Auto-renewal task did not stop within timeout")
            finally:
                self._task = None
    ...
    async def _check_and_renew_x509(self):
        ...
        try:
            ...
            if not current_svid:
                ...
                new_svid = (
                    await self._renew_x509_with_retry()
                )  # Use retry mechanism for initial fetch
                if new_svid:
                    ...
                    if self._on_renewal_failed:
                        self._on_renewal_failed(
                            "x509", Exception("Initial fetch failed")
                        )
                return
            ...
            if self._needs_renewal(current_svid):
                ...
                if new_svid:
                    ...
                    if self._on_renewal_failed:
                        self._on_renewal_failed(
                            "x509", Exception("Renewal failed after retries")
                        )
            else:
                ...
                SVID_X509_EXPIRY_TIMESTAMP.set(
        ...
        for audience_key in list(self._jwt_audiences):
            try:
                ...
                if not cached_jwt or cached_jwt.is_expired():
                    ...
                    new_jwt = await self._renew_jwt_with_retry(
                        audience
                    )  # Use retry mechanism for initial fetch
                    if new_jwt:
                        logger.info(f"✅ JWT SVID fetched for audience: {audience}")
                        SVID_JWT_EXPIRY_TIMESTAMP.labels(audience=audience_str).set(
                            new_jwt.expiry.timestamp()
                        )
                        ...
                        logger.error(
                            f"❌ Failed to fetch initial JWT SVID for audience: {audience}"
                        )
                        AUTO_RENEW_FAILURE_TOTAL.labels(svid_type="jwt").inc()
                        if self._on_renewal_failed:
                            self._on_renewal_failed(
                                f"jwt:{audience_str}", Exception("Initial fetch failed")
                            )
                    continue
                ...
                if self._needs_renewal(cached_jwt):
                    ...
                    if new_jwt:
                        ...
                        logger.error(
                            f"❌ Failed to renew JWT SVID for audience {audience}"
                        )
                        AUTO_RENEW_FAILURE_TOTAL.labels(svid_type="jwt").inc()
                        if self._on_renewal_failed:
                            self._on_renewal_failed(
                                f"jwt:{audience_str}", Exception("Renewal failed")
                            )
                else:
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if attempt < self.config.max_retries - 1:
                    ...
                    logger.error(
                        f"All JWT renewal attempts failed for audience {audience}"
                    )
