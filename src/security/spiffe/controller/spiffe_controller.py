...
try:
    from ..optimizations import SPIREOptimizations, SPIREPerformanceConfig, MultiRegionConfig
...
class SPIFFEController:
        ...
        if enable_optimizations and OPTIMIZATIONS_AVAILABLE:
            try:
                ...
                multi_region = MultiRegionConfig(
                    ...
                    fallback_regions=os.getenv("SPIRE_FALLBACK_REGIONS", "eu-west,asia-pacific").split(",")
                )
                
                self.optimizations = SPIREOptimizations(
                    performance_config=perf_config,
                    multi_region_config=multi_region
                )
                logger.info("✅ SPIFFE/SPIRE optimizations enabled (Paradox Zone)")
            except Exception as e:
                logger.warning(f"⚠️ Failed to initialize optimizations: {e}, continuing without optimizations")
    ...
    def _renew_identity(self):
        ...
        try:
            ...
            if new_identity:
                old_id = self.current_identity.spiffe_id if self.current_identity else "unknown"
    ...
    def validate_peer(self, peer_svid: X509SVID, expected_spiffe_id: Optional[str] = None) -> bool:
        """
        ...
        if peer_svid.cert_chain:
            ...
            trust_bundle = peer_svid.cert_chain[1:] if len(peer_svid.cert_chain) > 1 else None
        ...
        return self.workload_api.validate_peer_svid(peer_svid, expected_id=expected_spiffe_id)
