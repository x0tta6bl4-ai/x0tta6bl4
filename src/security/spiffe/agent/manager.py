...
class SPIREAgentManager:
    ...
    def attest_node(
        ...
        **attestation_data
        ...
        logger.warning("Attestation strategy %s is not fully implemented", strategy.value)
        return False
    ...
    def register_workload(self, entry: WorkloadEntry) -> bool:
        ...
        cmd = [
            self._spire_server_bin, "entry", "create",
        ...
        try:
            ...
            logger.info("Successfully registered workload entry: %s", result.stdout.strip())
            return True
            ...
            logger.exception("An unexpected error occurred during workload registration.")
            return False
    ...
    def health_check(self) -> bool:
        ...
        try:
            result = subprocess.run(
                [self._spire_agent_bin, "healthcheck", "-socketPath", str(self.socket_path)],
                capture_output=True,
                timeout=5,
            )
