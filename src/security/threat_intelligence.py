...
class BloomFilter:
    ...
    @classmethod
    def from_bytes(
        cls, data: bytes, size: int = 10000, num_hashes: int = 7
    ) -> "BloomFilter":
...
class ThreatIntelligenceEngine:
    ...
    def report_indicator(
        ...
        with self._lock:
            ...
            if indicator_id in self.indicators:
                ...
                logger.info(
                    f"Updated indicator {indicator_id}, corroborations: {existing.corroborations}"
                )
                return existing
    ...
    def receive_indicators(
        self, indicators: List[Dict[str, Any]], source_node: str
    ) -> int:
        """
        ...
        logger.info(
            f"Received {len(indicators)} indicators from {source_node}, added {added} new"
        )
        return added
    ...
    def detect_dos_attack(self, source: str) -> Optional[ThreatIndicator]:
        ...
        self.connection_counts[source] = [
            t for t in self.connection_counts[source] if now - t < window
        ]
    ...
    def detect_brute_force(self, source: str) -> Optional[ThreatIndicator]:
        ...
        self.failed_auths[source] = [
            t for t in self.failed_auths[source] if now - t < window
        ]
    ...
    def update_reputation(self, entity: str, delta: float) -> float:
        ...
        with self._lock:
            ...
            if new_score < 0.1:
                ...
                logger.warning(
                    f"Entity {entity} blocked due to low reputation: {new_score}"
                )
    ...
    def create_report(
        ...
        report_id = hashlib.sha256(f"{title}:{time.time()}".encode()).hexdigest()[:16]

        indicators = [
            self.indicators[iid] for iid in indicator_ids if iid in self.indicators
        ]
    ...
    def cleanup_expired(self) -> int:
        ...
        with self._lock:
            expired = [iid for iid, ind in self.indicators.items() if ind.is_expired()]
            for iid in expired:
