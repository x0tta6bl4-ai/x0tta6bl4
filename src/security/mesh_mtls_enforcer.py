...
from src.security.spiffe.mtls import MTLSControllerProduction, MTLSContext, TLSRole
...
class MeshMTLSEnforcer:
    ...
    def __init__(
        ...
        logger.info(f"✅ Mesh mTLS Enforcer initialized (TLS13={enforce_tls13}, VerifySVID={verify_svid})")
    ...
    def verify_tls_version(self, ssl_context: ssl.SSLContext):
        ...
        if hasattr(ssl_context, 'minimum_version'):
            ...
            logger.debug(f"✓ TLS 1.3 enforcement verified (min_version={ssl_context.minimum_version})")
    
    def verify_peer_svid(
        self,
        peer_cert_der: bytes,
        expected_spiffe_ids: Optional[list] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                ...
                if not peer_spiffe_id.startswith(f"spiffe://{self.trust_domain}/"):
                    raise ValueError(f"SPIFFE ID {peer_spiffe_id} does not belong to trust domain {self.trust_domain}")
                
                # Check expected SPIFFE IDs if provided
                if expected_spiffe_ids and peer_spiffe_id not in expected_spiffe_ids:
                    raise ValueError(f"SPIFFE ID {peer_spiffe_id} not in expected list: {expected_spiffe_ids}")
                
                # Check certificate expiration
                if cert.not_valid_after < datetime.utcnow():
                    raise ValueError(f"Certificate expired at {cert.not_valid_after}")
                
                time_to_expiry = (cert.not_valid_after - datetime.utcnow()).total_seconds()
                ...
                raise ValueError("Certificate does not contain SAN extension with SPIFFE IDs")
    ...
    def verify_certificate_chain(self, cert_chain: list) -> bool:
        ...
        try:
            ...
            if leaf_cert.not_valid_after < datetime.utcnow():
                raise ValueError(f"Leaf certificate expired at {leaf_cert.not_valid_after}")
            
            # Check intermediate/root certificates if present
            for i, cert_der in enumerate(cert_chain[1:], 1):
                intermediate = x509.load_der_x509_certificate(cert_der, default_backend())
                if intermediate.not_valid_after < datetime.utcnow():
                    raise ValueError(f"Certificate at index {i} expired at {intermediate.not_valid_after}")
    ...
    async def setup_secure_client(self) -> httpx.AsyncClient:
        ...
        try:
            ...
            client = httpx.AsyncClient(
                ...
                cert=(
                    self.mtls_context.cert_file.name if self.mtls_context.cert_file else None,
                    self.mtls_context.key_file.name if self.mtls_context.key_file else None
                ),
