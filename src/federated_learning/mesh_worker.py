...
class FLMeshWorker:
    ...
    async def start(self):
        ...
        success = self.coordinator.register_node(
            self.node_id, capabilities=self.capabilities
        )
    ...
    async def _collect_metrics(self) -> Optional[MeshMetrics]:
        ...
        try:
            ...
            # Non-blocking snapshot to avoid cumulative delays in tight loops.
        ...
        if self.mesh_router:
            ...
            if peers_count > 0:
                latencies = [p.get("latency", 0.0) for p in stats.get("peers", [])]
    ...
    async def train_local_model(self, round_number: int) -> Optional[ModelUpdate]:
        ...
        if len(self.metrics_history) < 10:
            logger.warning(
                f"Not enough metrics for training: {len(self.metrics_history)}"
            )
            return None
        ...
        try:
            ...
            features = [
                m.to_feature_vector() for m in self.metrics_history[-100:]
            ]  # Use last 100
            ...
            loss = sum(
                abs(f[i] - weights[i]) for f in features for i in range(num_features)
            ) / (len(features) * num_features)

            # Create model weights
            model_weights = ModelWeights(weights=weights, num_samples=len(features))
