...
class ModelWeights:
    ...
    # Legacy flat representation expected by older integration tests.
    ...
    def __post_init__(self):
        # Backward/forward compatibility between flat and structured formats.
        if self.weights and not self.layer_weights:
            self.layer_weights["flat"] = list(self.weights)
        elif not self.weights and self.layer_weights:
            self.weights = self.to_flat_vector()
    ...
    def from_flat_vector(
        cls, vector: List[float], layer_shapes: Dict[str, Tuple[int, int]]
    ) -> "ModelWeights":
...
class ModelUpdate:
    ...
    def __post_init__(self):
        ...
        if self.loss and not self.training_loss:
            self.training_loss = self.loss
        elif self.training_loss and not self.loss:
            self.loss = self.training_loss

        if self.training_time and not self.training_time_seconds:
            self.training_time_seconds = self.training_time
        elif self.training_time_seconds and not self.training_time:
            self.training_time = self.training_time_seconds
...
class FLMessage:
    ...
    def heartbeat(cls, node_id: str, status: Dict[str, Any]) -> "FLMessage":
        ...
        return cls(
            msg_type=FLMessageType.HEARTBEAT, content={"status": status}, sender=node_id
        )
