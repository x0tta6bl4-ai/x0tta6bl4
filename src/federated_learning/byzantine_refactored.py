...
class ByzantineRefactored:
    ...
    def _compute_distances(
        ...
        try:
            ...
            distances = np.linalg.norm(
                vec_array[:, np.newaxis] - vec_array[np.newaxis, :], axis=2
            )
    ...
    def _weighted_average(
        ...
        avg = [
            sum(vectors[i][j] * weights[i] for i in range(len(vectors))) / total_weight
            for j in range(vec_dim)
        ]
        return avg

    # CC = 1 (just tracking)
    def _identify_byzantine(
        self, scores: List[Tuple[float, int]], updates: List[Any]
    ) -> List[str]:
