    ...
    def _initialize_fl(self) -> None:
        ...
        try:
            ...
            config = GraphSAGEFLConfig(
                ...
                enable_byzantine_robust=self.fl_config.get(
                    "enable_byzantine_robust", True
                ),
                aggregation_method=self.fl_config.get(
            ...
            self.coordinator = GraphSAGEFLCoordinator(
                node_id=self.node_id, fl_config=config
            )
...
def create_fl_integration(
    node_id: str, enable_fl: bool = True, **kwargs
) -> FLAppIntegration:
    """
