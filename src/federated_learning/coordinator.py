...
class NodeInfo:
    ...
    def is_eligible(self, min_trust: float = 0.5) -> bool:
        ...
        return (
            self.status in (NodeStatus.ONLINE, NodeStatus.IDLE)
            and self.trust_score >= min_trust
            and self.byzantine_violations < 3
        )
...
class FederatedCoordinator:
    ...
    def __init__(self, coordinator_id: str, config: Optional[CoordinatorConfig] = None):
        ...
        try:
            ...
            if self.config.aggregation_method in ["enhanced_fedavg", "adaptive"]:
                ...
                self.aggregator: Aggregator = get_aggregator(
                    self.config.aggregation_method, f=self.config.byzantine_tolerance
                )
            else:
    ...
    def register_node(
        self, node_id: str, capabilities: Optional[Dict[str, Any]] = None
    ) -> bool:
    ...
    def start_round(
        ...
        with self._lock:
            ...
            weights = [self.nodes[n].trust_score for n in eligible]
            total_weight = sum(weights)
            weights = [w / total_weight for w in weights]

            num_to_select = min(self.config.target_participants, len(eligible))
    ...
    def submit_update(self, update: ModelUpdate) -> bool:
        ...
        with self._lock:
            ...
            if self.current_round.status not in (RoundStatus.STARTED, RoundStatus.COMPLETED):
                logger.warning(
                    f"Round not accepting updates: {self.current_round.status}"
                )
                return False
            ...
            # For completed rounds, only collect late updates for compatibility;
            # do not retrigger aggregation/state transitions.
            if self.current_round.status == RoundStatus.COMPLETED:
                return True

            # Aggregate when minimum participation is reached (legacy/unit contract).
    ...
    def _trigger_aggregation(self) -> None:
        ...
        try:
            ...
            result = self.aggregator.aggregate(
                updates=updates, previous_model=self.global_model
            )
            ...
            for node_id in self.current_round.selected_nodes:
                if (
                    node_id in self.nodes
                    and self.nodes[node_id].status == NodeStatus.TRAINING
                ):
    ...
    def initialize_model(self, weights: ModelWeights) -> GlobalModel:
        ...
        with self._lock:
            self.global_model = GlobalModel(
                version=0, round_number=0, weights=weights, aggregation_method="initial"
            )
