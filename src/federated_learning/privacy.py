...
class PrivacyBudget:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "avg_noise_scale": (
                sum(self.noise_scales) / len(self.noise_scales)
                if self.noise_scales
                else 0
            ),
        }
...
class GradientClipper:
    ...
    def clip_batch(
        self, batch_gradients: List[List[float]]
...
class GaussianNoiseGenerator:
    ...
    def calibrate_noise(
        self, sensitivity: float, epsilon: float, delta: float = 1e-5
    ) -> float:
        """
...
class DifferentialPrivacy:
    ...
    def privatize_gradients(
        self, gradients: List[float], num_samples: int = 1
    ...
    def privatize_model_update(
        self, weights: List[float], num_samples: int = 1
    ) -> Tuple[List[float], Dict[str, float]]:
        """
...
class SecureAggregation:
    ...
    def generate_masks(
        self, party_id: int, vector_size: int
    ...
    def mask_update(self, update: List[float], party_id: int) -> List[float]:
        """
    ...
    def aggregate_masked(self, masked_updates: List[List[float]]) -> List[float]:
        """
...
def compute_dp_sgd_privacy(
    sample_rate: float, noise_multiplier: float, epochs: int, delta: float = 1e-5
) -> float:
    """
    ...
    epsilon = (
        sample_rate * math.sqrt(2 * steps * math.log(1 / delta)) / noise_multiplier
    )
