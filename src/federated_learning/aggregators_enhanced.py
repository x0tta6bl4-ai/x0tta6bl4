...
class EnhancedAggregator(Aggregator):
    ...
    def aggregate(
        ...
        metrics.updates_rejected = (
            result.updates_rejected if hasattr(result, "updates_rejected") else 0
        )
    ...
    def _calculate_weight_drift(
        self, previous_model: GlobalModel, current_model: GlobalModel
    ) -> float:
        """
        ...
        try:
            ...
            avg_magnitude = math.sqrt(
                sum(w * w for w in prev_weights) / len(prev_weights)
            )
            if avg_magnitude > 0:
    ...
    def get_aggregation_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_aggregation_time": sum(m.aggregation_time_seconds for m in recent)
            / len(recent),
            "avg_quality_score": sum(m.quality_score for m in recent) / len(recent),
            "avg_convergence_score": sum(m.convergence_score for m in recent)
            / len(recent),
            "avg_updates_accepted": sum(m.updates_accepted for m in recent)
            / len(recent),
            "avg_memory_mb": sum(m.memory_used_mb for m in recent) / len(recent),
            "total_byzantine_detected": sum(
                m.byzantine_detected for m in self.metrics_history
            ),
        }
...
class EnhancedFedAvgAggregator(EnhancedAggregator, FedAvgAggregator):
    ...
    def __init__(self, enable_metrics: bool = True):
        ...
        EnhancedAggregator.__init__(
            self, name="enhanced_fedavg", enable_metrics=enable_metrics
        )
...
class AdaptiveAggregator(EnhancedAggregator):
    ...
    def _select_strategy(
        self, updates: List[ModelUpdate], previous_model: Optional[GlobalModel]
    ) -> str:
        """
        ...
        if n >= 3:
            try:
                ...
                if avg_variance > self.outlier_threshold:
                    logger.info(
                        f"⚠️ High variance detected ({avg_variance:.2f}), using trimmed_mean"
                    )
                    return "trimmed_mean"
            except Exception as e:
...
def get_enhanced_aggregator(
    method: str = "enhanced_fedavg", **kwargs
) -> EnhancedAggregator:
    """
    ...
    if method not in aggregators:
        raise ValueError(
            f"Unknown enhanced aggregator: {method}. Available: {list(aggregators.keys())}"
        )
