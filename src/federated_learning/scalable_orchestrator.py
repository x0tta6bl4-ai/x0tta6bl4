...
class ScalableNodeRegistry:
    ...
    def remove_node(self, node_id: str) -> None:
        ...
        to_delete = [
            hash_val for hash_val, nid in self.ring.items()
            if nid == node_id
        ]
...
class ScalableFLOrchestrator:
    ...
    def _select_coordinator(self) -> str:
        ...
        return min(
            ...
            key=lambda cid: len(self.coordinators[cid].nodes)
        )
    ...
    def _adaptive_node_selection(self, target_count: int) -> Set[str]:
        ...
        selected = np.random.choice(
            node_ids,
            size=min(target_count, len(node_ids)),
            replace=False,
            p=weights
        )
...
class CoordinatorProxy:
    ...
    async def aggregate(self, round_number: int) -> bool:
        ...
        logger.info(
            f"{self.coordinator_id} aggregating round {round_number}"
        )
        return True
...
class ByzantineRobustAggregator:
    ...
    @staticmethod
    def multikrum_aggregation(updates: List[np.ndarray], 
                             num_byzantine: int,
                             m: int = 1) -> np.ndarray:
        """
...
class GradientCompressor:
    ...
    @staticmethod
    def top_k_sparsify(gradient: np.ndarray, k_percent: float = 0.1) -> Tuple[np.ndarray, Dict]:
        """
...
class AdaptiveClientSampler:
    ...
    def select_clients(self, round_num: int, 
                      ...
                      exclude_stragglers: bool = False) -> List[str]:
        """
        ...
        for client_id in range(self.total_clients):
            ...
            if exclude_stragglers:
                straggler_ratio = len(self.straggler_history[client_str]) / max(1, round_num)
    ...
    def update_convergence_score(self, client_id: str, improvement: float):
        ...
        self.convergence_scores[client_id] = 0.7 * self.convergence_scores[client_id] + 0.3 * improvement
