...
class EnhancedKrumAggregator(KrumAggregator):
        ...
        try:
            ...
            self._stats["avg_aggregation_time"] = (
                (self._stats["avg_aggregation_time"] * (self._stats["total_rounds"] - 1) +
                 aggregation_time) / self._stats["total_rounds"]
            )
    ...
    def _compute_distances_vectorized(self, vectors: List[List[float]]) -> List[List[float]]:
    ...
    def aggregate(
        ...
        try:
            ...
            for d in range(dim):
                ...
                filtered_values = [
                    v for v in values if v[1] not in outlier_indices
                ]
                ...
                if trim_count > 0 and len(filtered_values) > 2 * trim_count:
                    trimmed = filtered_values[trim_count:len(filtered_values) - trim_count]
                else:
            ...
            self._stats["avg_trimmed"] = (
                (self._stats["avg_trimmed"] * (self._stats["total_rounds"] - 1) +
                 2 * trim_count) / self._stats["total_rounds"]
            )
    ...
    def _detect_outliers(
        ...
        try:
            ...
            if method == "iqr":
                ...
                for i in range(n):
                    if np.any(vec_array[i] < lower_bound) or np.any(vec_array[i] > upper_bound):
