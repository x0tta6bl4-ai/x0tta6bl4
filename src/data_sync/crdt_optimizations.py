...
try:
    from .crdt_sync import CRDT, LWWRegister, Counter, ORSet, CRDTSync
    CRDT_AVAILABLE = True
except ImportError:
...
class CRDTSyncOptimizer:
    ...
    def __init__(self, node_id: str, enable_compression: bool = True):
        ...
        self.conflict_resolution = "last_write_wins"  # or "merge", "manual", "vector_clock"
    ...
    def generate_deltas(self, key: str, include_vector_clock: bool = True) -> List[CRDTDelta]:
        """
        ...
        for key, peer_crdt_data in peer_state.items():
            if key in self.sync_manager.crdts:
                ...
                try:
                    ...
                    if isinstance(peer_crdt_data, dict) and "vector_clock" in peer_crdt_data:
                        peer_vector_clock = peer_crdt_data["vector_clock"]
                    
                    if peer_crdt:
                        # Use conflict-free merge strategy
                        merge_strategy = self.merge_strategies.get(self.conflict_resolution, self._merge_last_write_wins)
                        merge_strategy(key, peer_crdt, peer_vector_clock)
                        self.last_sync_state[key] = self._get_state_hash(self.sync_manager.crdts[key])
                except Exception as e:
    ...
    def _merge_last_write_wins(self, key: str, peer_crdt: Any, peer_vector_clock: Optional[VectorClock] = None):
        """Last-write-wins merge strategy"""
        self.sync_manager.crdts[key].merge(peer_crdt)
    
    def _merge_vector_clock(self, key: str, peer_crdt: Any, peer_vector_clock: Optional[VectorClock] = None):
    ...
    def _merge_all(self, key: str, peer_crdt: Any, peer_vector_clock: Optional[VectorClock] = None):
        """Merge all strategy (for set-based CRDTs)"""
        self.sync_manager.crdts[key].merge(peer_crdt)
    
    def _merge_manual(self, key: str, peer_crdt: Any, peer_vector_clock: Optional[VectorClock] = None):
    ...
    def _run_garbage_collection(self):
        ...
        for key, deltas in self.pending_deltas.items():
            ...
            self.pending_deltas[key] = [
                ...
                if now - delta.timestamp < self.gc_ttl
            ]
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                ...
                if self.metrics.total_syncs > 0 else 0.0
            ),
