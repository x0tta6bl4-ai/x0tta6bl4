...
class TokenBridge:
    ...
    def register_address(self, node_id: str, eth_address: str):
        ...
        if WEB3_AVAILABLE:
            ...
            eth_address = eth_address if eth_address.startswith("0x") else f"0x{eth_address}"
    ...
    async def _process_event_type(self, event_name: str, from_block: int, to_block: int):
    ...
    async def _handle_event(self, event_name: str, event: Any):
        ...
        # Record transaction
        self._tx_history.append(BridgeTransaction(
            tx_id=f"{event_name}_{tx_hash[:8]}",
            direction=BridgeDirection.FROM_CHAIN,
            from_address=str(args.get("from", args.get("user", args.get("payer", "")))),
    ...
    async def _sync_relay_payment(self, payer: str, relayer: str, amount_wei: int):
        ...
        if payer_node and relayer_node:
            logger.info(f"Synced relay payment: {payer_node} → {relayer_node}: {amount} X0T")
    ...
    async def push_rewards_to_chain(
        ...
        try:
            ...
            tx = self.contract.functions.distributeEpochRewards(
                ...
                "gasPrice": self.web3.to_wei(self.config.max_gas_price_gwei, "gwei"),
    ...
    async def authorize_relayer(self, node_id: str, authorized: bool = True) -> Optional[str]:
        """
    ...
    def get_chain_stats(self) -> Dict:
        ...
        try:
            return {
                "total_staked": float(self.contract.functions.totalStaked().call()) / 1e18,
...
class EpochRewardScheduler:
    ...
    async def _distribute_epoch(self):
        ...
        tx_hash = await self.bridge.push_rewards_to_chain(
            rewards={},  # Actual amounts calculated on-chain
            uptimes=uptime_ints
        )
