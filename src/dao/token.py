...
class MeshToken:
    ...
    def transfer(self, from_node: str, to_node: str, amount: float) -> bool:
        ...
        if self.balance_of(from_node) < amount:
            logger.warning(
                f"Insufficient balance: {from_node} has {self.balance_of(from_node)}, needs {amount}"
            )
            return False
    ...
    def _record_metrics_transfer(self, from_node: str, to_node: str):
        ...
        try:
            from src.monitoring.metrics import (record_token_transfer,
                                                set_token_balance)
    ...
    def reward_deployment(
        self, node_id: str, referrer_id: Optional[str] = None
    ) -> bool:
        ...
        if success and referrer_id and referrer_id in self.balances:
            ...
            self.mint(
                referrer_id, self.REFERRAL_BONUS, reason=f"referral_bonus_{node_id}"
            )
            logger.info(
                f"🎁 Referral bonus 100 X0T sent to {referrer_id} for {node_id}"
            )
        return success
    ...
    def stake(self, node_id: str, amount: float) -> bool:
        ...
        if node_id in self.stakes:
            ...
            self.stakes[node_id] = StakeInfo(
                amount=amount, staked_at=now, lock_until=now + self.STAKE_LOCK_SECONDS
            )

        logger.info(
            f"Staked {amount} {self.SYMBOL} by {node_id} (total: {self.stakes[node_id].amount})"
        )
    ...
    def _record_metrics_stake(self, node_id: str):
        ...
        try:
            from src.monitoring.metrics import (set_token_balance,
                                                set_token_staked)
    ...
    def unstake(self, node_id: str, amount: float) -> bool:
        ...
        if remaining < self.MIN_STAKE:
            ...
            logger.info(
                f"Unstaked {amount} {self.SYMBOL} by {node_id} (remaining: {remaining})"
            )
    ...
    def pay_for_resource(
        ...
        logger.info(
            f"Resource payment: {payer_node} paid {price} to {provider_node} for {amount} {resource_type.value}"
        )
    ...
    def distribute_epoch_rewards(
        self, node_uptimes: Dict[str, float]
    ) -> Dict[str, float]:
        """
        ...
        if total_score == 0:
            logger.info(
                f"Epoch {self.epoch_number}: No rewards distributed (no eligible stakers)"
            )
            return rewards
        ...
        logger.info(
            f"Epoch {self.epoch_number}: Distributed {sum(rewards.values()):.2f} {self.SYMBOL} to {len(rewards)} nodes"
        )
        return rewards
