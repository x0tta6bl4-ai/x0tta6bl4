...
class MetricsBuffer:
    ...
    def __init__(self, parameter: str, max_points: int = 10000):
        ...
        self.last_stats: MetricStatistics | None = None
    ...
    def get_statistics(self, force_recalc: bool = False) -> MetricStatistics:
        ...
        if (
            not force_recalc
            and self.last_stats
            and (now - self.last_stats_update) < self.stats_cache_interval
        ):
    ...
    def get_trend(self, seconds: int = 3600) -> str | None:
...
class SelfLearningThresholdOptimizer:
    ...
    def _generate_recommendation(
        self, parameter: str, buffer: MetricsBuffer, stats: MetricStatistics
    ) -> ThresholdRecommendation | None:

        strategies_scores = {}

        p95_threshold = stats.p95
        strategies_scores["percentile"] = (
            p95_threshold,
            0.85,
    ...
    def detect_anomalies(
        self, parameter: str, sensitivity: float = 2.0
    ) -> list[MetricPoint]:
        ...
        self.anomaly_counts[parameter] = self.anomaly_counts.get(parameter, 0) + len(
            anomalies
        )
    ...
    def get_statistics(self, parameter: str) -> MetricStatistics | None:
        if parameter not in self.buffers:
            return None
        return self.buffers[parameter].get_statistics()

    def get_recommendation(self, parameter: str) -> ThresholdRecommendation | None:
    ...
    def get_learning_stats(self) -> dict[str, Any]:
        return {
            ...
            "anomaly_detection_rate": {
                param: self.anomaly_counts.get(param, 0)
                / max(
                    1,
                    self.anomaly_counts.get(param, 0)
                    + self.normal_counts.get(param, 0),
                )
                * 100
    ...
    def export_thresholds(self) -> dict[str, float]:
        return {
            param: rec.recommended_value for param, rec in self.recommendations.items()
        }
