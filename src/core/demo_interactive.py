...
# CORS для frontend — ограничиваем origins
...
class InteractiveDemo:
    ...
    def create_session(self, num_nodes: int = 5) -> str:
        ...
        for i in range(num_nodes):
            ...
            nodes[node_id] = MeshNode(
                id=node_id, x=100 + (i % 3) * 200, y=100 + (i // 3) * 200
            )
        ...
        # Создать сессию
        session = DemoSession(session_id=session_id, nodes=nodes, links=links)
    ...
    async def destroy_node(self, session_id: str, node_id: str) -> Dict:
        ...
        session.add_event(
            "anomaly_detected",
            node_id,
            {"detection_time": detection_time - start_time, "confidence": 0.98},
        )
        ...
        session.add_event("recovery_initiated", node_id, {"timestamp": time.time()})
        ...
        session.add_event(
            "node_recovered",
            node_id,
            {"recovery_time": node.recovery_time, "mttr": node.recovery_time},
        )
    ...
    def get_session_status(self, session_id: str) -> Dict:
        ...
        return {
            ...
            "links": [
                {"source": link.source, "target": link.target, "status": link.status}
                for link in session.links
            ],
...
async def create_demo(num_nodes: int = 5):
    ...
    if num_nodes < 3 or num_nodes > 10:
        raise HTTPException(
            status_code=400, detail="num_nodes must be between 3 and 10"
        )
...
async def get_metrics(session_id: str):
    ...
    return {
        ...
        "uptime_percent": (
            100.0 if total_failures == 0 else (total_recoveries / total_failures * 100)
        ),
        "healthy_nodes": sum(
            1 for n in session.nodes.values() if n.status == NodeStatus.HEALTHY
        ),
