...
class ChaosEngine:
    ...
    async def inject_node_failure(
        self, node_id: str, duration: float = 30.0, severity: float = 1.0
    ) -> ChaosEvent:
        """
        Inject node failure.
    ...
    async def inject_network_partition(
        self, node_ids: List[str], duration: float = 60.0
    ) -> ChaosEvent:
        """
    ...
    async def inject_high_load(
        self, node_id: str, duration: float = 60.0, load_percent: float = 0.95
    ) -> ChaosEvent:
