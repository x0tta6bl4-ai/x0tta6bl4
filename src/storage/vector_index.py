...
class VectorIndex:
    ...
    def __init__(
        ...
        # Helper to load models with a timeout to avoid blocking the process
        ...
        # Optionally disable automatic embedding model loading (useful for tests/CI)
        if os.getenv("RAG_DISABLE_EMBEDDING_MODEL", "false").lower() == "true":
            logger.info(
                "⚠️ Embedding model loading disabled via RAG_DISABLE_EMBEDDING_MODEL"
            )
        elif SENTENCE_TRANSFORMERS_AVAILABLE:
            try:
                timeout = int(os.getenv("RAG_MODEL_LOAD_TIMEOUT", "5"))
                model = _try_load_model(
                    lambda: SentenceTransformer("all-MiniLM-L6-v2"), timeout=timeout
                )
                if model is not None:
                    ...
                    logger.warning(
                        "⚠️ Embedding model not available (timeout or failure), using fallback"
                    )
            except Exception as e:
                logger.warning(f"⚠️ Failed to initialize embedding model: {e}")

        # Initialize HNSW index (can be disabled in tests/CI to avoid blocking/resource usage)
        if os.getenv("RAG_DISABLE_HNSW", "false").lower() == "true":
            logger.info("⚠️ HNSW initialization disabled via RAG_DISABLE_HNSW")
            self.index = None
        elif HNSW_AVAILABLE:
            self._init_index()
        else:
    ...
    def save(self, path: Optional[Path] = None):
        ...
        try:
            # Save HNSW index (skip if HNSW disabled)
            if (
                os.getenv("RAG_DISABLE_HNSW", "false").lower() != "true"
                and self.index
                and HNSW_AVAILABLE
            ):
    ...
    def load(self, path: Optional[Path] = None):
        ...
        try:
            ...
            # Load HNSW index (skip if disabled)
            if (
                os.getenv("RAG_DISABLE_HNSW", "false").lower() != "true"
                and HNSW_AVAILABLE
            ):
                ...
                if index_file.exists():
                    ...
                    try:
                        ...
                        # If load fails, continue with empty metadata (avoid test hang)
                        self.index = None
