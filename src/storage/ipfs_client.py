...
try:
    ...
    logger.warning("⚠️ ipfshttpclient not available. Install with: pip install ipfshttpclient")
...
class IPFSClient:
    ...
    async def add(self, data: str, pin: bool = True) -> str:
        ...
        try:
            # Add data to IPFS
            result = self.client.add_json(data) if isinstance(data, (dict, list)) else self.client.add_str(data)
