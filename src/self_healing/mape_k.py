...
class MAPEKMonitor:
    ...
    def __init__(
        self,
        knowledge: Optional['MAPEKKnowledge'] = None,
    ...
    def check(self, metrics: Dict) -> bool:
        ...
        if self.threshold_manager or self.knowledge:
            ...
            if metrics.get('cpu_percent', 0) > self.default_thresholds['cpu_percent']:
                return True
            if metrics.get('memory_percent', 0) > self.default_thresholds['memory_percent']:
                return True
            if metrics.get('packet_loss_percent', 0) > self.default_thresholds['packet_loss_percent']:
                return True
        ...
        if self.use_graphsage and self.graphsage_detector:
            try:
                ...
                logger.warning(f"GraphSAGE detection failed: {e}, falling back to threshold")
    ...
    def analyze(self, metrics: Dict, node_id: str = "unknown", event_id: Optional[str] = None) -> str:
        ...
        if self.use_graphsage and self.graphsage_detector:
            try:
                ...
                if hasattr(self.graphsage_detector, 'predict_with_causal'):
                    ...
                    if prediction.is_anomaly:
                        ...
                        if causal_result and causal_result.root_causes:
                            ...
                            logger.info(f"GraphSAGE+Causal root cause: {root_cause.explanation}")
                            return issue
                        else:
                ...
                logger.warning(f"GraphSAGE analysis failed: {e}, falling back to threshold-based")
...
class MAPEKKnowledge:
    ...
    def __init__(self, knowledge_storage=None):
        ...
        self.successful_patterns: Dict[str, List[Dict]] = {}  # issue -> [successful incidents]
        self.failed_patterns: Dict[str, List[Dict]] = {}  # issue -> [failed incidents]
        self.threshold_adjustments: Dict[str, float] = {}  # metric -> adjusted threshold
                ...
                if hasattr(self.knowledge_storage, 'record_incident_sync'):
                    ...
                    try:
                        ...
                        if loop.is_running():
                            ...
                            asyncio.create_task(self.knowledge_storage.store_incident(incident_entry, node_id))
                        else:
                            # If no loop, run it
                            loop.run_until_complete(self.knowledge_storage.store_incident(incident_entry, node_id))
                    except RuntimeError:
                        # No event loop, create new one
                        asyncio.run(self.knowledge_storage.store_incident(incident_entry, node_id))
                    logger.debug(f"📚 Stored incident in Knowledge Storage v2.0: {issue}")
            except Exception as e:
        ...
        if self.knowledge_storage:
            try:
                ...
                if hasattr(self.knowledge_storage, 'search_patterns_sync'):
                    ...
                    if results:
                        logger.debug(f"🔍 Found {len(results)} patterns from Knowledge Storage")
                        return results
                    ...
                    try:
                        ...
                        if loop.is_running():
                            ...
                            logger.debug("⚠️ Cannot run async search in running loop, using local patterns")
    ...
    def get_average_mttr(self, issue: str) -> Optional[float]:
        ...
        if not successful_incidents:
            ...
            if self.knowledge_storage:
                try:
                    ...
                    if patterns:
                        mttr_values = [p.get('mttr', 0) for p in patterns if p.get('mttr') and p.get('mttr', 0) > 0]
                        if mttr_values:
        ...
        mttr_values = [inc.get('mttr', 0) for inc in successful_incidents if inc.get('mttr') and inc.get('mttr', 0) > 0]
    ...
    def get_recommended_action(self, issue: str) -> Optional[str]:
        ...
        best_action = min(
            action_scores.items(),
            key=lambda x: x[1]['total_mttr'] / x[1]['count']
        )[0]
        
        return best_action
    
    def get_adjusted_threshold(self, metric_name: str, default_threshold: float) -> float:
        """
        ...
        for metric_name, value in metrics.items():
            if isinstance(value, (int, float)):
                if metric_name not in self.threshold_adjustments:
                    self.threshold_adjustments[metric_name] = value * 1.1  # Initial: 10% above
...
class SelfHealingManager:
    ...
    def _apply_feedback_loop(
        ...
        if success and mttr < 3.0:  # Very successful recovery
            ...
            logger.debug(f"Feedback: Successful recovery (MTTR={mttr:.3f}s), reinforcing patterns")
        elif not success or mttr > 7.0:  # Failed or slow recovery
            # Tighten thresholds (need earlier detection)
            self.threshold_adjustments += 1
            logger.debug(f"Feedback: Failed/slow recovery (MTTR={mttr:.3f}s), adjusting thresholds")
        ...
        if success:
            ...
            logger.debug(f"Feedback: Action '{action}' successful for '{issue}', reinforcing strategy")
