...
class SemanticCache:
        ...
        if enable_embedding_model and SENTENCE_TRANSFORMERS_AVAILABLE:
            try:
                ...
                if model is not None:
                    ...
                    logger.warning(
                        "⚠️ SemanticCache embedding model not available (timeout or failure)"
                    )
            except Exception as e:
    ...
    def _find_similar_cached_query(self, query_embedding: np.ndarray) -> Optional[str]:
        ...
        for cache_key, cached_result in self.cache.items():
            ...
            similarity = self._compute_similarity(
                query_embedding, cached_result.query_embedding
            )
    ...
    def put(
        self, query: str, results: List[Dict[str, Any]], scores: List[float]
    ) -> None:
        """
...
class CachedRAGPipeline:
    ...
    def __init__(self, rag_pipeline, cache_config: Optional[Dict[str, Any]] = None):
        """
    ...
    async def retrieve(
        self, query: str, k: int = 10, use_cache: bool = True
    ) -> Dict[str, Any]:
        """
