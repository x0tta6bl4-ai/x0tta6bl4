...
class DocumentChunker:
    ...
    def __init__(
        ...
        logger.info(
            f"✅ DocumentChunker initialized: {strategy.value}, size={chunk_size}, overlap={chunk_overlap}"
        )
    ...
    def _chunk_sentence(
        ...
        for sentence in sentences:
            ...
            if current_size + sentence_size > self.chunk_size and current_chunk:
                ...
                current_chunk = (
                    [overlap_text[: self.chunk_overlap]]
                    if self.chunk_overlap > 0
                    else []
                )
                current_size = len(current_chunk[0]) if current_chunk else 0
                start_index = (
                    end_index - len(overlap_text)
                    if self.chunk_overlap > 0
                    else end_index
                )
                chunk_idx += 1
    ...
    def _chunk_paragraph(
        ...
        for paragraph in paragraphs:
            ...
            if para_size > self.chunk_size:
                ...
                para_chunks = self._chunk_fixed_size(
                    paragraph, f"{document_id}_para", {}
                )
