...
class BatchRetriever:
    ...
    async def retrieve_batch(
        ...
        throughput = (
            (len(queries) * 1000) / processing_time_ms if processing_time_ms > 0 else 0
        )
    ...
    async def _retrieve_batch_sequential(
        self, queries: List[BatchQuery], use_cache: bool
    ) -> List[BatchResult]:
        ...
        for query in queries:
            ...
            try:
                result = await self.rag_pipeline.retrieve(
                    query.query, k=query.k, use_cache=use_cache
                )
    ...
    async def _retrieve_batch_optimized(
        ...
        tasks = []
        for query in queries:
            task = asyncio.create_task(self._retrieve_single(query, use_cache))
            tasks.append(task)
    ...
    async def _retrieve_single(self, query: BatchQuery, use_cache: bool) -> BatchResult:
    ...
    async def retrieve_batch_chunked(
        ...
        if all_results:
            ...
            throughput = (
                len(all_results) * 1000
            ) / aggregated_stats.total_processing_time_ms
        else:
            avg_latency = 0
            throughput = 0
