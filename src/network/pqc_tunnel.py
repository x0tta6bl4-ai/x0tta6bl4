...
class PQCTunnel:
    ...
    def process_handshake_init(self, data: bytes) -> Tuple[str, bytes, bytes]:
        ...
        # Store session key
        self.session_keys[peer_node_id] = shared_secret
    ...
    def process_handshake_response(self, data: bytes) -> Tuple[str, bytes]:
        ...
        if PQC_AVAILABLE:
            kem = oqs.KeyEncapsulation(self.KEM_ALGORITHM, secret_key=self.keys.private_key)
...
class PQCTunnelManager:
    ...
    async def establish_tunnel(self, reader, writer, peer_id: str) -> bool:
        ...
        try:
            ...
            # Process response
            peer_node_id, shared_secret = self.tunnel.process_handshake_response(resp_data)
    ...
    async def accept_tunnel(self, reader, writer) -> Optional[str]:
        ...
        try:
            ...
            peer_node_id, shared_secret, response = self.tunnel.process_handshake_init(init_data)
