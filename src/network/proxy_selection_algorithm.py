...
class DomainProfile:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.optimal_proxy_scores, dict):
            object.__setattr__(
                self, "optimal_proxy_scores", dict(self.optimal_proxy_scores)
            )
...
class ProxySelectionAlgorithm:
    ...
    async def _select_lowest_latency(
        self, proxies: List[ProxyEndpoint]
    ) -> Optional[ProxyEndpoint]:
        """Select proxy with lowest average latency."""
        scored = [(p, self._get_metrics(p).get_avg_latency()) for p in proxies]
    ...
    async def _select_geographic(
        self, proxies: List[ProxyEndpoint], preferred_region: Optional[str]
    ) -> Optional[ProxyEndpoint]:
        ...
        # Country code match
        country_matches = [
            p for p in proxies if p.country_code == preferred_region.upper()
        ]
        if country_matches:
...
class AdaptiveLoadBalancer:
    ...
    async def acquire_proxy(
        self, proxies: List[ProxyEndpoint], domain: Optional[str] = None
        ...
        async with self._lock:
            self._active_connections[selected.id] = (
                self._active_connections.get(selected.id, 0) + 1
            )
    ...
    async def release_proxy(self, token: str):
        ...
        async with self._lock:
            if proxy_id in self._active_connections:
                self._active_connections[proxy_id] = max(
                    0, self._active_connections[proxy_id] - 1
                )
