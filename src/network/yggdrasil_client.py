...
def get_yggdrasil_status() -> Dict[str, Any]:
    ...
    try:
        ...
        if (
            ...
            or (isinstance(e, OSError) and "No such file or directory" in str(e))
        ):
            logger.warning("yggdrasilctl not found, using mock mode")
            node_id = os.environ.get("NODE_ID", "node-a")
            address_map = {
        ...
        return {"status": "error", "error": f"Failed to get node info: {e.stderr}"}
    except subprocess.TimeoutExpired:
        return {"status": "error", "error": "Command timeout after 5 seconds"}
