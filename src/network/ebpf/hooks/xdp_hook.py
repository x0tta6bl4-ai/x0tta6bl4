...
class XDPHook:
    ...
    def _check_driver_support(self, interface: str, mode: str) -> bool:
        ...
        try:
            ...
            if operstate_path.exists():
                ...
                if operstate != "up":
                    logger.debug(f"Interface {interface} is not up, may affect mode support")
        except Exception:
            pass
    ...
    def attach(
        ...
        for attempt_mode in mode_order:
            ...
            try:
                ...
                cmd = ['ip', 'link', 'set', 'dev', interface, mode_flag, 'obj', program_path, 'sec', 'xdp']
                result = subprocess.run(cmd, capture_output=True, text=True, timeout=5)
                
                if result.returncode == 0:
                    # Verify attachment
                    verify_cmd = ['ip', 'link', 'show', 'dev', interface]
                    verify_result = subprocess.run(verify_cmd, capture_output=True, text=True, timeout=2)
                    ...
                    if 'xdp' in verify_result.stdout.lower():
                        ...
                        logger.debug(f"Attachment succeeded but verification failed for {interface}")
                else:
                    logger.debug(
                ...
                logger.warning(f"ip link command timed out for {interface} in {attempt_mode} mode")
                continue
    ...
    def detach(self, interface: str) -> bool:
        ...
        if not self._check_interface_exists(interface):
            logger.warning(f"Interface {interface} does not exist, removing from tracking")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if 'xdp' not in verify_result.stdout.lower():
                    ...
                    logger.warning(f"Detachment command succeeded but XDP still present on {interface}")
