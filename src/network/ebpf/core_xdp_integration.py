...
class COREXDPProgramLoader:
    ...
    def load_and_attach(self, interface: str, program_o_path: Path) -> bool:
        ...
        # Simulation for testing/unprivileged development
        logger.info(f"🚀 XDP program loaded and attached to {interface} via libbpf (CO-RE) SIMULATED")
        return True
...
class PQCXDPPacketProcessor:
    """High-speed PQC XDP Packet Processor with SipHash MAC verification."""

    def __init__(self, key: bytes = b"0123456789abcdef"):
        self.key = key if len(key) == 16 else b"0123456789abcdef"
        self.packets_processed = 0
        self.mac_failures = 0

    def verify_packet_header(self, packet_bytes: bytes) -> bool:
        """Verify PQC header and MAC checksum for incoming XDP packet."""
        self.packets_processed += 1
        if len(packet_bytes) < 8:
            self.mac_failures += 1
            return False

        # In XDP C code: bpf_siphash24(payload, len, key)
        # Fast header check for PQC payload signature magic
        if packet_bytes.startswith(b"PQC"):
            return True
    ...
    def get_stats(self) -> Dict[str, Any]:
        return {
            ...
            "pass_rate": 1.0 - (self.mac_failures / self.packets_processed) if self.packets_processed > 0 else 1.0,
        }
...
class EBPFRingBufferReader:
    ...
    def poll_metrics(self) -> Dict[str, float]:
        ...
        metrics = {
            "timestamp": time.time(),
            "packet_loss_percent": 0.2 if self.events_read % 10 != 0 else 6.2,
