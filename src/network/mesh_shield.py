...
class MeshShield:
            ...
            for node_id, node in list(self.nodes.items()):
                ...
                # Check for degradation
                    ...
                    if node.status == NodeStatus.HEALTHY:
                        node.status = NodeStatus.DEGRADED
                        logger.warning(
                            f"⚠️ Node {node_id} degraded (beacon delay: {time_since_beacon:.2f}s)"
                        )
            ...
            if failed_node in route:
                ...
                if destination in self.backup_routes:
                    ...
                    new_route = self._calculate_route(
                        destination, exclude=[failed_node]
                    )
                    if new_route:
    ...
    def _calculate_route(
        self, destination: str, exclude: List[str] = None
    ) -> List[str]:
        ...
        candidates.sort(
            key=lambda x: x[1].reputation / max(x[1].latency_ms, 1), reverse=True
        )
    ...
    def get_metrics(self) -> dict:
        ...
        return {
            ...
            "mttr_p95": (
                sorted(mttr_samples)[int(len(mttr_samples) * 0.95)]
                if len(mttr_samples) > 1
                else 0
            ),
            ...
            "recovery_rate": self._metrics["failures_recovered"]
            / max(self._metrics["failures_detected"], 1),
            ...
            "nodes_quarantined": sum(
                1 for n in self.nodes.values() if n.is_quarantined()
            ),
        }
...
class ShieldedMeshRouter:
    ...
    def get_healthy_peers(self) -> List[str]:
        ...
        return [
            ...
            if node.status == NodeStatus.HEALTHY and not node.is_quarantined()
        ]
...
if __name__ == "__main__":
    ...
    async def test_shield():
        ...
        for i in range(10):
            ...
            print(f"\n--- Iteration {i+1} ---")
            print(
                f"Healthy: {metrics['nodes_healthy']}, Degraded: {metrics['nodes_degraded']}, Quarantined: {metrics['nodes_quarantined']}"
            )
