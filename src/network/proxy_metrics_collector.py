...
class ProxyMetricsCollector:
    ...
    async def record_proxy_request(
        ...
        if success:
            ...
            await self.record("proxy_requests_failed", 1, {
                "proxy_id": proxy_id,
    ...
    async def record_domain_request(
        ...
        await self.record("domain_requests_total", 1, {
            "domain": domain,
            "proxy_id": proxy_id
        })
        
        if success:
            await self.record("domain_requests_success", 1, {
                "domain": domain,
                "proxy_id": proxy_id
            })
        else:
            await self.record("domain_requests_failed", 1, {
                "domain": domain,
                "proxy_id": proxy_id
            })
    ...
    def get_proxy_metrics(
        ...
        for name, series in self.metrics.items():
            ...
            cutoff = time.time() - window_seconds
            values = [
                v.value for v in series.values
...
DEFAULT_METRICS = [
    ...
    ("proxy_health_response_time_ms", MetricType.HISTOGRAM, "Proxy health check response time"),
    ...
    ("selection_algorithm_calls", MetricType.COUNTER, "Proxy selection algorithm calls"),
    ("selection_algorithm_latency_ms", MetricType.HISTOGRAM, "Selection algorithm latency"),
]
