...
class ProxyControlPlane:
    ...
    def __init__(self, proxy_manager: ResidentialProxyManager, host: str = "0.0.0.0", port: int = 8081):
    ...
    def _setup_routes(self):
        ...
        self.app.router.add_post("/proxies/{proxy_id}/health-check", self.check_proxy_health)
    ...
    async def health_check(self, request: web.Request) -> web.Response:
        ...
        healthy_count = sum(
            1 for p in self.proxy_manager.proxies
        ...
        return web.json_response({
            ...
            "proxies": {
                ...
                "unhealthy": sum(1 for p in self.proxy_manager.proxies if p.status == ProxyStatus.UNHEALTHY),
                "banned": sum(1 for p in self.proxy_manager.proxies if p.status == ProxyStatus.BANNED)
            },
    ...
    async def get_proxy(self, request: web.Request) -> web.Response:
        ...
        proxy = next(
            (p for p in self.proxy_manager.proxies if p.id == proxy_id),
            None
        )
    ...
    async def get_metrics(self, request: web.Request) -> web.Response:
        ...
        total_requests = sum(p.success_count + p.failure_count for p in self.proxy_manager.proxies)
        total_successes = sum(p.success_count for p in self.proxy_manager.proxies)
        
        avg_response_time = 0.0
        if self.proxy_manager.proxies:
            avg_response_time = sum(p.response_time_ms for p in self.proxy_manager.proxies) / len(self.proxy_manager.proxies)
        
        return web.json_response({
            "proxies": {
                "total": len(self.proxy_manager.proxies),
                "healthy": sum(1 for p in self.proxy_manager.proxies if p.status == ProxyStatus.HEALTHY),
                "degraded": sum(1 for p in self.proxy_manager.proxies if p.status == ProxyStatus.DEGRADED),
                "unhealthy": sum(1 for p in self.proxy_manager.proxies if p.status == ProxyStatus.UNHEALTHY),
                "banned": sum(1 for p in self.proxy_manager.proxies if p.status == ProxyStatus.BANNED)
            },
            "requests": {
            ...
            "performance": {
                ...
                "min_response_time_ms": min((p.response_time_ms for p in self.proxy_manager.proxies), default=0),
                "max_response_time_ms": max((p.response_time_ms for p in self.proxy_manager.proxies), default=0)
            },
            "domains": {
                "tracked": len(self.proxy_manager.domain_reputations),
                "high_risk": sum(1 for d in self.proxy_manager.domain_reputations.values() if d.score < 0.3)
            },
            "timestamp": datetime.utcnow().isoformat()
        })
    ...
    async def sync_xray_config(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            from src.network.residential_proxy_manager import XrayResidentialIntegration
            
            integration = XrayResidentialIntegration(
            ...
            return web.json_response({
                ...
                "proxies_used": len([p for p in self.proxy_manager.proxies if p.status == ProxyStatus.HEALTHY])
            })
