...
class UnifiedPathfinder:
    ...
    def add_edge(self, edge: NetworkEdge) -> None:
        ...
        self.edges[(edge.destination, edge.source)] = NetworkEdge(
            ...
            throughput_mbps=edge.throughput_mbps,
            reliability=edge.reliability,
        )
        self._invalidate_all_cache()

    def compute_k_disjoint_paths(
        self, source: str, destination: str, k: Optional[int] = None
    ) -> List[PathMetrics]:
        if k is None:
            k = self.k_disjoint
    ...
    def _compute_disjoint_paths(
        self, source: str, destination: str, k: int
    ) -> List[List[str]]:
    ...
    def _update_cache(
        self, cache_key: Tuple[str, str], paths: List[PathMetrics]
    ) -> None:
