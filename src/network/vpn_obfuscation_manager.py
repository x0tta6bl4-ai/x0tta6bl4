...
from .obfuscation.traffic_shaping import TrafficShaper, TrafficProfile
...
class VPNObfuscationManager:
    ...
    def rotate_parameters(self):
        ...
        if self.rotation_strategy == RotationStrategy.RANDOM:
            ...
            sni_index = (ROTATING_SNI_OPTIONS.index(self.current_sni) + 1) % len(ROTATING_SNI_OPTIONS)
            fp_index = (ROTATING_FINGERPRINT_OPTIONS.index(self.current_fingerprint) + 1) % len(ROTATING_FINGERPRINT_OPTIONS)
            spx_index = (ROTATING_SPIDERX_OPTIONS.index(self.current_spiderx) + 1) % len(ROTATING_SPIDERX_OPTIONS)
    ...
    def obfuscate(self, data: bytes) -> bytes:
        ...
        if self.current_method == ObfuscationMethod.FAKETLS:
            ...
            if self.stegomesh is None:
                logger.warning("STEGOMESH not available, returning shaped data unchanged")
                return shaped_data
    ...
    def optimize_parameters_for_dpi_evasion(self):
        ...
        if best_method:
            ...
            logger.info(f"Selected best obfuscation method: {best_method} (score: {best_score:.2f})")
