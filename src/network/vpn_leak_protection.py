...
from src.core.safe_subprocess import SafeSubprocess, ValidationError, SecurityError
...
class VPNLeakProtector:
                ...
                if result.success:
                    ...
                    for line in lines:
                        if "DNS Servers" in line:
                            in_dns_section = True
                            continue
                        if in_dns_section and line.strip() and not any(keyword in line for 
                                                                      keyword in ["Description", "Physical Address", "DHCP"]):
                ...
                if os.path.exists("/etc/systemd/resolved.conf"):
                    ...
                    if result.success and result.stdout.strip() == "active":
                        ...
                        dns_result = SafeSubprocess.run(
                            ["resolvectl", "dns", str(self.vpn_interface), "1.1.1.1", "8.8.8.8"],
                            timeout=10
                        )
                        ...
                        if not domain_result.success:
                            logger.warning(f"Failed to set domain: {domain_result.stderr}")
            elif sys.platform == "win32":
    ...
    async def _restore_original_dns(self) -> None:
        ...
        try:
            ...
            if sys.platform.startswith("linux"):
                ...
                result = SafeSubprocess.run(
                    ["resolvectl", "revert", str(self.vpn_interface)],
                    timeout=10
                )
        ...
        try:
            if sys.platform.startswith("linux"):
                ...
                if os.path.exists("/sbin/iptables"):
                    # Allow loopback
                    await self._run_iptables_command(["-A", "INPUT", "-i", "lo", "-j", "ACCEPT"])
                    await self._run_iptables_command(["-A", "OUTPUT", "-o", "lo", "-j", "ACCEPT"])
                    ...
                    for network in GOOGLE_CLOUD_NETWORKS:
                        await self._run_iptables_command(["-A", "OUTPUT", "-d", network, "-j", "ACCEPT"])
                    
                    # Allow VPN interface
                    await self._run_iptables_command(["-A", "INPUT", "-i", str(self.vpn_interface), "-j", "ACCEPT"])
                    await self._run_iptables_command(["-A", "OUTPUT", "-o", str(self.vpn_interface), "-j", "ACCEPT"])
                    
                    # Allow DNS over VPN
                    await self._run_iptables_command(["-A", "OUTPUT", "-p", "udp", "--dport", "53", "-o", str(self.vpn_interface), "-j", "ACCEPT"])
                    await self._run_iptables_command(["-A", "OUTPUT", "-p", "tcp", "--dport", "53", "-o", str(self.vpn_interface), "-j", "ACCEPT"])
                    
                    # Allow HTTPS for DoH
                    await self._run_iptables_command(["-A", "OUTPUT", "-p", "tcp", "--dport", "443", "-o", str(self.vpn_interface), "-j", "ACCEPT"])
                    
                    # Block all other outgoing traffic
                    await self._run_iptables_command(["-P", "OUTPUT", "DROP"])
                    
                    logger.info("Firewall configuration applied with Google Cloud split-tunneling")
            else:
                logger.warning(f"Firewall configuration not implemented for {sys.platform}")
    ...
    async def _restore_firewall(self) -> None:
        ...
        try:
            if sys.platform.startswith("linux"):
                ...
                logger.warning(f"Firewall restoration not implemented for {sys.platform}")
    ...
    async def _enable_kill_switch(self) -> None:
        ...
        try:
            if sys.platform.startswith("linux"):
                ...
                await self._run_iptables_command(["-A", "VPN_KILL_SWITCH", "-o", str(self.vpn_interface), "-j", "ACCEPT"])
                await self._run_iptables_command(["-A", "VPN_KILL_SWITCH", "-i", "lo", "-j", "ACCEPT"])
                
                # Block everything else
                await self._run_iptables_command(["-A", "VPN_KILL_SWITCH", "-j", "DROP"])
    ...
    async def test_dns_leak(self, domains: Optional[List[str]] = None) -> LeakTestResult:
        """
        ...
        try:
                            ...
                            if response.status == 200:
                                ...
                                if "json" in service:
                                    try:
                                        ...
                                        logger.debug(f"Failed to parse JSON from {service}: {e}")
                                else:
            ...
            if len(detected_ips) > 0:
                ...
                if expected_vpn_ip:
                    ...
                    is_leaking = False
                    details["warning"] = "VPN_SERVER not configured, cannot determine leak status"
            else:
                is_leaking = True
