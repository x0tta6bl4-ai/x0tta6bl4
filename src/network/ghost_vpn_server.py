...
def _load_real_module():
    ...
    real_path = base_dir / "services" / "nl-server" / "ghost-vpn" / "ghost_vpn_server.py"

    if real_path.exists():
        spec = importlib.util.spec_from_file_location("ghost_vpn_server_real", str(real_path))
        if spec and spec.loader:
            module = importlib.util.module_from_spec(spec)
            sys.modules["ghost_vpn_server_real"] = module
