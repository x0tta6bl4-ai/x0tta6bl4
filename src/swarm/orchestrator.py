...
from src.swarm.agent import Agent, AgentState, AgentCapability
...
class SwarmOrchestrator:
    ...
    def __init__(self, config: SwarmConfig):
        ...
        if config.anti_meave_protection and HAS_ANTI_MEAVE and AntiMeaveOracle is not None:
    ...
    async def _metrics_collector(self) -> None:
        ...
        while self._running:
            try:
                ...
                failed = sum(1 for a in self.agents.values() if a.state == AgentState.ERROR)
                ...
                if self.metrics.current_throughput > 0:
                    ...
                    self.metrics.speedup_vs_sequential = self.metrics.current_throughput / baseline
                
                # Calculate parallelism factor
                if self.metrics.total_agents > 0:
                    self.metrics.parallelism_factor = self.metrics.active_agents / self.metrics.total_agents
