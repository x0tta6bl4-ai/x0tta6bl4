...
class VisualOverlay:
    ...
    def _render_element(
        self, draw: ImageDraw.Draw, element: OverlayElement
    ) -> None:  # noqa: ANN001
        ...
        if element.content:
            try:
                font = ImageFont.truetype(
                    "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", style.font_size
                )
            except Exception:
        ...
        while open_set:
            ...
            for neighbor_id in graph.get_neighbors(current_id):
                ...
                if neighbor_id not in g_score or tentative_g < g_score[neighbor_id]:
                    ...
                    f = (
                        tentative_g
                        + heuristic(neighbor, end_node) * self.heuristic_weight
                    )
...
class VisionCache:
    ...
    async def set(self, data: bytes, operation: str, result: Any) -> None:
        ...
        async with self._lock:
            ...
            self._cache[key] = {"data": result, "timestamp": time.time(), "hits": 0}
            self._access_order.append(key)
