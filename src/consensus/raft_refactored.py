...
class RaftLogValidator:
    ...
    # CC = 2 (two checks)
    def is_log_consistent(
        self, log: List[LogEntry], prev_log_index: int, prev_log_term: int
    ) -> bool:
...
class RaftNodeRefactored:
    ...
    def _validate_append_entries(self, prev_log_index: int, prev_log_term: int) -> bool:
        ...
        if not self.log_validator.is_log_consistent(
            self.log, prev_log_index, prev_log_term
        ):
            return False
    ...
    # CC = 1 (simple update)
    def _apply_new_entries(
        self, prev_log_index: int, entries: List[LogEntry], leader_commit: int
    ):
    ...
    def receive_request_vote(
        ...
        if not self.log_validator.is_candidate_log_uptodate(
            last_log_term, last_log_index, my_last_term, my_last_index
        ):
            return False
