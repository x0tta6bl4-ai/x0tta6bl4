...
class RaftNetworkClient:
    ...
    def __init__(
        ...
        logger.info(
            f"Raft Network Client initialized for {node_id} (gRPC: {self.use_grpc})"
        )
    ...
    async def request_vote(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning(
                    f"RequestVote attempt {attempt + 1} failed for {peer_id}: {e}"
                )
    ...
    async def append_entries(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning(
                    f"AppendEntries attempt {attempt + 1} failed for {peer_id}: {e}"
                )
                if attempt < self.max_retries - 1:
    ...
    async def _grpc_request_vote(
        self, peer_address: str, request: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _grpc_append_entries(
        self, peer_address: str, request: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Send AppendEntries via gRPC"""
        logger.debug(
            f"gRPC AppendEntries to {peer_address}: {len(request.get('entries', []))} entries"
        )
    ...
    async def _http_append_entries(
        self, peer_address: str, request: Dict[str, Any]
    ) -> Dict[str, Any]:
...
class RaftNetworkServer:
    ...
    def __init__(
        self, node_id: str, listen_address: str = "0.0.0.0:50051", use_grpc: bool = True
    ):
        self.node_id = node_id
        ...
        logger.info(
            f"Raft Network Server initialized for {node_id} at {listen_address} (gRPC: {self.use_grpc})"
        )
        ...
        try:
            ...
            async def handle_http_request_vote(request):
                try:
                    ...
                    return web.json_response(
                        {"term": 0, "vote_granted": False, "reason": str(e)}, status=500
                    )
            ...
            async def handle_http_append_entries(request):
                try:
                    ...
                    return web.json_response(
                        {"term": 0, "success": False, "reason": str(e)}, status=500
                    )
...
def get_raft_network_client(node_id: str, rpc_timeout: int = 1000) -> RaftNetworkClient:
    ...
    if _raft_network_client is None:
        _raft_network_client = RaftNetworkClient(
            node_id=node_id, rpc_timeout=rpc_timeout
        )
    return _raft_network_client


def get_raft_network_server(
    node_id: str, listen_address: str = "0.0.0.0:50051"
) -> RaftNetworkServer:
    """Get or create global Raft network server"""
    global _raft_network_server
    if _raft_network_server is None:
        _raft_network_server = RaftNetworkServer(
