...
class MAPEKQualityMonitor:
    ...
    async def _process_cycle_results(self, state) -> None:
        ...
        if phase == "COMPLETE":
            ...
            logger.info(
                f"MAPE-K quality cycle completed: {len(decisions)} decisions, {len(actions)} actions"
            )
    ...
    def _calculate_health_score(self) -> float:
        ...
        health_score = (
            quality_factor * 0.5 + security_factor * 0.3 + debt_factor * 0.2
        ) * 100
    ...
    async def analyze_quality(self) -> Dict[str, Any]:
        ...
        if total_metrics:
            ...
            self.metrics.set_gauge(
                "security_issues", float(total_metrics.security_issues)
            )
            self.metrics.set_gauge(
                "technical_debt", float(total_metrics.technical_debt)
            )
    ...
    async def _suggest_security_fix(self, improvement: QualityImprovement) -> bool:
        ...
        try:
            ...
            if (
                "HARDCODED_SECRET" in improvement.description
                or "password" in content.lower()
                or "api_key" in content.lower()
            ):
                suggestions.append(
                    {
            ...
            # Detect weak crypto
            if (
                "WEAK_CRYPTO" in improvement.description
                or "md5(" in content.lower()
                or "sha1(" in content.lower()
            ):
                suggestions.append(
                    {
                        "type": "WEAK_CRYPTO",
        ...
        try:
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                try:
                    ...
                    logger.warning(
                        f"Syntax error in {file_path}, cannot generate test suggestions"
                    )
                    return False
...
'''
    ...
    async def _suggest_refactoring(self, improvement: QualityImprovement) -> bool:
        ...
        try:
            ...
            for suggestion in suggestions:
                logger.info(
                    f"  [{suggestion['severity']}] {suggestion['type']}: {suggestion['suggestion']}"
                )
                logger.info(f"    Action: {suggestion['action']}")

            # Store suggestions
            suggestion_file = (
                file_path.parent / f"{file_path.stem}_refactoring_suggestions.json"
            )
            with open(suggestion_file, "w") as f:
                json.dump(
                    {
