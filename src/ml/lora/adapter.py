...
class LoRAAdapter:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "LoRAAdapter":
        ...
        adapter = cls(
            ...
            adapter_path=(
                Path(data["adapter_path"]) if data.get("adapter_path") else None
            ),
        )
        return adapter
...
def save_lora_adapter(
    adapter: LoRAAdapter, save_path: Path, model: Optional[Any] = None
) -> bool:
    """
...
def load_lora_adapter(
    load_path: Path, base_model: Optional[Any] = None
...
def apply_lora_adapter(base_model: Any, adapter: LoRAAdapter) -> Optional[Any]:
    """
