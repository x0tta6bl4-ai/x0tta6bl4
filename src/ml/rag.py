...
try:
    ...
    def record_rag_retrieval(*args, **kwargs): pass
...
class HNSWVectorStore:
    ...
    def __init__(self, embedding_dim: int = 384, max_elements: int = 10000, 
                 ef_construction: int = 200, ef: int = 50, 
        ...
        if HNSWLIB_AVAILABLE:
            ...
            self.index.init_index(max_elements=max_elements, 
                                 ef_construction=ef_construction, M=16)
    ...
    def retrieve(self, query_embedding: np.ndarray, k: int = 5) -> List[Tuple[str, float]]:
        ...
        if self.index is not None:
            ...
            labels, distances = self.index.knn_query(query_norm.reshape(1, -1), k=min(k, len(self.documents)))
...
class RAGAnalyzer:
    ...
    def __init__(self, embedding_dim: int = 384, use_langchain: bool = True, 
                 use_hnsw: bool = True, persist_dir: Optional[str] = None):
        """
        ...
        self.text_splitter = RecursiveCharacterTextSplitter(
            chunk_size=500,
            chunk_overlap=50
        ) if self.use_langchain else None
    ...
    async def retrieve_context(
        ...
        if len(self.index_stats['retrieval_times']) > 100:
            self.index_stats['retrieval_times'] = self.index_stats['retrieval_times'][-100:]
        ...
        augmented = {
            **current_analysis,
            "rag": {
                ...
                "average_similarity": float(np.mean(context.similarities)) if context.similarities else 0.0,
                "context_insights": [
                    {
...
async def example_rag_workflow():
    ...
    # Index knowledge
    knowledge = [
        {"insight": "Network latency spike detected at 14:30", "timestamp": "2026-01-10"},
