...
from src.ml.ensemble_anomaly_detector import (EnsembleAnomalyDetector,
                                              EnsembleVotingStrategy)
from src.ml.production_anomaly_detector import (AnomalySeverity,
...
class HybridAnomalySystem:
    ...
    def record_metric(
        ...
        if self.mode in (
            ...
            prod_result = self.production_detector.record_metric(
        ...
        if self.mode in (
            ...
            self.ensemble_detector.fit(
                full_metric,
                list(self.production_detector.metric_history.get(full_metric, []))[
                    -100:
                ],
            )
        ...
        return self._combine_results(
            component, metric_name, value, prod_result, ensemble_result
        )
    ...
    def _combine_results(
        ...
        if self.mode == HybridDetectionMode.PRODUCTION_ONLY:
            ...
            if prod_anom or ensemble_anom:
                ...
                severity = (
                    prod_result.severity
                    if prod_result
                    else self._estimate_severity_from_confidence(confidence)
                )
            ...
            if prod_anom and ensemble_anom:
                ...
                severity = (
                    prod_result.severity if prod_result else AnomalySeverity.MEDIUM
                )
            else:
                return None
    ...
    def _update_agreement_ratio(self, component: str) -> None:
        ...
        component_detections = [d for d in self.detections if d.component == component]

        if not component_detections:
            return

        agreed = sum(1 for d in component_detections if d.agreement)
        self.component_agreement[component] = (
            agreed / len(component_detections) if component_detections else 0.0
        )

        all_agreed = sum(1 for d in self.detections if d.agreement)
        self.agreement_ratio = (
            all_agreed / len(self.detections) if self.detections else 0.0
        )
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            # Backward-compatible key expected by older integration tests.
            "detections_made": total_detections,
    ...
    def get_component_analysis(self, component: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "anomaly_rate": (
                len(anomalies) / len(component_detections)
                if component_detections
                else 0.0
            ),
            "agreement_ratio": self.component_agreement.get(component, 0.0),
            "avg_confidence": (
                sum(d.confidence for d in anomalies) / len(anomalies)
                if anomalies
                else 0.0
            ),
    ...
    def _get_severity_distribution(
        self, anomalies: List[HybridDetectionResult]
    ) -> Dict[str, int]:
...
def get_hybrid_anomaly_system(
    mode: HybridDetectionMode = HybridDetectionMode.HYBRID,
) -> HybridAnomalySystem:
