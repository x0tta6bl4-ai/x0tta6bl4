...
class FeatureNormalizer:
    ...
    def normalize_features(
        features: Dict[str, float], baseline: NetworkBaseline
    ) -> Dict[str, float]:
        """
        ...
        if baseline.std_loss_rate > 0:
            normalized["loss_rate_z"] = (
                loss_rate - baseline.mean_loss_rate
            ) / baseline.std_loss_rate
        else:
        ...
        if baseline.std_latency > 0:
            normalized["latency_z"] = (
                latency - baseline.mean_latency
...
class GraphSAGEAnomalyDetectorV3:
    ...
    def predict_enhanced(
        ...
        normalized_features = self.normalizer.normalize_features(
            ...
            (
                ...
                if self.adaptive_threshold
                else NetworkBaseline()
            ),
        )

        # Detect anomalies
        anomaly_score = self._compute_anomaly_score(
            node_id, normalized_features, neighbors, network_nodes_count
        )
        ...
        explanation = self._generate_explanation(
            node_id, normalized_features, is_anomaly, anomaly_score
        )

        # Get recommendations
        recommendations = self._get_recommendations(
            node_id, node_features, is_anomaly, anomaly_score
        )
    ...
    def _compute_anomaly_score(
        ...
        if neighbors:
            neighbor_anomaly = self._compute_neighbor_anomaly(
                normalized_features, neighbors
            )
        ...
        if network_nodes_count:
            isolation_score = max(
                0.0, 1.0 - (len(neighbors) / max(network_nodes_count - 1, 1))
            )
    ...
    def _calibrate_confidence(
        self, anomaly_score: float, base_confidence: float
    ) -> float:
        """
        ...
        # Recent alert rate
        recent_alerts = sum(
            1 for p in list(self.prediction_history)[-50:] if p.get("is_anomaly", False)
        )
        history_len = min(50, len(self.prediction_history))
        recent_alert_rate = recent_alerts / history_len if history_len > 0 else 0.0
