...
class IsolationForestDetector:
    ...
    def _build_tree(self, data: np.ndarray, depth: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "threshold": threshold,
            "left": (
                self._build_tree(left_data, depth + 1) if len(left_data) > 0 else None
            ),
            "right": (
                self._build_tree(right_data, depth + 1) if len(right_data) > 0 else None
            ),
        }
    ...
    def _get_tree_depth(self, tree: Dict[str, Any], current_depth: int) -> int:
        ...
        left_depth = (
            self._get_tree_depth(tree["left"], current_depth + 1)
            if tree["left"]
            else current_depth
        )
        right_depth = (
            self._get_tree_depth(tree["right"], current_depth + 1)
            if tree["right"]
            else current_depth
        )
    ...
    def _path_length(
        self, tree: Dict[str, Any], value: float, current_depth: int
    ) -> float:
...
class LocalOutlierFactorDetector:
    ...
    def _compute_lof(self, point: float) -> float:
        ...
        local_reachability = len(reachability_dists) / max(
            sum(reachability_dists), 1e-10
        )

        lof_scores_neighbors = [
            self.lof_scores.get(self.training_data[i], 1.0)
            for i in range(min(self.k_neighbors, len(self.training_data)))
        ]

        mean_lof_neighbors = (
            np.mean(lof_scores_neighbors) if lof_scores_neighbors else 1.0
        )
    ...
    def predict(self, value: float) -> Tuple[bool, float]:
        ...
        if threshold > 0:
            raw_confidence = (lof_score / threshold - 1.0) / 2.0
...
class MovingAverageDetector:
    ...
    def fit(self, data: List[float]) -> None:
        """Train MA detector"""
        self.values = deque(
            list(data)[-self.window_size * 2 :], maxlen=self.window_size * 2
        )
...
class EnsembleAnomalyDetector:
    ...
    def __init__(
        self, voting_strategy: EnsembleVotingStrategy = EnsembleVotingStrategy.WEIGHTED
    ):
    ...
    def fit(self, metric_name: str, data: List[float]) -> None:
        ...
        if len(data) < 20:
            logger.warning(
                f"Insufficient data for ensemble training: {len(data)} samples"
            )
            return
    ...
    def predict(self, metric_name: str, value: float) -> EnsembleDetectionResult:
        ...
        is_anomaly, consensus_level, reason = self._aggregate_votes(
            algorithm_votes, algorithm_scores
        )
    ...
    # Backward-compatible alias returning legacy tuple format.
    def detect(self, metric_name: str, value: float) -> Tuple[bool, float]:
        result = self.predict(metric_name, value)
        return result.is_anomaly, result.confidence

    def _aggregate_votes(
        self, votes: Dict[str, bool], scores: Dict[str, float]
        ...
        if self.voting_strategy == EnsembleVotingStrategy.MAJORITY:
            ...
            weighted_score = sum(
                scores[k] * self.algorithm_weights[k] for k in scores.keys()
            )
            ...
            consensus = (
                1.0
                if all_anomaly
                else (sum(1 for v in votes.values() if v) / len(votes))
            )
            reason = (
                "Consensus: all detectors must agree" if is_anomaly else "No consensus"
            )
    ...
    def _calculate_confidence(
        self, scores: Dict[str, float], is_anomaly: bool
    ) -> float:
...
def get_ensemble_detector(
    strategy: Optional[EnsembleVotingStrategy] = None,
) -> EnsembleAnomalyDetector:
    """Get or create singleton ensemble detector"""
    global _ensemble
    if strategy is not None:
        return EnsembleAnomalyDetector(strategy)
