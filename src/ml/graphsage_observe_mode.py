...
class GraphSAGEObserveMode:
    ...
    def detect(
        self, graph_data: Dict[str, Any], node_id: str
    ...
    def _send_alert_to_prometheus(self, event: AnomalyEvent):
        ...
        ANOMALY_DETECTED.labels(
            node_id=event.node_id, confidence_level=confidence_level
        ).inc()
    ...
    def _block_action(self, event: AnomalyEvent):
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Node {event.node_id} isolated from mesh network (via topology)"
                )
            except ImportError:
    ...
    def _save_event_for_analysis(self, event: AnomalyEvent):
        ...
        try:
            ...
            events_dir = Path("/tmp/x0tta6bl4/anomaly_events")  # nosec B108
            ...
            logger.warning(
                f"Failed to save event to file: {e}, continuing with in-memory storage only"
            )
        ...
        if self.mode in [DetectorMode.OBSERVE, DetectorMode.WARN]:
            ...
            if stats.get("true_positives", 0) > 0:
                ...
                if precision >= 0.95:  # Требуем 95% precision для block mode
                    ...
                    logger.warning(
                        f"Precision {precision:.2f} < 0.95, cannot migrate to BLOCK mode"
                    )
            else:
