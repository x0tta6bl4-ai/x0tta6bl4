...
class CausalAnalysisEngine:
    ...
    def __init__(
        self, correlation_window_seconds: float = 300.0, min_confidence: float = 0.5
    ):
        """
    ...
    def _correlate_incidents(self, new_incident: IncidentEvent):
        ...
        for existing_id, existing_incident in self.incidents.items():
            ...
            time_diff = abs(
                (new_incident.timestamp - existing_incident.timestamp).total_seconds()
            )
            ...
            service_correlation = self._check_service_dependency(
                existing_incident.service_id, new_incident.service_id
            )

            # Metric correlation
            metric_correlation = self._check_metric_correlation(
                existing_incident.metrics, new_incident.metrics
            )
    ...
    def _check_service_dependency(
        self, source_service: Optional[str], target_service: Optional[str]
    ) -> float:
        """
        ...
        while queue:
            ...
            if current_id in self.causal_graph:
                incident_graph.add_node(
                    current_id, **self.causal_graph.nodes[current_id]
                )
            ...
            for predecessor in self.causal_graph.predecessors(current_id):
                if predecessor not in visited:
                    ...
                    incident_graph.add_edge(predecessor, current_id, **edge_data)
                    queue.append(predecessor)

        return incident_graph

    def _identify_root_causes(
        self, incident_id: str, incident_graph: nx.DiGraph
    ) -> List[RootCause]:
        """
        ...
        root_nodes = [
            node
            for node in incident_graph.nodes()
        ...
        if not root_nodes:
            min_in_degree = min(
                incident_graph.in_degree(node) for node in incident_graph.nodes()
            )
            root_nodes = [
                node
                for node in incident_graph.nodes()
        ...
        for root_node_id in root_nodes:
            ...
            path_confidence = self._calculate_path_confidence(
                root_node_id, incident_id, incident_graph
            )
            ...
            explanation = self._generate_explanation(
                root_incident, incident_id, incident_graph
            )

            # Get remediation suggestions
            remediation = self._get_remediation_suggestions(
                root_cause_type, root_incident
            )
    ...
    def _calculate_path_confidence(
        self, source_id: str, target_id: str, graph: nx.DiGraph
    ) -> float:
    ...
    def _generate_explanation(
        self, root_incident: IncidentEvent, target_incident_id: str, graph: nx.DiGraph
    ) -> str:
    ...
    def _get_contributing_factors(
        self, root_node_id: str, graph: nx.DiGraph
    ) -> List[str]:
    ...
    def _get_remediation_suggestions(
        self, root_cause_type: str, incident: IncidentEvent
    ) -> List[str]:
        ...
        if "CPU" in root_cause_type:
            ...
            suggestions.append(
                "Investigate service logs for detailed error information"
            )

        return suggestions

    def _build_event_chain(
        self, incident_id: str, root_causes: List[RootCause]
    ) -> List[str]:
        ...
        try:
            ...
            path = nx.shortest_path(self.causal_graph, root_cause.event_id, incident_id)
            return path
        except (nx.NetworkXNoPath, KeyError):
    ...
    def _calculate_overall_confidence(self, root_causes: List[RootCause]) -> float:
        ...
        confidence = sum(
            cause.confidence * weight for cause, weight in zip(top_causes, weights)
        )
