...
class EnsembleAnomalyDetector:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Ensemble Anomaly Detector initialized with {len(self.models)} models"
        )

    def train(
        self, features: List[Dict[str, float]], labels: Optional[List[int]] = None
    ):
        """
    ...
    def _generate_explanation(
        self, model_scores: Dict[str, float], consensus: float
    ) -> str:
        ...
        for model_name, score in model_scores.items():
            if score > 0.7:
                explanations.append(
                    f"{model_name} indicates high anomaly risk ({score:.2%})"
                )
            elif score < 0.3:
                explanations.append(
                    f"{model_name} indicates normal behavior ({score:.2%})"
                )
