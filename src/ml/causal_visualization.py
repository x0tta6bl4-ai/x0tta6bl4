...
from .causal_analysis import (CausalAnalysisEngine, CausalAnalysisResult,
...
class CausalAnalysisVisualizer:
    ...
    def generate_dashboard_data(self, incident_id: str) -> DashboardData:
        ...
        if incident_id not in [r.incident_id for r in self.engine.analysis_history]:
            ...
            result = next(
                r for r in self.engine.analysis_history if r.incident_id == incident_id
            )
    ...
    def _generate_timeline(self, result: CausalAnalysisResult) -> List[TimelineEvent]:
        ...
        event_ids = (
            result.event_chain
            if result.event_chain
            else list(result.causal_graph.nodes())
        )
        ...
        for event_id in event_ids:
            ...
            if event_id == result.incident_id:
                ...
                root_cause = next(
                    rc for rc in result.root_causes if rc.event_id == event_id
                )
                ...
                description = (
                    f"Correlated with main incident on node {incident.node_id}"
                )
            ...
            timeline.append(
                TimelineEvent(
                    ...
                    confidence=(
                        incident.anomaly_score if event_type == "anomaly" else None
                    ),
                )
            )
    ...
    def _generate_dependency_graph(
        self, result: CausalAnalysisResult
    ) -> Dict[str, Any]:
    ...
    def _generate_metrics_panel(self, result: CausalAnalysisResult) -> Dict[str, Any]:
        ...
        if len(result.event_chain) >= 2:
            ...
            for key in set(first_event.metrics.keys()) & set(last_event.metrics.keys()):
                before_after[key] = {
                    ...
                    "improvement": (
                        (
                            (first_event.metrics[key] - last_event.metrics[key])
                            / first_event.metrics[key]
                            * 100
                        )
                        if first_event.metrics[key] > 0
                        else 0
                    ),
                }
        ...
        return {
            ...
            "summary": {
                ...
                "affected_nodes": len(
                    set(
                        self.engine.incidents[eid].node_id
                        for eid in result.event_chain
                        if eid in self.engine.incidents
                    )
                ),
                "affected_services": len(
                    set(
                        self.engine.incidents[eid].service_id
                        for eid in result.event_chain
                        if eid in self.engine.incidents
                        and self.engine.incidents[eid].service_id
                    )
                ),
            },
        }

    def _generate_remediation_panel(
        self, result: CausalAnalysisResult
    ) -> Dict[str, Any]:
        ...
        estimated_time = (
            "5-10 minutes" if len(result.root_causes) == 1 else "10-20 minutes"
        )
            ...
            "dashboard": {
                ...
                "panels": [
                    ...
                    },
                    {
                        ...
                        "type": "nodeGraph",
                        "targets": [{"expr": "causal_analysis_dependency_graph"}],
                    },
                    {
                        "id": 3,
                        "title": "Root Causes",
                        "type": "table",
                        "targets": [{"expr": "causal_analysis_root_causes"}],
                    },
                ],
            }
        }
