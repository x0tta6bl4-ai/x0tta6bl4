...
class IntegratedAnomalyAnalyzer:
    ...
    def process_node_anomaly(
        ...
        if detection_result["is_anomaly"]:
            ...
            if not is_new:
                logger.debug(
                    f"Incident {incident_id} is duplicate of {incident_or_dup}"
                )
    ...
    def _create_integrated_result(
        ...
        if causal_result.primary_root_cause:
            ...
            for rec in root_cause.remediation_suggestions:
                if any(
                    x in rec.lower()
                    for x in ["restart", "restart immediately", "emergency"]
                ):
                    immediate_actions.append(rec)
                elif any(
                    x in rec.lower()
                    for x in ["check", "verify", "investigate", "review"]
                ):
        ...
        severity = (
            "critical"
            if detection_result["anomaly_score"] > 0.8
            else (
                "high"
                if detection_result["anomaly_score"] > 0.6
                else "medium" if detection_result["anomaly_score"] > 0.4 else "low"
            )
        )
        ...
        for rc in causal_result.root_causes[:3]:
            root_causes_list.append(
                {
                    "type": (
                        rc.root_cause_type.value
                        if hasattr(rc.root_cause_type, "value")
                        else str(rc.root_cause_type)
                    ),
        ...
        if causal_result.primary_root_cause:
            primary_rc = {
                "type": (
                    causal_result.primary_root_cause.root_cause_type.value
                    if hasattr(
                        causal_result.primary_root_cause.root_cause_type, "value"
                    )
                    else str(causal_result.primary_root_cause.root_cause_type)
                ),
    ...
    def get_integrated_report(self) -> Dict[str, Any]:
        ...
        if not self.completed_analyses:
            return {"status": "no_analyses", "message": "No analyses completed yet"}

        # Statistics
        total_incidents = len(self.completed_analyses)
        critical = sum(
            1 for r in self.completed_analyses.values() if r.severity == "critical"
        )
        high = sum(1 for r in self.completed_analyses.values() if r.severity == "high")

        # Average latencies
        avg_detection_latency = (
            sum(r.total_latency_ms for r in self.completed_analyses.values())
            / total_incidents
            if total_incidents
            else 0
        )
        ...
        causal_confidences = [
            r.causal_confidence
            for r in self.completed_analyses.values()
        ...
        return {
            ...
            "average_causal_confidence": (
                sum(causal_confidences) / len(causal_confidences)
            ...
            "recent_incidents": [
                {
                    ...
                    "severity": r.severity,
                    "root_cause": (
                        r.primary_root_cause["explanation"]
...
def create_integrated_analyzer_for_mapek(
    ...
    """
    if detector is None:
        from graphsage_anomaly_detector_v3_enhanced import \
            create_graphsage_v3_for_mapek
