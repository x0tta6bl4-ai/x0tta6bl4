...
def record_graphsage_inference(latency_ms: float, is_anomaly: bool, severity: Optional[str] = None):
...
def record_lora_training_update(adapter_name: str, loss: float, epoch: int):
    ...
    try:
        lora_training_loss.labels(adapter_name=adapter_name, model_name='mape_k').set(loss)
...
def record_ebpf_compilation(latency_ms: float, bytecode_size_kb: float, program_type: str):
