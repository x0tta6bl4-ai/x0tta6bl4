...
class MLMetricsCollector:
    ...
    def record_graphsage_inference(
        self, duration: float, prediction_type: str = "normal"
    ) -> None:
        """Запись GraphSAGE инференса"""
        self.metrics.graphsage_inference_duration.observe(duration)
        self.metrics.graphsage_predictions_total.labels(
    ...
    def record_rag_retrieval(self, duration: float, query_type: str, hit: bool) -> None:
        ...
        self.metrics.rag_retrieval_results.labels(
            query_type=query_type, hit="hit" if hit else "miss"
        ).inc()
...
class MeshMetricsCollector:
    ...
    def record_connection(self, connection_type: str = "direct") -> None:
        ...
        self.metrics.mesh_connections_total.labels(
    ...
    def record_bandwidth(
        self, bytes_transferred: int, direction: str = "outbound"
    ) -> None:
        """Запись передачи данных"""
        self.metrics.mesh_bandwidth_bytes.labels(direction=direction).inc(
            bytes_transferred
        )
    ...
    def record_latency(
        ...
        """Запись латентности"""
        self.metrics.mesh_latency.labels(node_id=node_id, peer_id=peer_id).observe(
            latency_seconds
        )
...
class FederatedLearningMetricsCollector:
    ...
    def record_fl_communication(self, bytes_transferred: int, direction: str) -> None:
        """Запись коммуникации"""
        self.metrics.fl_communication_bytes.labels(direction=direction).inc(
            bytes_transferred
        )
