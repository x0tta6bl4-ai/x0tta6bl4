...
pqc_handshake_success_total = Counter(
    "pqc_handshake_success_total", "Total successful PQC handshakes"
)
...
pqc_fallback_enabled = Gauge(
    "pqc_fallback_enabled", "PQC fallback enabled (1=yes, 0=no)"
)

key_rotation_success_total = Counter(
    "pqc_key_rotation_success_total", "Total successful PQC key rotations"
)

key_rotation_failure_total = Counter(
    "pqc_key_rotation_failure_total", "Total failed PQC key rotations", ["reason"]
)
...
def record_handshake_failure(reason: str):
    ...
    try:
        ...
        send_alert_sync(
            ...
            annotations={
                "impact": "Security compromised",
                "action": "Review PQC configuration",
            },
