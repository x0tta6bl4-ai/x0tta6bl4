    ...
    def _deploy_to_cluster(self, image_url: str) -> bool:
        ...
        try:
            ...
            helm_cmd = [
                ...
                "--create-namespace",
                "--set", f"image.repository={image_url.split(':')[0]}",
                "--set", f"image.tag={image_url.split(':')[1] if ':' in image_url else 'latest'}",
                "--wait",
                "--timeout", f"{self.config.wait_timeout}s"
            ]
            ...
            logger.info(f"✅ Helm deployment successful: {self.config.helm_release_name}")
            return True
