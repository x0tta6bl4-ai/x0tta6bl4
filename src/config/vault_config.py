...
class VaultClientConfig:
    ...
    def from_env(cls) -> 'VaultClientConfig':
        ...
        return cls(
            ...
            retry_delay=float(
                os.getenv('VAULT_RETRY_DELAY', cls.retry_delay)
            ),
            retry_backoff=float(
                os.getenv('VAULT_RETRY_BACKOFF', cls.retry_backoff)
            ),
            cache_ttl=int(os.getenv('VAULT_CACHE_TTL', cls.cache_ttl)),
            token_refresh_threshold=float(
                os.getenv(
                    'VAULT_TOKEN_REFRESH_THRESHOLD',
                    cls.token_refresh_threshold
                )
            ),
        )
...
class VaultMonitorConfig:
    ...
    def from_env(cls) -> 'VaultMonitorConfig':
        ...
        return cls(
            check_interval=int(
                os.getenv('VAULT_MONITOR_INTERVAL', cls.check_interval)
            ),
            token_warning_threshold=int(
                os.getenv(
                    'VAULT_TOKEN_WARNING_THRESHOLD',
                    cls.token_warning_threshold
                )
            ),
            enabled=os.getenv(
