...
class SlotSynchronizer:
    ...
    def detect_collision(self) -> bool:
        ...
        # Exponential backoff
        backoff = self.config.collision_backoff_ms * (2 ** min(self._collision_count, 5))
...
class MeshSlotNetwork:
    ...
    def get_network_status(self) -> Dict:
        ...
        return {
            "nodes": {
                nid: node.get_sync_status()
                for nid, node in self.nodes.items()
            },
            "total_nodes": len(self.nodes),
            "avg_mttd": sum(n.calculate_mttd() for n in self.nodes.values()) / len(self.nodes)
        }
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        for nid, node_status in status["nodes"].items():
            print(f"   {nid}: offset={node_status['local_offset_ms']:.2f}ms, "
