    ...
    async def index_ledger(self, force: bool = False) -> bool:
        ...
        try:
            ...
            for i, section in enumerate(sections, 1):
                logger.info(f"  [{i}/{len(sections)}] Индексирование: {section['title']}")
        ...
        try:
            ...
            for i, chunk in enumerate(rag_result.retrieved_chunks):
                ...
                results.append({
                    ...
                    "section": chunk.metadata.get("title", "Unknown") if hasattr(chunk, 'metadata') and chunk.metadata else "Unknown"
                })
