...
class LedgerDriftDetector:
    ...
    def build_ledger_graph(self) -> Dict[str, Any]:
        ...
        for i, section in enumerate(sections):
            ...
            for dep in section["dependencies"]:
                dep_idx = next((j for j, s in enumerate(sections) if s["title"] == dep), None)
    ...
    async def detect_code_drift(self) -> List[DriftResult]:
        ...
        try:
            ...
            critical_functions = ["detect_drift", "build_ledger_graph", "get_drift_detector"]
            for func in critical_functions:
                if func in code_info["functions"] and func not in doc_components["functions_mentioned"]:
                    drifts.append(DriftResult(
                        ...
                        metadata={"function": func, "file_count": len(code_info["files"])}
                    ))
        ...
        try:
            ...
            for pattern, description in deprecated_patterns:
                ...
                for match in matches:
                    ...
                    if "Known issues" not in context and "deprecated" not in context.lower():
                        drifts.append(DriftResult(
                            drift_type="doc_drift",
                            severity="low",
    ...
    async def detect_drift(self) -> Dict[str, Any]:
                ...
                if len(graph["nodes"]) > 0:
                    ...
                    if hasattr(self.anomaly_detector, 'is_trained') and self.anomaly_detector.is_trained:
                        ...
                        for node in graph["nodes"]:
                            ...
                            node_features = {
                                "content_length": float(node.get("content_length", 0)) / 1000.0,  # Нормализация
                                "title_length": float(len(node.get("title", ""))) / 100.0,
                                "in_degree": float(len([e for e in graph["edges"] if e["target"] == node["id"]])) / 10.0,
                                "out_degree": float(len([e for e in graph["edges"] if e["source"] == node["id"]])) / 10.0,
                                "last_update_age": 0.5,  # Placeholder - можно добавить реальную дату
                                "drift_count": float(len([d for d in all_drifts if d.section == node.get("title", "")])) / 10.0,
                            ...
                            for edge in graph["edges"]:
                                if edge["source"] == node["id"]:
                                    neighbor_node = next((n for n in graph["nodes"] if n["id"] == edge["target"]), None)
                                    if neighbor_node:
                                        neighbor_features = {
                                            "content_length": float(neighbor_node.get("content_length", 0)) / 1000.0,
                                            "title_length": float(len(neighbor_node.get("title", ""))) / 100.0,
                                            "in_degree": float(len([e for e in graph["edges"] if e["target"] == neighbor_node["id"]])) / 10.0,
                                            "out_degree": float(len([e for e in graph["edges"] if e["source"] == neighbor_node["id"]])) / 10.0,
                                            "last_update_age": 0.5,
                                            "drift_count": float(len([d for d in all_drifts if d.section == neighbor_node.get("title", "")])) / 10.0,
                                            "complexity": 0.3,
                                            "importance": 0.5
                                        }
                                        neighbors.append((str(neighbor_node["id"]), neighbor_features))
                            ...
                            try:
                                prediction = self.anomaly_detector.predict(
                                    ...
                                    neighbors=neighbors[:5]  # Ограничиваем количество neighbors для производительности
                                )
                                ...
                                logger.debug(f"⚠️ GraphSAGE prediction failed for node {node['id']}: {e}")
                                # Fallback: если ML-модель не работает, используем простую статистику
                                continue
                        
                        if anomalies:
                            logger.info(f"📊 GraphSAGE (ML): обнаружено {len(anomalies)} аномалий в графе")
                        else:
                            logger.debug("📊 GraphSAGE (ML): аномалий не обнаружено")
                    else:
                        # Fallback: модель не обучена, используем простую статистику
                        logger.debug("⚠️ GraphSAGE модель не обучена, используем fallback статистику")
                        if NUMPY_AVAILABLE and len(graph["nodes"]) > 1:
                            node_features = []
                            for node in graph["nodes"]:
                                features = [
                                    float(node.get("content_length", 0)),
                                    float(len([e for e in graph["edges"] if e["target"] == node["id"]]))
                                ]
                            ...
                            if node_features:
                                ...
                                if anomalies:
                                    logger.info(f"📊 GraphSAGE (fallback): обнаружено {len(anomalies)} аномалий в графе")
            except Exception as e:
                ...
                if len(all_drifts) > 0:
                    ...
                    for drift in all_drifts:
                        ...
                        incident = IncidentEvent(
                            ...
                            timestamp=datetime.fromisoformat(drift.detected_at.replace('Z', '+00:00')) if 'Z' in drift.detected_at else datetime.fromisoformat(drift.detected_at),
                            node_id="ledger",
                            service_id=None,
                            ...
                            metrics={
                                ...
                                "severity_score": {"low": 0.3, "medium": 0.5, "high": 0.7, "critical": 0.9}.get(drift.severity, 0.5)
                            },
                            detected_by="drift_detector",
                            anomaly_score=0.8 if drift.severity == "critical" else (0.6 if drift.severity == "high" else 0.4)
                        )
                    ...
                    if incident_events:
                        ...
                        critical_incident = max(incident_events, key=lambda x: x.anomaly_score)
                        ...
                        try:
                            ...
                            if root_causes:
                                ...
                                logger.debug("📊 Causal Analysis (ML): root causes не обнаружены")
                        except Exception as e:
                            logger.warning(f"⚠️ Causal Analysis ML failed: {e}, используем fallback")
        ...
        return {
            ...
            "ml_integration": {
                "graphsage_used": len(anomalies) > 0 and any(a.get("method") != "fallback_statistics" for a in anomalies),
                "causal_analysis_used": len(root_causes) > 0 and any(rc.get("method") == "ml_causal_analysis" for rc in root_causes)
            },
