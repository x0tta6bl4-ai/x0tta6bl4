...
async def start_vpn(is_server: bool):
    ...
    tun_name = b'x0t-srv0' if is_server else b'x0t-clnt0'
    ...
    try:
        ...
        mask = "24" if is_server else "32"
        commands = [
            ["ip", "addr", "add", f"{addr}/{mask}", "dev", tun_interface],
            ["ip", "link", "set", "dev", tun_interface, "up"],
        ]

        if not is_server:
            # Add point-to-point route to the server's tunnel IP
            commands.append(["ip", "route", "add", "10.10.0.1", "dev", tun_interface])
