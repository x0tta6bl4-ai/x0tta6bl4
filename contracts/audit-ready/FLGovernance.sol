...
contract FLGovernance is ReentrancyGuard {
    ...
    // Snapshot голосов на момент начала голосования
    struct VotingSnapshot {
        uint256 snapshotBlock;
        uint256 tokenBalance;
        uint256 votes;
    }
    mapping(uint256 => mapping(address => VotingSnapshot)) public snapshots;
    ...
    function propose(
        ...
        snapshots[proposalId][msg.sender] = VotingSnapshot({
            snapshotBlock: block.number,
            tokenBalance: proposerBalance > 0 ? proposerBalance : 0,
            votes: 0
    ...
    function vote(uint256 proposalId, uint8 support) external nonReentrant {
        ...
        // Snapshot voting power at proposal start if not already recorded
        VotingSnapshot storage voterSnapshot = snapshots[proposalId][msg.sender];
        if (voterSnapshot.tokenBalance == 0) {
            voterSnapshot.snapshotBlock = block.number;
            voterSnapshot.tokenBalance = tokenBalance;
            voterSnapshot.votes = 0;
        }
        
        // КВАДРАТИЧНОЕ ГОЛОСОВАНИЕ: √(токены) from snapshot
        uint256 votes = sqrt(voterSnapshot.tokenBalance);
