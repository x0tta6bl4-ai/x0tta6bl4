...
class ModelBlockchain:
        ...
        for block in self.chain:
            if block.block_type == BlockType.MODEL_UPDATE:
                history.append({
                    ...
                    "contributors": len(block.metadata.contributors) if block.metadata else 0,
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        rollbacks = sum(
            1 for b in self.chain if b.block_type == BlockType.ROLLBACK
        )
        checkpoints = sum(
            1 for b in self.chain if b.block_type == BlockType.CHECKPOINT
        )
        ...
        return {
            ...
            "latest_version": max(self._version_to_block.keys()) if self._version_to_block else 0
        }


def create_genesis_blockchain(
    initial_weights: List[float],
    chain_id: str = "x0tta6bl4-fl"
) -> ModelBlockchain:
    """
