...
class ResourceUtilization:
    ...
    def total_utilization(self) -> float:
        ...
        total = (self.cpu_percent * weights['cpu'] +
...
class AdaptiveScaler:
    ...
    def should_scale_up(self) -> bool:
        ...
        if (datetime.now() - self.last_scale_time).total_seconds() < self.cooldown_seconds:
            return False
    ...
    def get_scaling_factor(self) -> float:
        ...
        scale_up_factor, scale_down_factor = factors.get(
            self.policy,
            (1.25, 0.9)
        )
...
class PerformancePredictor:
    ...
    def predict_convergence_rounds(self, target_accuracy: float) -> int:
        ...
        rounds_needed = max(0, int((target_accuracy - current_accuracy) / avg_improvement))
    ...
    def identify_bottlenecks(self) -> Dict[str, float]:
        ...
        bottlenecks = {
            ...
            'computation': statistics.mean([m.computation_time_ms for m in recent]) / 
                          statistics.mean([m.total_time_ms for m in recent]),
            'stragglers': statistics.mean([m.stragglers_count for m in recent]) / 
                         statistics.mean([m.nodes_participated for m in recent]) if recent[0].nodes_participated > 0 else 0
        }
    ...
    def recommend_optimizations(self) -> List[str]:
        ...
        if bottlenecks.get('aggregation', 0) > 0.3:
            recommendations.append("Consider distributed aggregation to reduce aggregation bottleneck")
        
        if bottlenecks.get('network', 0) > 0.3:
            recommendations.append("Implement model compression for faster network transfers")
        
        if bottlenecks.get('stragglers', 0) > 0.1:
            recommendations.append("Use straggler mitigation techniques (e.g., deadline-based selection)")
