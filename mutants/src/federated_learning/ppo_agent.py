...
class PPOAgent:
    ...
    def get_action(self, state: List[float], deterministic: bool = False) -> Tuple[int, float, float]:
        """
...
def train_episode(
    ...
    for _ in range(max_steps):
        ...
        agent.store_transition(
            state_vec, action, result.reward,
            log_prob, value, result.done
        )
...
def train_ppo(
    ...
    for ep in range(num_episodes):
        ...
        if (ep + 1) % update_interval == 0:
            ...
            logger.info(f"Episode {ep + 1}: reward={episode_metrics['total_reward']:.2f}")
