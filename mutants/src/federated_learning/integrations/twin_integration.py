...
from ..coordinator import FederatedCoordinator, CoordinatorConfig, NodeStatus
from ..protocol import ModelUpdate, ModelWeights, GlobalModel
from ..privacy import DifferentialPrivacy, DPConfig
from ..blockchain import ModelBlockchain, ModelMetadata, ConsensusProof
...
class TwinBackedRoutingEnv(MeshRoutingEnv):
    ...
    def __init__(
        ...
        logger.info(
            f"TwinBackedRoutingEnv initialized with {len(twin.nodes)} nodes"
        )
    ...
    def reset(
        ...
        active_nodes = [
            nid for nid, node in self.twin.nodes.items()
    ...
    def get_routing_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_hops": sum(r.get("total_hops", 0) for r in successful) / max(1, len(successful)),
            "avg_reward": sum(r.get("total_reward", 0) for r in self.routing_history) / len(self.routing_history)
        }
...
class TwinMetricsCollector:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "privacy": {
                "total_epsilon": sum(m.get("epsilon_spent", 0) for m in self.privacy_metrics),
...
class FederatedTrainingOrchestrator:
        ...
        for node_id, agent in self.agents.items():
            ...
            for _ in range(self.config.episodes_per_round):
                episode_metrics = train_episode(
                    agent, env,
                    max_steps=self.config.max_hops * 2
                )
    ...
    def _aggregation_phase(
        ...
        for node_id, update in updates.items():
            model_updates.append(ModelUpdate(
                ...
                weights=ModelWeights(
                    layer_weights={"flat": update["weights"]}
                ),
                num_samples=update["samples"]
            ))
        ...
        if result.success and result.global_model:
            ...
            logger.warning(f"Aggregation failed: {getattr(result, 'message', 'unknown')}")
            return None
    ...
    def get_training_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "blockchain_stats": self.blockchain.get_stats() if self.blockchain else None,
            "privacy_budget": self.dp.get_stats() if self.dp else None
        }
    
    def inject_chaos(
        self,
        scenario: str = "node_failure",
        ...
        if scenario == "node_failure":
            if target is None:
                healthy = [
                    nid for nid, n in self.twin.nodes.items()
