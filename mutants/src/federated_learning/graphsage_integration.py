...
from .coordinator import FederatedCoordinator, CoordinatorConfig
from .secure_aggregators import GraphSAGEAggregator, SecureFedAvgAggregator
from .model_sync import ModelSynchronizer
from .protocol import ModelUpdate, GlobalModel, ModelWeights
...
class GraphSAGEFLCoordinator:
    ...
    def __init__(
        ...
        if self.config.aggregation_method == "graphsage":
            ...
            self.aggregator = SecureFedAvgAggregator(enable_dp=self.config.enable_privacy)
        else:
...
class GraphSAGEDistributedTrainer:
    ...
    def train(
        ...
        Returns:
            Training results
        """
        logger.info(f"Starting distributed training: {self.num_rounds} rounds, {len(participating_nodes)} nodes")
        ...
        for round_num in range(1, self.num_rounds + 1):
            ...
            if global_model:
                # Distribute
                self.coordinator.distribute_global_model(global_model, participating_nodes)
