    ...
    def compose_linear(
        ...
        try:
            ...
            composed_model = PeftModel.from_pretrained(
                self.base_model,
                str(self.adapters[adapter_ids[0]].adapter_path)
            )
            ...
            for adapter_id, weight in zip(adapter_ids[1:], weights[1:]):
                ...
                for key in state_dict:
                    if key in other_state:
                        state_dict[key] = state_dict[key] * weights[0] + other_state[key] * weight
    ...
    def merge_adapters(self, adapter_ids: List[str]) -> Optional[Any]:
        ...
        try:
            ...
            for adapter_id in adapter_ids:
                ...
                adapter_path = self.adapters[adapter_id].adapter_path
                merged_model = PeftModel.from_pretrained(merged_model, str(adapter_path))
        ...
        try:
            if quantization_type == "int8":
                if not BITSANDBYTES_AVAILABLE:
                    logger.warning("⚠️ bitsandbytes not available, skipping int8 quantization")
...
class LoRAIncrementalTrainer:
    ...
    def __init__(self, base_model: Any, checkpoint_dir: Optional[str] = None):
        ...
        try:
            ...
            import tempfile
            self.checkpoint_dir = Path(tempfile.gettempdir()) / "x0tta6bl4_lora_checkpoints"
            self.checkpoint_dir.mkdir(parents=True, exist_ok=True)
