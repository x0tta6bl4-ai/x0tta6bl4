...
class PolicyRanker:
    ...
    def record_outcome(self, outcome: PolicyOutcome) -> None:
        ...
        if outcome.policy_id in self.policies:
            ...
            policy.success_rate = (alpha * new_success_value + 
...
class DecisionEngine:
    ...
    async def decide_on_action(
        ...
        # Filter policies
        valid_policies = [
            p for p in self.ranker.policies.values()
            if p.id in available_policies
        ]
        ...
        decision = {
            ...
            "ranked_candidates": [
                {"id": p.id, "name": p.name, "score": float(s)}
                for p, s in ranked[:3]
            ],
    ...
    def _generate_reasoning(
        ...
        if policy.priority == PolicyPriority.HIGH or policy.priority == PolicyPriority.CRITICAL:
    ...
    async def continuous_learning(self, window_size: int = 100) -> Dict[str, Any]:
        ...
        for policy_id, perf in policy_performance.items():
            ...
            if success_rate > 0.8:
                insights.append(f"Policy {policy_id} is highly reliable ({success_rate:.1%})")
    ...
    def get_decision_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "unique_policies": len(set(d["selected_policy"] for d in self.decision_log)),
            "timestamp": datetime.now().isoformat()
        }
...
async def example_decision_workflow():
    ...
    decision = await engine.decide_on_action(
        ["scale_up", "optimize_config", "restart_component"],
        context
    )
