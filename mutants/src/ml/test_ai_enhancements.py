...
class TestGraphSAGEV3Enhancements:
    ...
    def test_adaptive_threshold(self, detector):
        ...
        logger.info(f"✓ Adaptive threshold: good={threshold_good:.2f}, poor={threshold_poor:.2f}")
    ...
    def test_feature_normalization(self, detector):
        ...
        normalized = detector.normalizer.normalize_features(
            features,
            detector.adaptive_threshold.baseline
        )
...
class TestEnhancedCausalAnalysis:
    ...
    def test_temporal_pattern_detection(self, analyzer):
        ...
        root_causes = analyzer._analyze_temporal_patterns(analyzer.incidents['recurring-0'])
        
        # Should detect pattern
        has_pattern = any('pattern' in rc.root_cause_id for rc in root_causes)
        logger.info(f"✓ Temporal pattern detection: {'found' if has_pattern else 'none'}")
...
class TestIntegration:
    ...
    def test_integrated_report(self, integrated_analyzer):
        ...
        for i in range(3):
            node_features = {'rssi': -75, 'loss_rate': 0.02 + i*0.01, 'cpu_percent': 60 + i*5}
        ...
        logger.info(f"✓ Integrated report generated: {report['summary']['total_incidents']} incidents")
...
class BenchmarkComparisons:
    ...
    def test_graphsage_inference_latency(self):
        ...
        # Verify targets
        assert avg_latency < 30, f"Average latency {avg_latency:.1f}ms exceeds <30ms target"
    ...
    def test_causal_analysis_latency(self):
        ...
        from src.ml.causal_analysis_v2_enhanced import EnhancedCausalAnalysisEngine, IncidentEvent, IncidentSeverity
