...
class Policy:
    ...
    def matches_target(self, resource: str, action: str) -> bool:
        ...
        resource_match = (
            resource_pattern == "*" or
            resource == resource_pattern or
            (isinstance(resource_pattern, list) and resource in resource_pattern)
        )
        
        action_match = (
            action_pattern == "*" or
            action == action_pattern or
            (isinstance(action_pattern, list) and action in action_pattern)
        )
...
class PolicyEngine:
    ...
    def _build_attributes(
        ...
        # Resource attributes
        attributes["resource.name"] = Attribute(AttributeType.RESOURCE, "name", resource)
        attributes["resource.endpoint"] = Attribute(AttributeType.RESOURCE, "endpoint", resource)
    ...
    def _cache_key(self, attributes: Dict[str, Attribute]) -> str:
        ...
        key_parts = sorted(
            f"{k}:{v.value}" for k, v in attributes.items()
        )
        ...
        def decorator(func: Callable) -> Callable:
            def wrapper(*args, **kwargs):
                ...
                if decision.effect == PolicyEffect.AUDIT:
                    logger.info(
                        f"AUDIT: {subject} accessed {resource}/{action}"
                    )
