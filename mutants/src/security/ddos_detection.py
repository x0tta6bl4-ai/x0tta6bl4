...
class DDoSDetector:
    ...
    def _update_baseline(self):
        ...
        recent_traffic = [
            t for t in self.traffic_history
        ...
        self.baseline.avg_bandwidth_mbps = (
            alpha * bandwidth_mbps +
            (1 - alpha) * self.baseline.avg_bandwidth_mbps
        )
    ...
    def _detect_attack(
        ...
        current_bandwidth = sum(t["packet_size"] for t in recent_traffic) * 8 / (self.detection_window * 1_000_000)
        
        # Check against baseline
        threshold_rate = self.baseline.avg_packets_per_second * self.threshold_multiplier
        threshold_bandwidth = self.baseline.avg_bandwidth_mbps * self.threshold_multiplier
    ...
    def _classify_attack(
        ...
        protocol: str,
        target_port: int,
        recent_traffic: List[Dict[str, Any]]
    ) -> DDoSType:
        ...
        if target_port in [80, 443, 8080]:
            ...
            avg_packet_size = sum(t["packet_size"] for t in recent_traffic) / len(recent_traffic)
