...
class PolicyRule:
    ...
    spiffe_id_pattern: Optional[str] = None  # Pattern matching (e.g., "spiffe://domain/workload/*")
...
class PolicyEngine:
    ...
    def __init__(
        ...
        self.rate_limit_tracker: Dict[str, List[datetime]] = {}  # spiffe_id -> [timestamps]
        ...
        logger.info(f"Policy Engine initialized with {len(self.rules)} rules (default: {default_action.value}, OPA: {self.enable_opa})")
        ...
        for rule in self.rules:
            ...
            if self._rule_matches(rule, peer_spiffe_id, resource, workload_type):
                ...
                if rule.opa_policy and self.enable_opa:
                    opa_decision = self._evaluate_opa_policy(rule, peer_spiffe_id, resource, workload_type)
    ...
    def _check_rate_limit(self, peer_spiffe_id: str, rate_limit: Dict[str, int]) -> bool:
        ...
        recent_requests = [
            ts for ts in self.rate_limit_tracker[peer_spiffe_id]
    ...
    def _log_audit(self, peer_spiffe_id: str, resource: Optional[str], decision: PolicyDecision):
        """Log audit event."""
        audit_entry = {
    ...
    def update_rule(self, rule_id: str, updated_rule: PolicyRule) -> bool:
        ...
        # Update rule in list
        rule_index = next((i for i, r in enumerate(self.rules) if r.rule_id == rule_id), None)
        if rule_index is not None:
            ...
            logger.info(f"✅ Policy rule {rule_id} updated to version {updated_rule.version}")
            return True
    ...
    def rollback_rule(self, rule_id: str, target_version: Optional[int] = None) -> bool:
        ...
        if target_version is None:
            ...
            target_rule = next((r for r in versions if r.version == target_version), None)
            if not target_rule:
        ...
        rolled_back_rule = PolicyRule(
            ...
            allowed_resources=target_rule.allowed_resources.copy() if target_rule.allowed_resources else None,
            time_window=target_rule.time_window.copy() if target_rule.time_window else None,
            rate_limit=target_rule.rate_limit.copy() if target_rule.rate_limit else None,
    ...
    def _check_advanced_conditions(self, rule: PolicyRule, peer_spiffe_id: str, resource: Optional[str]) -> bool:
        """
