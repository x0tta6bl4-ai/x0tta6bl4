...
class QuantumAuditor:
    ...
    def audit_algorithm(
        ...
        # Determine status
        if compliance and len([f for f in findings if f["severity"] in [AuditSeverity.HIGH, AuditSeverity.CRITICAL]]) == 0:
    ...
    def get_audit_summary(self) -> Dict[str, Any]:
        ...
        validated = sum(1 for a in self.audit_results.values() if a.status == PQAlgorithmStatus.VALIDATED)
        failed = sum(1 for a in self.audit_results.values() if a.status == PQAlgorithmStatus.FAILED)
        ...
        return {
            ...
            "compliance_rate": (validated / total_algorithms * 100) if total_algorithms > 0 else 0.0
        }
