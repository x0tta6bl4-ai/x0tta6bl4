...
class AlertingRules:
    ...
    def __init__(self):
        ...
        logger.info(f"AlertingRules initialized with {len(self.rules)} rules, {len(self.routes)} routes, {len(self.groups)} groups")
    ...
    def get_rules_by_component(self, component: str) -> List[AlertRule]:
        ...
        return [r for r in self.rules if r.labels.get("component") == component and r.enabled]
    ...
    def _build_promql_expr(self, rule: AlertRule) -> str:
        ...
        metric = rule.metric_name.split("{")[0] if "{" in rule.metric_name else rule.metric_name
    ...
    def add_suppression(self, suppression: AlertSuppression) -> bool:
        ...
        if suppression.duration:
            ...
            suppression.expires_at = datetime.now() + timedelta(seconds=duration_seconds)
        
        self.suppressions.append(suppression)
        logger.info(f"Added alert suppression: {suppression.name} (expires: {suppression.expires_at})")
        return True
    ...
    def remove_suppression(self, suppression_id: str) -> bool:
        ...
        self.suppressions = [s for s in self.suppressions if s.suppression_id != suppression_id]
    ...
    def is_alert_suppressed(self, alert_labels: Dict[str, str]) -> bool:
        ...
        for suppression in self.suppressions:
            ...
            # Check if all match_labels are present in alert_labels
            if all(alert_labels.get(k) == v for k, v in suppression.match_labels.items()):
    ...
    def get_alerting_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_suppressions": len([s for s in self.suppressions if not s.expires_at or s.expires_at > datetime.now()])
        }
