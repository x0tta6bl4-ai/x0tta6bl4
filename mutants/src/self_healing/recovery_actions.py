...
class RateLimiter:
    ...
    def allow(self) -> bool:
        ...
        while self.action_times and (now - self.action_times[0]).total_seconds() > self.window_seconds:
            self.action_times.popleft()
        
        # Check if limit exceeded
        if len(self.action_times) >= self.max_actions:
            logger.warning(f"Rate limit exceeded: {len(self.action_times)}/{self.max_actions} actions in {self.window_seconds}s")
            return False
...
class RecoveryActionExecutor:
    ...
    def execute(self, action: str, context: Optional[Dict[str, Any]] = None) -> bool:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if result.success:
                    logger.info(f"✅ Recovery action executed: {action_type.value} (duration: {result.duration_seconds:.2f}s)")
                else:
                    logger.error(f"❌ Recovery action failed: {action_type.value} - {result.error_message}")
                
                return result.success
                
            except Exception as e:
                last_exception = e
                logger.warning(f"Recovery action attempt {attempt + 1}/{self.max_retries} failed: {e}")
                
                if attempt < self.max_retries - 1:
                    time.sleep(self.retry_delay * (attempt + 1))  # Exponential backoff
                else:
                    duration = time.time() - start_time
                    logger.error(f"❌ Recovery action failed after {self.max_retries} attempts: {e}")
    ...
    def _execute_action_internal(self, action_type: RecoveryActionType, context: Dict[str, Any]) -> RecoveryResult:
    ...
    def _restart_service(self, context: Dict[str, Any]) -> RecoveryResult:
        ...
        try:
            ...
            logger.warning(f"Service restart requested but no container manager found for {service_name}")
            return RecoveryResult(
                success=True,
                action_type=RecoveryActionType.RESTART_SERVICE,
                duration_seconds=0.0,
                details={"method": "simulated", "service": service_name, "node_id": node_id}
            )
    ...
    def _scale_up(self, context: Dict[str, Any]) -> RecoveryResult:
        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0:
                    return RecoveryResult(
                        ...
                        duration_seconds=0.0,
                        details={"method": "kubernetes", "service": service_name, "replicas": replicas}
                    )
            except (FileNotFoundError, subprocess.TimeoutExpired):
                pass
            ...
            return RecoveryResult(
                ...
                action_type=RecoveryActionType.SCALE_UP,
                duration_seconds=0.0,
                details={"method": "simulated", "service": service_name, "replicas": replicas}
            )
    ...
    async def scale_up(self, deployment_name: str, replicas: int, namespace: str = "default") -> bool:
    ...
    async def scale_down(self, deployment_name: str, replicas: int, namespace: str = "default") -> bool:
    ...
    async def execute_action(self, action_type: str, context: Optional[Dict[str, Any]] = None, **kwargs) -> bool:
    ...
    def get_circuit_breaker_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_failure": self.circuit_breaker.state.last_failure_time.isoformat() if self.circuit_breaker.state.last_failure_time else None
        }
