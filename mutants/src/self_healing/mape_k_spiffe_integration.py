...
from src.security.spiffe.workload import X509SVID, JWTSVID
...
class SPIFFEMapEKLoop:
    ...
    async def initialize(self):
        ...
        try:
            ...
            # Initialize with join token from environment
    ...
    async def monitor(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ttl_percentage = (time_to_expiry.total_seconds() / 3600.0) * 100  # Assuming 1h TTL
            ...
            if trust_bundle and trust_bundle.get("version", 0) != self.trust_bundle_version:
                logger.info(f"Trust bundle updated: v{self.trust_bundle_version} → v{trust_bundle['version']}")
    ...
    async def analyze(self, monitor_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"📊 Analysis complete: {analysis['total_anomalies']} anomalies, risk_level={analysis['risk_level']}")
        return analysis
    ...
    async def plan(self, analysis_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"📋 Plan created: {len(plan['actions'])} actions, priority={plan['priority']}")
        return plan
    ...
    async def execute(self, plan_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"✅ Execution complete: {results['actions_executed']} succeeded, {results['actions_failed']} failed")
        return results
    ...
    async def knowledge(self, execute_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"📚 Knowledge: {knowledge['total_alerts']} total alerts, {knowledge['current_identities']} active identities")
        return knowledge
