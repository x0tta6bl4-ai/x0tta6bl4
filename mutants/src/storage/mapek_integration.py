...
class MAPEKKnowledgeStorageAdapter:
    ...
    async def record_incident_async(
        ...
        try:
            incident_id = await self.knowledge_storage.store_incident(incident, self.node_id)
        ...
        try:
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    logger.warning("⚠️ Cannot run async search in running loop. Use async method.")
                    return []
                else:
                    # If no loop, run it
