...
class DigitalTwinsSimulator:
    ...
    async def run_chaos_test(
        ...
        Returns:
            Результат chaos-теста
        """
        logger.info(f"🚨 ЗАПУСК CHAOS-ТЕСТА: {scenario.value} с интенсивностью {intensity}")
        ...
        success = actual_recovery_time < 3.0 and final_impact.get('network_health', 0) > 0.9
    ...
    async def _apply_chaos_scenario(
        ...
        if scenario == ChaosScenario.NODE_DOWN:
            ...
            for node_id in targets:
                self.nodes[node_id].cpu_usage = min(100.0, self.nodes[node_id].cpu_usage * 3)
                self.nodes[node_id].packet_loss = min(50.0, self.nodes[node_id].packet_loss * 10)
                affected_nodes.append(node_id)
    ...
    def _collect_metrics(self) -> Dict:
        ...
        avg_cpu = sum(n.cpu_usage for n in self.nodes.values()) / total_nodes if total_nodes > 0 else 0
        avg_memory = sum(n.memory_usage for n in self.nodes.values()) / total_nodes if total_nodes > 0 else 0
        avg_latency = sum(n.latency for n in self.nodes.values()) / total_nodes if total_nodes > 0 else 0
        avg_loss = sum(n.packet_loss for n in self.nodes.values()) / total_nodes if total_nodes > 0 else 0
    ...
    def _calculate_impact(self, metrics: Dict) -> Dict:
        ...
        return {
            ...
            'node_availability': metrics.get('healthy_nodes', 0) / metrics.get('total_nodes', 1),
