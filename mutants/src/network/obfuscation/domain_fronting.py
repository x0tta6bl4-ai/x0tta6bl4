...
class DomainFrontingSocket(socket.socket):
    ...
    def __init__(self, sock: socket.socket, transport: 'DomainFrontingTransport', tls_sock: socket.socket):
        self._raw_sock = sock
    ...
    def recv(self, bufsize: int, flags=0) -> bytes:
        ...
        try:
            ...
            if header_end != -1:
                ...
                self._buffer = b"" # Consume all (assuming 1 req = 1 resp for this transport mode)
                return data
...
class DomainFrontingTransport(ObfuscationTransport):
    ...
    def wrap_socket(self, sock: socket.socket) -> socket.socket:
        ...
        tls_sock = self.context.wrap_socket(
            sock, 
            server_hostname=self.front_domain,
