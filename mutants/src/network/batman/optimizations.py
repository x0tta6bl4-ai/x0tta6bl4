...
class AODVFallback:
    ...
    def request_route(self, destination: str) -> bool:
        ...
        if destination in self.active_requests:
            if self.active_requests[destination] >= self.config.aodv_route_request_retries:
...
class BatmanAdvOptimizations:
    ...
    def __init__(self, config: Optional[BatmanAdvConfig] = None):
        ...
        self.multipath_router = MultiPathRouter(self.config) if self.config.multipath_enabled else None
        self.aodv_fallback = AODVFallback(self.config) if self.config.aodv_enabled else None
    ...
    def apply_config(self) -> Dict[str, str]:
        ...
        config["retransmission_buffer_size"] = str(self.config.retransmission_buffer_size)
