...
class EBPFGraphSAGEStreaming:
    ...
    def update_node_from_ebpf(self, node_id: str, ebpf_metrics: Dict[str, float]) -> Dict[str, float]:
        """
        ...
        features = {
            ...
            'throughput': self._calculate_throughput(tcp_packets, udp_packets, icmp_packets),
        ...
        if node_id not in self.node_features_cache:
            self.node_features_cache[node_id] = deque(maxlen=self.node_features_window_size)
...
def integrate_ebpf_graphsage_mapek(
    mapek_loop,
    ebpf_exporter,
    graphsage_detector
):
    ...
    streaming = EBPFGraphSAGEStreaming(
        graphsage_detector=graphsage_detector,
        metrics_exporter=ebpf_exporter
    )
