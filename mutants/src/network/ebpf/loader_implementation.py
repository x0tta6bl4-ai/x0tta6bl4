...
class EBPFLoaderImplementation(EBPFLoader):
    ...
    def _release_bpf_maps(self, program_id: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.debug(f"Maps for program {program_id} will be released on unload")
                return True
        except Exception as e:
    ...
    def attach_to_interface_complete(
        ...
        program_id: str,
        interface: str,
        mode: EBPFAttachMode = EBPFAttachMode.DRV
    ) -> bool:
        """
    ...
    def detach_from_interface_complete(self, program_id: str, interface: str) -> bool:
        ...
        if success:
            ...
            if self._verify_program_detached(program_id):
                logger.info(f"✅ Verified program {program_id} detached from {interface}")
            else:
                logger.warning(f"⚠️ Program {program_id} may still be attached to {interface}")
    ...
    def get_program_stats(self, program_id: str) -> Optional[Dict]:
        ...
        stats = {
            ...
            'type': program_info.get('type', 'unknown').value if hasattr(program_info.get('type'), 'value') else 'unknown',
            'path': program_info.get('path', 'unknown'),
            'attached_to': program_info.get('attached_to'),
            'attach_mode': program_info.get('attach_mode', 'unknown').value if hasattr(program_info.get('attach_mode'), 'value') else 'unknown',
            'pinned_path': program_info.get('pinned_path'),
