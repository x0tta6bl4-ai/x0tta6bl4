...
class UnsupervisedAnomalyDetector:
    ...
    def __init__(
        ...
        self.model_save_path = model_save_path or os.getenv("VAE_MODEL_PATH", "./models/vae_detector")
        ...
        self.vae_training_samples = deque(maxlen=10000)  # Store recent samples for retraining
    ...
    def add_training_sample(self, features: Dict[str, float]):
        ...
        if self.vae_detector and self.auto_train_vae:
            ...
            if len(self.vae_training_samples) >= self.min_samples_for_vae and not self.vae_is_trained:
                logger.info(f"📊 Collected {len(self.vae_training_samples)} samples, starting VAE training...")
                self.train_vae(list(self.vae_training_samples))
            elif len(self.vae_training_samples) >= self.min_samples_for_vae * 2 and self.vae_is_trained:
                # Retrain if we have 2x the minimum samples
                logger.info(f"📊 Collected {len(self.vae_training_samples)} samples, retraining VAE...")
                self.train_vae(list(self.vae_training_samples))
    
    def train(self, features_list: List[Dict[str, float]], vae_epochs: Optional[int] = None, vae_batch_size: Optional[int] = None, vae_lr: Optional[float] = None):
        """
    ...
    def train_vae(
        ...
        if len(features_list) < batch_size:
            logger.warning(f"⚠️ Not enough samples for VAE training ({len(features_list)} < {batch_size}), skipping")
            return
        ...
        for epoch in range(epochs):
            ...
            avg_val_loss = val_loss / (len(X_val) // batch_size + 1) if len(X_val) > 0 else avg_train_loss
            self.vae_detector.train()
            ...
            if avg_val_loss < self.vae_best_loss:
                ...
                if self.vae_patience_counter >= self.early_stopping_patience:
                    logger.info(f"⏹️ Early stopping at epoch {epoch + 1}/{epochs} (patience: {self.early_stopping_patience})")
                    # Load best model
                    self._load_vae_model()
                    break
            
            if (epoch + 1) % 10 == 0:
                logger.debug(f"VAE epoch {epoch + 1}/{epochs}, train_loss: {avg_train_loss:.4f}, val_loss: {avg_val_loss:.4f}")
        
        self.vae_detector.eval()
        self.vae_is_trained = True
        logger.info(f"✅ VAE training completed after {epoch + 1} epochs (best val_loss: {self.vae_best_loss:.4f})")
    ...
    def predict(self, features: Dict[str, float]) -> Dict[str, any]:
        ...
        if self.vae_detector:
            ...
            if hasattr(self, 'vae_feature_min') and hasattr(self, 'vae_feature_range'):
                feature_normalized = (feature_array - self.vae_feature_min) / self.vae_feature_range
            else:
                # Fallback: simple normalization
                feature_normalized = feature_array / (np.abs(feature_array).max() + 1e-10)
    ...
    def _save_vae_model(self):
        ...
        try:
            ...
            norm_data = {
                "feature_min": self.vae_feature_min.tolist() if self.vae_feature_min is not None else None,
                "feature_max": self.vae_feature_max.tolist() if self.vae_feature_max is not None else None,
                "feature_range": self.vae_feature_range.tolist() if self.vae_feature_range is not None else None,
    ...
    def _load_vae_model(self):
        ...
        try:
            ...
            if os.path.exists(model_path):
                self.vae_detector.load_state_dict(torch.load(model_path))
                self.vae_detector.eval()
