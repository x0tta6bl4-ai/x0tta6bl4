...
class AdaptiveParameterTuner:
    ...
    def adapt_parameters(self, target_latency_ms: float = 100) -> HNSWParameters:
        ...
        if p95 > target_latency_ms * 1.5:
            self.current_params.ef_search = min(
                self.current_params.ef_search - 5,
                10
            )
            logger.info(f"Reduced ef_search to {self.current_params.ef_search}")
        elif p95 < target_latency_ms * 0.5:
            self.current_params.ef_search = min(
                self.current_params.ef_search + 10,
                500
            )
...
class HNSWPerformanceOptimizer:
    ...
    async def batch_retrieve(
        ...
        tasks = [
            self.retrieve_with_optimization(q, retrieval_fn, k=k)
            for q in queries
        ]
        ...
        return {
            query: results
            for query, (results, _) in zip(queries, results_list)
        }
