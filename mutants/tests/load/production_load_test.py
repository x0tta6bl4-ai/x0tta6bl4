...
class ProductionLoadTester:
    ...
    async def _run_ramp_up_test(self, session: aiohttp.ClientSession):
        ...
        for step in range(steps):
            ...
            tasks = []
            requests_per_user = (self.config.total_requests // self.config.concurrent_users) // steps
        ...
        for _ in range(count):
            try:
                ...
                if self.config.test_mesh_routing:
                    start = time.time()
                    async with session.get(f"{self.config.base_url}/mesh/peers", timeout=5) as resp:
    ...
    def get_summary(self) -> Dict[str, any]:
        ...
        if self.metrics.total_requests > 0:
            success_rate = (self.metrics.successful_requests / self.metrics.total_requests) * 100.0
            # Cap at 100%
        ...
        return {
            ...
            "avg_throughput_per_sec": self.metrics.total_requests / duration if duration > 0 else 0,
    ...
    def _check_pass_criteria(
        ...
        return (
            ...
            latency_p95 <= self.config.max_latency_p95 and
