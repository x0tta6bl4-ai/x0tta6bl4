...
class TestNetworkChaos:
    ...
    async def test_inject_packet_loss(self, injector):
        ...
        await injector.inject_packet_loss(
            loss_rate=0.3,
            duration_seconds=0.5
        )
    ...
    async def test_network_partition(self, injector):
        ...
        await injector.inject_network_partition(
            partition_duration_seconds=0.2,
...
class TestResourceChaos:
    ...
    async def test_cpu_spike(self, injector):
        ...
        start = time.time()
        await injector.inject_cpu_spike(
            cpu_percent=80.0,
...
class TestDataChaos:
    ...
    async def test_data_corruption(self, injector):
        ...
        start = time.time()
        await injector.inject_data_corruption(
            corruption_rate=0.5,
            duration_seconds=0.3
        )
        duration = time.time() - start
    ...
    async def test_clock_skew(self, injector):
        ...
        start = time.time()
        await injector.inject_clock_skew(
            skew_seconds=5.0,
            duration_seconds=0.2
        )
        duration = time.time() - start
