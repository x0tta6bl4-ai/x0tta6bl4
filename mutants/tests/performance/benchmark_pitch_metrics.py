...
class MTTDBenchmark:
    ...
    async def measure_detection_time(
        self,
        failure_scenario: str = "node_failure",
        iterations: int = 10
        ...
        """
        logger.info(f"🔍 Measuring MTTD for {failure_scenario} ({iterations} iterations)...")
        ...
        for i in range(iterations):
            ...
            logger.info(f"  Iteration {i+1}: {detection_time:.2f}s (target: {self.TARGET_MTTD_SECONDS}s)")
...
class MTTRBenchmark:
    ...
    async def measure_recovery_time(
        self,
        recovery_type: str = "node_failure",
        iterations: int = 10
    ) -> List[BenchmarkResult]:
        """
        ...
        Returns:
            List of BenchmarkResult objects
        """
        logger.info(f"🔧 Measuring MTTR for {recovery_type} ({iterations} iterations)...")
        ...
        for i in range(iterations):
            ...
            logger.info(f"  Iteration {i+1}: {recovery_time:.2f}s (target: {self.TARGET_MTTR_SECONDS}s)")
...
class PitchMetricsBenchmark:
    ...
    def _print_summary(self, suite: BenchmarkSuite):
        ...
        if "pqc_handshake" in suite.summary:
            ...
            status = "✅ PASS" if s["passed"] else "❌ FAIL"
            logger.info(f"PQC Handshake p95: {s['p95']:.3f}ms (target: {s['target_p95']}ms) {status}")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Run x0tta6bl4 Pitch Metrics Benchmarks")
    ...
    parser.add_argument("--pqc", action="store_true", help="Run PQC handshake benchmark only")
    parser.add_argument("--output-dir", default="benchmarks/results", help="Output directory")
    parser.add_argument("--iterations", type=int, default=10, help="Iterations for MTTD/MTTR")
    parser.add_argument("--pqc-iterations", type=int, default=1000, help="Iterations for PQC")
    ...
    if args.all or (not args.mttd and not args.mttr and not args.pqc):
        ...
        if args.mttd:
            results = await benchmark.mttd_benchmark.measure_detection_time(iterations=args.iterations)
        if args.mttr:
            results = await benchmark.mttr_benchmark.measure_recovery_time(iterations=args.iterations)
        if args.pqc:
            results = benchmark.pqc_benchmark.measure_handshake_latency(iterations=args.pqc_iterations)
