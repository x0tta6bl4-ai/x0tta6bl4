...
class BenchmarkRunner:
    ...
    def measure_pqc_latency(self, iterations: int = 100) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(f"✅ PQC Encryption: avg={encrypt_avg:.2f}ms, p95={encrypt_p95:.2f}ms")
            logger.info(f"✅ PQC Decryption: avg={decrypt_avg:.2f}ms, p95={decrypt_p95:.2f}ms")
    ...
    def measure_graphsage_inference(self, iterations: int = 50) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(f"✅ GraphSAGE Inference: avg={avg_time:.2f}ms, p95={p95_time:.2f}ms, p99={p99_time:.2f}ms")
    ...
    async def measure_api_latency(self, base_url: str = "http://localhost:8080", iterations: int = 100) -> Dict[str, float]:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                logger.info(f"✅ API Latency: avg={avg:.2f}ms, p50={p50:.2f}ms, p95={p95:.2f}ms, p99={p99:.2f}ms")
    ...
    async def run_full_suite(self, base_url: str = "http://localhost:8080") -> BenchmarkSuite:
        ...
        if format == "json":
            ...
            with open(filename, "w", newline="") as f:
                ...
                for key, value in asdict(suite).items():
                    if value is not None and key not in ["suite_name", "timestamp", "results"]:
                        writer.writerow([key, value, "ms" if "ms" in key else "seconds", suite.timestamp])
            logger.info(f"✅ Results saved to {filename}")
            return filename
...
async def main():
    ...
    parser.add_argument("--url", default="http://localhost:8080", help="Base URL for API benchmarks")
