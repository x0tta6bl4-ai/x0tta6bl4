...
class TestErrorHandler:
    ...
    async def test_handle_error_async_with_alerting(self):
        ...
        with patch('src.core.error_handler.ALERTING_AVAILABLE', True), \
             patch('src.core.error_handler.send_alert', new_callable=AsyncMock) as mock_alert, \
            ...
            with patch('prometheus_client.Counter', side_effect=ImportError("No prometheus")):
                await handler.handle_error(
    ...
    async def test_handle_error_async_alerting_failure(self):
        ...
        with patch('src.core.error_handler.ALERTING_AVAILABLE', True), \
             patch('src.core.error_handler.send_alert', side_effect=Exception("Alert failed")), \
             patch('src.core.error_handler.AlertSeverity') as mock_alert_severity, \
             patch('src.core.error_handler.logger.critical') as mock_log, \
    ...
    def test_handle_error_sync_prometheus_metrics(self):
        ...
        with patch('src.core.error_handler.logger.warning') as mock_log:
            ...
            with patch('builtins.__import__', side_effect=lambda name, *args, **kwargs: 
                       Mock(Counter=mock_counter_class) if name == 'prometheus_client' 
    ...
    def test_handle_error_sync_prometheus_unavailable(self):
        ...
        with patch('src.core.error_handler.logger.warning') as mock_log:
            ...
            with patch('builtins.__import__', side_effect=ImportError("No module named 'prometheus_client'")):
                # Should not raise exception
                handler.handle_error_sync(
                    ValueError("Test"),
                    context="test_context",
...
class TestErrorHandlerDecorator:
    ...
    async def test_decorator_async_function_error(self):
        ...
        with patch('src.core.error_handler.ErrorHandler.handle_error', new_callable=AsyncMock) as mock_handle:
    ...
    def test_decorator_sync_function_error(self):
        ...
        with patch('src.core.error_handler.ErrorHandler.handle_error_sync') as mock_handle:
            with pytest.raises(ValueError):
                test_func()
