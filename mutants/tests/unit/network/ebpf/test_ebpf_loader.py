...
from src.network.ebpf.loader import EBPFLoader, EBPFLoadError, EBPFAttachError, EBPFProgramType, EBPFAttachMode
...
class TestEBPFLoader:
    ...
    def test_parse_elf_sections(self, mock_elf_file, loader, temp_ebpf_file):
        ...
        mock_elf = MagicMock()
        mock_elf.iter_sections.return_value = [mock_section1, mock_section2, mock_section3]
    ...
    def test_attach_to_interface_valid(self, loader, temp_ebpf_file, mock_subprocess_run):
        ...
        def mock_run_side_effect(cmd, **kwargs):
            if cmd == ['ip', 'link', 'show', 'dev', 'eth0']:
                return MagicMock(
                    returncode=0,
                    stdout='eth0: xdp program attached',
                    stderr=''
                )
            return MagicMock(
                returncode=0,
    ...
    def test_attach_to_nonexistent_interface(self, loader, temp_ebpf_file, mock_subprocess_run):
    ...
    def test_detach_from_nonexistent_interface(self, loader, temp_ebpf_file, mock_subprocess_run):
    ...
    def test_get_stats(self, loader, mock_subprocess_run):
        ...
        mock_subprocess_run.return_value = MagicMock(
            returncode=0,
            stdout=mock_json_output.encode('utf-8'),
            stderr=b''
        )
    ...
    def test_load_programs_directory_scan(self, loader, temp_ebpf_file, mock_subprocess_run):
    ...
    def test_load_program_without_btf(self, loader, temp_ebpf_file, mock_subprocess_run):
    ...
    def test_verify_xdp_attachment(self, loader, mock_subprocess_run):
        ...
        mock_subprocess_run.return_value = MagicMock(
            returncode=0,
            stdout=b'eth0: xdp program attached',
            stderr=b''
        )
            ...
            with patch('pathlib.Path.exists', return_value=True):
                with patch('pathlib.Path.read_text', return_value='up'):
                    ...
                    with patch.object(loader, 'load_program', return_value=program_id):
                        with patch.object(loader, 'loaded_programs', {program_id: {'type': EBPFProgramType.XDP}}):
                            dest_file = loader.programs_dir / 'test.o'
                            with open(dest_file, 'wb') as f:
                                f.write(b'\x00')

                            success = loader._attach_xdp(str(dest_file), 'eth0', EBPFAttachMode.SKB)
                            assert success is True
