...
try:
    from src.ml.rag import RAGAnalyzer, Document, HNSWVectorStore, VectorStore, HNSWLIB_AVAILABLE
    RAG_AVAILABLE = True
except ImportError:
    RAG_AVAILABLE = False
...
class TestRAGAnalyzerHNSW:
    ...
    async def test_rag_retrieval_latency_tracking(self):
        ...
        knowledge = [
            {"insight": f"Insight {i}", "timestamp": "2026-01-10"}
            for i in range(10)
        ]
    ...
    async def test_rag_augment_analysis(self):
        ...
        augmented = await rag.augment_analysis(current_analysis, "CPU performance issue")
    ...
    async def test_rag_batch_indexing_efficiency(self):
        ...
        for batch_idx in range(3):
            knowledge = [
                {"insight": f"Batch {batch_idx} insight {i}", "timestamp": f"2026-01-{10+batch_idx:02d}"}
                for i in range(5)
            ]
            await rag.index_knowledge(knowledge)
