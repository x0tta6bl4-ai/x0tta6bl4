...
from src.security.mesh_shield import (
    MeshShield, ThreatLevel, QuarantineReason, ThreatIndicator
)
...
class TestMeshShield(unittest.TestCase):
    ...
    def test_report_low_anomaly_no_quarantine(self):
        ...
        result = self.shield.report_indicator(ThreatIndicator(
            node_id="node-1",
    ...
    def test_replay_attack_detection(self):
        ...
        for _ in range(3):
            self.shield.report_indicator(ThreatIndicator(
                node_id="node-4",
                indicator_type="replay_attack",
                value=1.0
            ))
