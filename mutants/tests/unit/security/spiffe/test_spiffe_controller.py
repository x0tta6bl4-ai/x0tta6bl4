...
def test_spiffe_controller_identity_and_mtls(tmp_path, monkeypatch):
    ...
    with patch('src.security.spiffe.controller.spiffe_controller.SPIREAgentManager', return_value=mock_agent):
        controller = SPIFFEController()
        ...
        if hasattr(controller, 'establish_mtls_connection'):
            ...
            with patch.object(controller, 'establish_mtls_connection', return_value=mock_mtls):
        ...
        cert = x509.CertificateBuilder().subject_name(
            ...
            x509.SubjectAlternativeName([x509.UniformResourceIdentifier("spiffe://x0tta6bl4.mesh/node/mock2")]),
            critical=False,
