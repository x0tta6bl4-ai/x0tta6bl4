...
class CacheResilienceTests:
    ...
    METRICS_ENDPOINT = "http://localhost:9090/api/v1/query?query=redis_connected_clients"
    ...
    def test_cache_stop_and_recovery(self):
        ...
        subprocess.run(f"docker stop {self.REDIS_CONTAINER}", shell=True, capture_output=True)
        time.sleep(2)
        ...
        if healthy:
            print(f"✅ API correctly available during cache outage (using DB): {status}")
        else:
            print("⚠️  API degraded during cache outage (expected without optimization)")
        
        # Restart cache
        print("Restarting cache...")
        subprocess.run(f"docker start {self.REDIS_CONTAINER}", shell=True, capture_output=True)
    ...
    def test_cache_performance_degradation(self):
        ...
        if with_cache and without_cache:
            cache_benefit = (without_cache.get("avg", 0) - with_cache.get("avg", 0)) / with_cache.get("avg", 1) * 100
            print(f"\n{'='*60}")
    ...
    def test_cache_connection_pool(self):
        ...
        for num_requests in [20, 50, 100]:
            ...
            assert success_rate >= 90, f"Success rate dropped below 90%: {success_rate:.1f}%"
    ...
    def test_cache_invalidation_timing(self):
        ...
        if times:
            ...
            assert avg_time < 50, f"Average response time {avg_time:.2f}ms too high for cached data"
