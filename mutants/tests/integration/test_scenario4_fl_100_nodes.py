...
async def fl_coordinator():
    ...
    coordinator = FederatedCoordinator(
        coordinator_id="test-coordinator-100",
        config=config
    )
...
def create_mock_update(node_id: str, round_num: int, is_byzantine: bool = False) -> ModelUpdate:
    ...
    return ModelUpdate(
        ...
        weights=weights,
        loss=random.uniform(0.1, 1.0) if not is_byzantine else random.uniform(10.0, 100.0),
...
async def test_register_100_nodes(fl_coordinator):
    ...
    online_nodes = [
        node for node in fl_coordinator.nodes.values()
