...
from src.ml.rag import RAGAnalyzer, Document, VectorStore
...
class TestRAG:
    ...
    async def test_document_indexing(self):
        ...
        docs = [
            Document(id="1", content="High latency issue", metadata={"topic": "performance"}),
            Document(id="2", content="Memory leak detected", metadata={"topic": "memory"}),
        ]
...
class TestAnomalyDetection:
    ...
    async def test_detector_training(self):
        ...
        normal_samples = [
            np.random.normal(0.5, 0.1, 32) for _ in range(100)
        ]
