...
class TestCacheWarming:
    ...
    async def test_warm_cache_single_call(self):
        ...
        tasks = [
            warmer.warm_cache("test_key", expensive_func, ttl=60) for _ in range(5)
        ]
...
class TestCacheThreadSafety:
    ...
    async def test_concurrent_writes(self):
        ...
        # Concurrent writes
        tasks = [cache.set(f"key_{i}", f"value_{i}", ttl=60) for i in range(100)]
        results = await asyncio.gather(*tasks)
    ...
    async def test_cache_warming_thundering_herd(self):
        ...
        tasks = [
            warmer.warm_cache("same_key", slow_function, ttl=60) for _ in range(50)
        ]
        results = await asyncio.gather(*tasks)
