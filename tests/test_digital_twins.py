...
from src.testing.digital_twins import (ChaosScenario, ChaosTestResult,
                                       DigitalTwinNode, DigitalTwinsSimulator)
...
async def test_chaos_test_node_down(simulator):
    ...
    result = await simulator.run_chaos_test(
        scenario=ChaosScenario.NODE_DOWN, intensity=0.3, duration=10.0
    )
...
async def test_chaos_test_link_failure(simulator):
    ...
    result = await simulator.run_chaos_test(
        scenario=ChaosScenario.LINK_FAILURE, intensity=0.2, duration=10.0
    )
...
async def test_chaos_test_ddos(simulator):
    ...
    result = await simulator.run_chaos_test(
        scenario=ChaosScenario.DDOS, intensity=0.4, duration=10.0
    )
...
async def test_chaos_test_byzantine(simulator):
    ...
    result = await simulator.run_chaos_test(
        scenario=ChaosScenario.BYZANTINE, intensity=0.1, duration=10.0
    )
...
async def test_chaos_test_resource_exhaustion(simulator):
    ...
    result = await simulator.run_chaos_test(
        scenario=ChaosScenario.RESOURCE_EXHAUSTION, intensity=0.3, duration=10.0
    )
