...
class MTTRValidator:
    ...
    def complete_recovery_event(
        ...
        event.mttr_seconds = (event.recovery_time - event.detection_time).total_seconds()
    ...
    async def validate_mttr(
        ...
        while time.time() - start_time < duration:
            ...
            self.complete_recovery_event(
                event_id,
                success=recovery_time < self.target_mttr_p95 * 2,  # Success if < 2x target
    ...
    def _calculate_validation_result(
        ...
        successful_events = [e for e in self.recovery_events if e.success and e.mttr_seconds]
        ...
        for recovery_type in RecoveryType:
            type_events = [
                e for e in successful_events
                if e.recovery_type == recovery_type
            ]
            if type_events:
                type_mttr = [e.mttr_seconds for e in type_events]
                mttr_by_type[recovery_type.value] = {
                    "p50": statistics.median(type_mttr),
                    "p95": type_mttr[int(len(type_mttr) * 0.95)] if len(type_mttr) > 0 else 0.0,
                    "p99": type_mttr[int(len(type_mttr) * 0.99)] if len(type_mttr) > 0 else 0.0,
                    "avg": statistics.mean(type_mttr),
                    "count": len(type_events)
                }
        
        # Calculate success rate
        recovery_success_rate = len(successful_events) / len(self.recovery_events) if self.recovery_events else 0.0
        
        # Check if targets are met
        target_met = (
            mttr_p95 <= self.target_mttr_p95 and
            recovery_success_rate >= self.target_success_rate
        )
    ...
    def generate_report(self, result: MTTRValidationResult) -> str:
        ...
        report.append(f"  p95: {result.mttr_p95:.3f}s (target: ≤{self.target_mttr_p95}s)")
        ...
        report.append(f"  Target Met (p95 ≤ {self.target_mttr_p95}s, success ≥ {self.target_success_rate:.0%}): {'✅ YES' if result.target_met else '❌ NO'}")
...
async def main():
    ...
    parser.add_argument("--duration", type=float, default=300.0, help="Test duration in seconds")
    parser.add_argument("--target-p95", type=float, default=7.0, help="Target MTTR p95 in seconds")
