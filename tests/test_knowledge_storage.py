...
class TestKnowledgeStorageV2:
    ...
    async def test_search_incidents(self, temp_storage):
        ...
        storage = KnowledgeStorageV2(storage_path=temp_storage, use_real_ipfs=False)
        ...
        results = await storage.search_incidents(
            "memory pressure recovery", k=3, threshold=0.5
        )
...
class TestVectorIndex:
    ...
    def test_search(self, temp_storage):
        ...
        # Add documents
        index.add(text="Memory pressure detected", metadata={"incident_id": "test-1"})
        index.add(text="CPU usage high", metadata={"incident_id": "test-2"})
