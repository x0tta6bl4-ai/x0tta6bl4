...
class TestNodeManagerService:
    ...
    def test_service_initialization(self):
        ...
        config = ServiceConfig(
            node_id="test-node", port=8080, health_check_interval=5.0
        )
    ...
    def test_node_registration(self):
        ...
        peer_node = {"node_id": "peer-1", "address": "192.168.1.2", "port": 8081}
    ...
    def test_concurrent_node_registration(self):
        ...
        for i in range(5):
            thread = threading.Thread(
                target=register_worker, args=(f"peer-{i}", f"192.168.1.{i+1}", 8080 + i)
            )
