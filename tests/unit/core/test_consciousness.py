...
class TestConsciousnessEngine:
    ...
    def test_calculate_entropy_with_history(self, engine):
        ...
        for i in range(20):
            ...
            engine.history.append(ConsciousnessMetrics(
                phi_ratio=phi,
                state=state,
    ...
    def test_calculate_frequency_alignment_bounds(self, engine):
        ...
        alignment_high = engine.calculate_frequency_alignment(1.5, current_frequency=1000.0)
    ...
    def test_evaluate_state_recovery_mode(self, engine):
        ...
        state = engine.evaluate_state(1.3)  # Normally HARMONIC, but in recovery > 1.2 = EUPHORIC
        assert state == ConsciousnessState.EUPHORIC
    ...
    def test_get_consciousness_metrics_with_timestamp(self, engine, optimal_metrics):
        ...
        metrics = engine.get_consciousness_metrics(optimal_metrics, timestamp=custom_timestamp)
    ...
    def test_get_trend_analysis_improving(self, engine, optimal_metrics):
        ...
        # Build history with improving phi-ratios
        for i in range(60):
            metrics_dict = {
                'cpu_percent': 60.0 - (i * 0.1),  # Improving
                'memory_percent': 65.0,
                'latency_ms': 85.0 - (i * 0.1),  # Improving
                'packet_loss': 0.5,
                'mesh_connectivity': 10
            }
        ...
        assert trend['trend'] in ['improving', 'stable']  # May be stable if improvement is small
    ...
    def test_get_trend_analysis_degrading(self, engine):
        ...
        assert trend['trend'] in ['degrading', 'stable']  # May be stable if degradation is small
        assert 'slope' in trend
