...
class TestMetaCognitiveMAPEK:
    ...
    async def test_execute(self, meta_mape_k):
        ...
        plan = {
            "recovery_plan": {
                "steps": [{"action": "test_action", "description": "Test step"}]
            },
    ...
    async def test_full_cycle_with_error(self, meta_mape_k):
        ...
        with patch.object(
            meta_mape_k, "meta_planning", side_effect=Exception("Test error")
        ):
...
class TestReasoningApproach:
    ...
    def test_enum_values(self):
        ...
        assert (
            ReasoningApproach.COMBINED_RAG_GRAPHSAGE.value == "combined_rag_graphsage"
        )
