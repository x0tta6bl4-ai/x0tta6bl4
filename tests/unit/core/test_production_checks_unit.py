...
class TestCheckProductionDependenciesProduction:
    ...
    def test_production_mode_with_all_deps(self, monkeypatch):
        """Test production mode succeeds when all dependencies present."""
        monkeypatch.setenv("X0TTA6BL4_PRODUCTION", "true")
        
        # Mock all required modules as present
        mock_modules = {
            'torch': MagicMock(),
            'redis': MagicMock(),
        ...
        with patch.dict('sys.modules', mock_modules, clear=False):
            # Should not raise if all deps are available
            try:
                check_production_dependencies()
            except ProductionDependencyError as e:
                # May still fail if PQC or GraphSAGE missing
...
class TestCriticalComponents:
    ...
    def test_pqc_component_critical(self):
        ...
        status = get_dependency_status()
        pqc_status = next((s for s in status if "PQC" in s[0] or "liboqs" in s[0]), None)
    ...
    def test_ml_component_critical(self):
        ...
        status = get_dependency_status()
        ml_components = [s for s in status if any(
            x in s[0] for x in ["GraphSAGE", "PyTorch", "ML"]
        )]
    ...
    def test_observability_components(self):
        ...
        status = get_dependency_status()
        obs_components = [s for s in status if any(
            x in s[0] for x in ["Prometheus", "OpenTelemetry", "trace"]
        )]
    ...
    def test_infrastructure_components(self):
        ...
        status = get_dependency_status()
        infra_components = [s for s in status if any(
            x in s[0] for x in ["Redis", "eBPF", "BCC"]
        )]
...
class TestDependencyStatusMessages:
    ...
    def test_message_for_unavailable_component(self):
        ...
        for name, available, message in unavailable_items:
            ...
            assert any(x in message.lower() for x in ["not", "missing", "unavailable", "fail"])
...
class TestProductionModeDetection:
    ...
    def test_production_mode_defaults_false(self, monkeypatch):
        """Test production mode defaults to false."""
        # When env var not set, should default to false
        monkeypatch.delenv("X0TTA6BL4_PRODUCTION", raising=False)
        
        # Reimport to pick up env change
        import importlib
        import src.core.production_checks
        importlib.reload(src.core.production_checks)
        
        # Production mode should be false by default
        assert src.core.production_checks.PRODUCTION_MODE is False
...
class TestDependencyCheckErrorHandling:
    ...
    def test_attribute_error_handling(self):
        ...
        mock_module.cuda = MagicMock(side_effect=AttributeError("no cuda"))
...
class TestProductionReadiness:
    ...
    def test_status_provides_actionable_info(self):
        ...
        for name, available, message in status:
            ...
            assert "available" in message.lower() or "not" in message.lower() or "missing" in message.lower()
