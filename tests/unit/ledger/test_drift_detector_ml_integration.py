...
from src.ledger.drift_detector import LedgerDriftDetector, DriftResult


@pytest.fixture
def mock_drift_detector():
    """Create drift detector with mocked ML components"""
    with patch('src.ml.graphsage_anomaly_detector.GraphSAGEAnomalyDetector') as MockGraphSAGE, \
...
async def test_graphsage_ml_integration(mock_drift_detector):
    ...
    assert result.get("ml_integration", {}).get("graphsage_used", False), \
        "GraphSAGE ML integration should be used"
    
    # Verify anomalies are detected
    assert "anomalies" in result
    if result["anomalies"]:
        assert any(a.get("method") != "fallback_statistics" for a in result["anomalies"]), \
...
async def test_causal_analysis_ml_integration(mock_drift_detector):
    ...
    assert mock_causal.add_incident.called, "CausalAnalysisEngine.add_incident() should be called"
    assert mock_causal.analyze.called, "CausalAnalysisEngine.analyze() should be called"
    
    # Verify ML integration flag
    assert result.get("ml_integration", {}).get("causal_analysis_used", False), \
        "Causal Analysis ML integration should be used"
    
    # Verify root causes are detected
    assert "root_causes" in result
    if result["root_causes"]:
        assert any(rc.get("method") == "ml_causal_analysis" for rc in result["root_causes"]), \
