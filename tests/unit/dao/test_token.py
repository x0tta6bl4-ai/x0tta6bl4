...
from src.dao.token import (MeshToken, ResourceType,
                           create_token_integrated_governance)
...
class TestMeshToken(unittest.TestCase):
    ...
    def test_resource_price_calculation(self):
        ...
        bandwidth_price = self.token.get_resource_price(
            ResourceType.BANDWIDTH, 1000
        )  # 1000 MB
        self.assertEqual(bandwidth_price, 1.0)  # 0.001 * 1000

        relay_price = self.token.get_resource_price(
            ResourceType.RELAY, 10000
        )  # 10k relays
