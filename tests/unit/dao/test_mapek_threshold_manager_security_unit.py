...
def test_apply_threshold_changes_writes_hmac_and_audit(tmp_path, monkeypatch):
    ...
    records = (
        manager.threshold_audit_file.read_text(encoding="utf-8").strip().splitlines()
    )
    ...
    assert (
        event["previous_values"]["cpu_threshold"] == DEFAULT_THRESHOLDS["cpu_threshold"]
    )
    assert len(event["thresholds_sha256"]) == 64

    thinking_status = manager.get_thinking_status()
    thinking_context = thinking_status["last_thinking_context"]
    assert thinking_status["thinking"]["profile"]["role"] == "governance"
    assert "zero_trust_review" in thinking_status["thinking"]["techniques"]
    assert thinking_context["applied"]["framing"]["problem"] == "mapek_threshold_change"
    assert (
        thinking_context["applied"]["framing"]["constraints"]["must_validate_bounds"]
        is True
    )
    assert "local-integrity-key" not in json.dumps(thinking_context, sort_keys=True)
...
def test_rejects_unknown_or_out_of_bounds_values(tmp_path, monkeypatch):
    ...
    assert (
        manager.apply_threshold_changes({"rogue_threshold": 10.0}, source="manual")
        is False
    )
    assert (
        manager.apply_threshold_changes({"cpu_threshold": 0.0}, source="manual")
        is False
    )
    assert (
        manager.apply_threshold_changes({"latency_threshold": 90000.0}, source="manual")
        is False
    )

    assert manager.get_threshold("cpu_threshold") == DEFAULT_THRESHOLDS["cpu_threshold"]
    assert (
        manager.get_threshold("latency_threshold")
        == DEFAULT_THRESHOLDS["latency_threshold"]
    )
...
def test_fallback_to_defaults_on_hmac_mismatch(tmp_path, monkeypatch):
    ...
    assert (
        manager.apply_threshold_changes({"cpu_threshold": 72.0}, source="dao") is True
    )
    ...
    assert (
        reloaded.get_threshold("cpu_threshold") == DEFAULT_THRESHOLDS["cpu_threshold"]
    )
...
def test_fallback_to_defaults_when_hmac_missing(tmp_path, monkeypatch):
    manager = _make_manager(tmp_path, monkeypatch, hmac_key="mesh-hmac-key")
    assert (
        manager.apply_threshold_changes({"cpu_threshold": 70.0}, source="dao") is True
    )
