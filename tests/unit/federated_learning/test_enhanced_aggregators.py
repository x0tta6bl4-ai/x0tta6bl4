...
try:
    ...
    from src.federated_learning.aggregators_enhanced import (
        AdaptiveAggregator, AggregationMetrics, EnhancedAggregator,
        EnhancedFedAvgAggregator, get_enhanced_aggregator)
        ...
        updates = [
            ModelUpdate(
                ...
                weights=ModelWeights(
                    layer_weights={"layer1": [1.0 + i * 0.1, 2.0 + i * 0.1]}
                ),
                num_samples=100,
            )
            for i in range(3)
    ...
    def test_enhanced_fedavg_aggregation(self):
        ...
        updates = [
            ModelUpdate(
                ...
                round_number=1,
                weights=ModelWeights(
                    layer_weights={"layer1": [float(i), float(i + 1)]}
                ),
                num_samples=100,
                training_loss=0.5,
            )
...
class TestAdaptiveAggregator:
    ...
    def test_adaptive_aggregator_initialization(self):
        ...
        aggregator = AdaptiveAggregator(trust_threshold=0.8, outlier_threshold=2.0)
