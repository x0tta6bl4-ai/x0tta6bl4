...
class TestModelWeights:
    ...
    def test_creation(self):
        weights = ModelWeights(
            layer_weights={"layer1": [1.0, 2.0, 3.0]},
            layer_biases={"layer1": [0.1]}
        )
...
class TestFullTrainingCycle:
    ...
    def test_complete_training_round(self):
        ...
        coordinator.initialize_model(
            ModelWeights(layer_weights={"flat": [0.0] * 10})
        )
    ...
    def test_multiple_rounds(self):
        ...
        coordinator.initialize_model(
            ModelWeights(layer_weights={"flat": [0.0] * 5})
        )
    ...
    def test_metrics_tracking(self):
        ...
        coordinator.initialize_model(
            ModelWeights(layer_weights={"flat": [0.0]})
        )
        
        # Run a round
        round_obj = coordinator.start_round()
        for node_id in round_obj.selected_nodes:
            coordinator.submit_update(ModelUpdate(
                node_id=node_id,
                round_number=1,
                weights=ModelWeights(layer_weights={"flat": [1.0]}),
                num_samples=100
