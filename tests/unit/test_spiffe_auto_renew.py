...
async def test_auto_renew_threshold_reached(client, mock_svid):
    ...
    client.current_svid.expiry = datetime.utcnow() + timedelta(seconds=100)  # 100 seconds remaining
...
async def test_auto_renew_threshold_not_reached(client, mock_svid):
    ...
    client.current_svid.expiry = datetime.utcnow() + timedelta(seconds=50000)  # 50000 seconds remaining
...
async def test_auto_renew_error_handling(client, mock_svid):
    ...
    client.current_svid = mock_svid
    client.current_svid.expiry = datetime.utcnow() + timedelta(seconds=100)  # Within threshold
    
    # Mock fetch_x509_svid to raise an error
    if asyncio.iscoroutinefunction(client.fetch_x509_svid):
        client.fetch_x509_svid = AsyncMock(side_effect=Exception("SPIRE connection failed"))
    else:
