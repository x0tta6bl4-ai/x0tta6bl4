...
def test_collect_rejects_hidden_overclaim_fields_in_proof_json(
    ...
    payload["customer_traffic_claim_allowed"] = True
    payload["observed_evidence"]["production_readiness_claim_allowed"] = True
    ...
    assert report["event_written"] is False
    assert report["ready_for_proof_gate"] is False
    assert "traffic_delivery_input_schema_validation_failed" in report["blockers"]
