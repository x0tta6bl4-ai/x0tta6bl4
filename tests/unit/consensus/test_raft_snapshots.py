...
class TestSnapshotCreation:
    ...
    def test_create_snapshot(self, temp_storage):
        ...
        with patch("src.consensus.raft_production.RaftNode") as mock_raft:
            ...
            mock_log = [
                LogEntry(
                    term=1, index=i, command={"op": "set"}, timestamp=datetime.now()
                )
                for i in range(6)
            ]
            mock_node = Mock()
    ...
    def test_multiple_snapshots_sequence(self, temp_storage):
        ...
        for snapshot_index in [10, 20, 30]:
            ...
            assert node.create_snapshot(
                snapshot_index, {"snapshot_num": len(snapshots_created) + 1}
            )
