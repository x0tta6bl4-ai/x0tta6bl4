...
class TestSPIFFEEdgeCases:
    ...
    async def test_socket_path_not_found(self, monkeypatch, tmp_path):
        ...
        client = WorkloadAPIClient(socket_path=Path("/nonexistent/socket.sock"))
    ...
    async def test_invalid_spiffe_id_format(self, monkeypatch, tmp_path):
        ...
        for invalid_id in invalid_ids:
            ...
            result = client.validate_peer_svid(
                invalid_svid, expected_id="spiffe://valid/"
            )
            # Invalid IDs should fail validation
            assert (
                result is False or invalid_id == ""
            )  # Empty might be handled differently
...
@pytest.mark.skipif(
    not SPIFFE_AVAILABLE or MTLSControllerProduction is None,
    reason="SPIFFE mTLS components not available",
)
class TestMTLSEdgeCases:
    ...
    async def test_mtls_connection_failure(self, monkeypatch, tmp_path):
        ...
        if hasattr(controller, "setup_mtls_context"):
            with patch.object(
                controller.workload_api, "fetch_x509_svid", new_callable=AsyncMock
            ) as mock_fetch:
...
class TestSPIFFESecurityBoundaries:
    ...
    async def test_path_traversal_prevention(self, monkeypatch, tmp_path):
        ...
        for malicious_id in malicious_ids:
            ...
            result = client.validate_peer_svid(
                malicious_svid, expected_id="spiffe://test/valid/"
            )
