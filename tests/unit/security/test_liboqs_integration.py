...
try:
    from security.post_quantum_liboqs import (LIBOQS_AVAILABLE,
                                              HybridPQEncryption,
                                              LibOQSBackend,
...
class TestPQMeshSecurityLibOQS:
    ...
    def test_initialization(self):
        ...
        mesh_sec = PQMeshSecurityLibOQS(
            "node-1", kem_algorithm="ML-KEM-768", sig_algorithm="ML-DSA-65"
        )
    ...
    def test_sign_verify_beacon(self):
        ...
        is_valid = mesh_sec.verify_beacon(
            beacon_data, signature, mesh_sec.sig_keypair.public_key
        )
        assert is_valid
        ...
        is_invalid = mesh_sec.verify_beacon(
            wrong_data, signature, mesh_sec.sig_keypair.public_key
        )
        assert not is_invalid
