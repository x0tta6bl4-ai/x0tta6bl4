...
from src.security.auto_isolation import (AutoIsolationManager, CircuitBreaker,
                                         IsolationLevel, IsolationReason)
from src.security.continuous_verification import (ContinuousVerificationEngine,
                                                  VerificationResult,
                                                  VerificationType)
...
class TestDecentralizedIdentity:
    ...
    def test_presentation_creation(self):
        ...
        presentation = manager.create_presentation(
            credentials=[vc], challenge="random-challenge-123"
        )
...
class TestPolicyEngine:
    ...
    def test_health_endpoint_allow(self):
        ...
        decision = engine.evaluate(
            subject={"node_id": "any-node"}, resource="/health", action="read"
        )
    ...
    def test_custom_policy(self):
        ...
        decision = engine.evaluate(
            subject={"node_id": "any"}, resource="/metrics/cpu", action="read"
        )
...
class TestContinuousVerification:
    ...
    def test_session_creation(self):
        ...
        session = engine.create_session(
            entity_id="peer-1", metadata={"device_fingerprint": "abc123"}
        )
    ...
    def test_session_verification(self):
        ...
        passed, checks, score = engine.verify_session(
            session.session_id,
            context={"claimed_id": "peer-1", "device_trust_level": 70},
        )
    ...
    def test_identity_mismatch_detection(self):
        ...
        identity_check = next(c for c in checks if c.type == VerificationType.IDENTITY)
    ...
    def test_session_expiration(self):
        ...
        passed, checks, score = engine.verify_session(
            session.session_id, check_types=[VerificationType.SESSION]
        )

        session_check = next(c for c in checks if c.type == VerificationType.SESSION)
    ...
    def test_adaptive_verification_interval(self):
        ...
        engine = ContinuousVerificationEngine(
            "node-1", base_interval_seconds=60, max_interval_seconds=300
        )
...
class TestIntegration:
    ...
    def test_full_zero_trust_flow(self):
        ...
        session = verification_engine.create_session(
            entity_id=did_manager.did, metadata={"device_fingerprint": "test-device"}
        )
