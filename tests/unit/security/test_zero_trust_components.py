...
class TestDeviceAttestation:
    ...
    def test_mesh_device_attestor(self):
        ...
        assert valid
        assert (
            trust_score.level.value >= TrustLevel.LOW.value
        )  # Initial trust starts low


@pytest.mark.skip(
    reason="SimplifiedNTRU, HybridEncryption, QuantumSafeKeyExchange, PQMeshSecurity are deprecated. Use LibOQS-based classes instead."
)
class TestPostQuantumCrypto:
    ...
    def test_quantum_safe_key_exchange(self):
        ...
        # B responds
        shared_secret_b, response_msg = exchange_b.respond_to_exchange(
            init_msg, keypair_b
        )
