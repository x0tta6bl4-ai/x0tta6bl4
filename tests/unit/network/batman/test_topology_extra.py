...
def test_topology_routing_and_prune():
    ...
    # Add nodes
    for nid in ['A','B','C','D']:
        topo.add_node(MeshNode(node_id=nid, mac_address=f'mac-{nid}', ip_address=f'10.0.0.{nid}', state=NodeState.ONLINE))
    # Add links (A-B, B-C, C-D) chain
    topo.add_link(MeshLink('A','B', LinkQuality.GOOD, throughput_mbps=100, latency_ms=10))
    topo.add_link(MeshLink('B','C', LinkQuality.GOOD, throughput_mbps=100, latency_ms=12))
    topo.add_link(MeshLink('C','D', LinkQuality.EXCELLENT, throughput_mbps=120, latency_ms=8))
...
def test_topology_get_neighbors_quality_filter():
    ...
    topo.add_link(MeshLink('A','B', LinkQuality.GOOD, throughput_mbps=50, latency_ms=15))
    topo.add_link(MeshLink('A','C', LinkQuality.POOR, throughput_mbps=10, latency_ms=40))
