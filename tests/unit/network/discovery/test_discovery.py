...
class TestDiscoveryMessage:
    ...
    def test_creation(self):
        ...
        msg = DiscoveryMessage(
            msg_type=MessageType.ANNOUNCE, sender_id="node-1", payload={"test": "data"}
        )
    ...
    def test_serialization(self):
        ...
        msg = DiscoveryMessage(
            msg_type=MessageType.QUERY, sender_id="sender", payload={"key": "value"}
        )
    ...
    def test_all_message_types(self):
        ...
        for msg_type in MessageType:
            msg = DiscoveryMessage(msg_type=msg_type, sender_id="test", payload={})
...
class TestKademliaNode:
    ...
    def test_add_peer(self):
        ...
        peer = PeerInfo(node_id="peer-1", addresses=[("10.0.0.1", 5000)])
    ...
    def test_find_closest(self):
        ...
        for i in range(30):
            peer = PeerInfo(node_id=f"peer-{i}", addresses=[(f"10.0.0.{i}", 5000)])
            node.add_peer(peer)
...
class TestMulticastDiscovery:
    ...
    def test_initialization(self):
        ...
        discovery = MulticastDiscovery(
            node_id="test-node", service_port=5000, services=["mesh"]
        )
    ...
    def test_get_local_ip(self):
        ...
        discovery = MulticastDiscovery(node_id="test", service_port=5000)
    ...
    def test_peers_initially_empty(self):
        ...
        discovery = MulticastDiscovery(node_id="alone", service_port=5000)
...
class TestMeshDiscovery:
    ...
    def test_initialization_minimal(self):
        ...
        discovery = MeshDiscovery(node_id="mesh-node", service_port=5000)
    ...
    def test_initialization_no_multicast(self):
        ...
        discovery = MeshDiscovery(
            node_id="no-mcast", service_port=5000, enable_multicast=False
        )
    ...
    def test_find_peers_for_service(self):
        ...
        discovery = MeshDiscovery(
            node_id="finder", service_port=5000, enable_multicast=False
        )
        ...
        discovery._peers["exit-1"] = PeerInfo(
            node_id="exit-1", addresses=[("10.0.0.2", 5000)], services=["mesh", "exit"]
        )
        discovery._peers["mesh-only"] = PeerInfo(
            node_id="mesh-only", addresses=[("10.0.0.3", 5000)], services=["mesh"]
        )
    ...
    def test_stats(self):
        """Статистика."""
        discovery = MeshDiscovery(
            node_id="stats-node", service_port=5000, enable_multicast=False
        )
...
async def test_multicast_start_stop():
    ...
    discovery = MulticastDiscovery(node_id="lifecycle-test", service_port=5000)

    try:
        await discovery.start()
    except (PermissionError, OSError) as exc:
        pytest.skip(f"multicast socket unavailable in test environment: {exc}")
...
async def test_peer_discovery_callback():
    ...
    discovery = MeshDiscovery(
        node_id="callback-test", service_port=5000, enable_multicast=False
    )
    ...
    test_peer = PeerInfo(node_id="new-peer", addresses=[("1.2.3.4", 5000)])
