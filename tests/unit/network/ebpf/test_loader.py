...
class TestEBPFLoader:
        ...
        with patch("src.network.ebpf.loader.ELF_TOOLS_AVAILABLE", False):
            ...
            try:
                ...
                program_id = loader.load_program(
                    Path(temp_path).name, EBPFProgramType.XDP
                )
    ...
    @patch("src.network.ebpf.loader.Path.exists")
    @patch("src.network.ebpf.loader.subprocess.run")
    def test_attach_to_interface_interface_not_found(
        ...
        with (
            ...
            def subprocess_side_effect(cmd, *args, **kwargs):
                ...
                if "ip link set" in cmd_str:
                    ...
                    return Mock(
                        returncode=0,
                        stdout="eth0: <BROADCAST,MULTICAST,UP,LOWER_UP> xdp generic",
                        stderr="",
                    )
                return Mock(returncode=0, stdout="", stderr="")
            ...
            success = loader.attach_to_interface(
                "test_prog", "eth0", EBPFAttachMode.SKB
            )
        ...
        with patch("src.network.ebpf.loader.subprocess.run") as mock_subprocess:
            ...
            def subprocess_side_effect(cmd, *args, **kwargs):
                ...
                if "ip link set" in cmd_str:
                    ...
                    return Mock(
                        returncode=0,
                        stdout="eth0: <BROADCAST,MULTICAST,UP,LOWER_UP>",
                        stderr="",
                    )
                return Mock(returncode=0, stdout="", stderr="")
