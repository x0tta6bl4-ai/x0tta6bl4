...
def orchestrator_setup(monkeypatch):
    ...
    monkeypatch.setattr(mod, "EBPFLoader", lambda *a, **kw: loader)
    monkeypatch.setattr(mod, "MeshNetworkProbes", lambda *a, **kw: probes)
    monkeypatch.setattr(mod, "EBPFMetricsExporter", lambda *a, **kw: metrics)
    monkeypatch.setattr(mod, "CiliumLikeIntegration", lambda *a, **kw: cilium)
    monkeypatch.setattr(mod, "DynamicFallbackController", lambda *a, **kw: fallback)
    monkeypatch.setattr(mod, "EBPFMAPEKIntegration", lambda *a, **kw: mapek)
    monkeypatch.setattr(mod, "RingBufferReader", lambda *a, **kw: ringbuf)
