...
class TestEBPFValidatorExtended:
    ...
    def test_nested_loop_detection(self, validator):
        ...
        for jump_from in [4, 6, 8]:
            ...
            bytecode[jump_from*8 + 2:jump_from*8 + 4] = offset.to_bytes(2, 'little', signed=True)
        
        result = validator.validate_bytecode(bytes(bytecode))
        
        # Should detect nested loops
        assert any("multiple jumps" in warning.lower() or "nested loop" in warning.lower() 
    ...
    def test_read_only_register_protection(self, validator):
        ...
        assert any("read-only" in warning.lower() or "r10" in warning.lower() 
                  for warning in result.warnings)
    ...
    def test_no_loops_detected(self, validator):
        ...
        for i in range(3):
            ...
            bytecode[i*8 + 2:i*8 + 4] = (8).to_bytes(2, 'little', signed=True)  # Forward
