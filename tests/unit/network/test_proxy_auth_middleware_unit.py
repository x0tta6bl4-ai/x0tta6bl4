...
from src.network.proxy_auth_middleware import (
    APIKeyStore,
    ClientIdentity,
    JWTAuthManager,
    Permission,
...
def test_client_identity_permission_helpers():
    ...
    assert identity.has_permission(Permission.PROXY_ADMIN) is False


def test_jwt_create_token_raises_when_secret_missing(monkeypatch):
    monkeypatch.delenv("PROXY_JWT_SECRET", raising=False)
    monkeypatch.setenv("ENVIRONMENT", "development")
    manager = JWTAuthManager(secret="")
    with pytest.raises(ValueError, match="JWT secret not configured"):
        manager.create_token("client-1", Role.VIEWER)


def test_jwt_validate_token_expired_and_invalid(caplog):
    manager = JWTAuthManager(secret="test-secret", expiry_hours=1)

    expired = manager.create_token("client-expired", Role.VIEWER, {"exp": time.time() - 1})
    with caplog.at_level("WARNING"):
        assert manager.validate_token(expired) is None
    assert "JWT token expired" in caplog.text

    with caplog.at_level("WARNING"):
        assert manager.validate_token("not-a-jwt-token") is None
...
def test_api_key_store_validate_missing_returns_none(monkeypatch):
    monkeypatch.delenv("PROXY_API_KEY_MISSING", raising=False)
    store = APIKeyStore()
    assert store.validate("does-not-exist") is None


def test_api_key_store_rotate_key_replaces_existing_name(monkeypatch):
    monkeypatch.setenv("PROXY_API_KEY_ROTATE", "old-secret:viewer")
    store = APIKeyStore()

    old_identity = store.validate("old-secret")
    assert old_identity is not None

    new_key = store.rotate_key("ROTATE")
    ...
    assert new_identity.api_key_id == "ROTATE"
...
@pytest.mark.asyncio
async def test_require_permission_decorator_requires_identity():
    middleware = ProxyAuthMiddleware(
        api_key_store=_APIKeyStore(),
        jwt_manager=_JWTManager(),
        rate_limiter=_PassRateLimiter(),
    ...
    response = await _handler(_FakeRequest())
    assert response.status == 401
    assert "Authentication required" in response.text


@pytest.mark.asyncio
async def test_require_permission_decorator_allows_with_permission():
    middleware = ProxyAuthMiddleware(
        api_key_store=_APIKeyStore(),
        jwt_manager=_JWTManager(),
        rate_limiter=_PassRateLimiter(),
...
@pytest.mark.asyncio
async def test_require_any_permission_decorator_requires_identity():
    middleware = ProxyAuthMiddleware(
        api_key_store=_APIKeyStore(),
        jwt_manager=_JWTManager(),
        rate_limiter=_PassRateLimiter(),
    ...
    @middleware.require_any_permission([Permission.PROXY_ADMIN])
    async def _handler(_request):
        return web.json_response({"ok": True})
...
@pytest.mark.asyncio
async def test_require_any_permission_decorator_denies_when_none_match():
    middleware = ProxyAuthMiddleware(
        api_key_store=_APIKeyStore(),
        jwt_manager=_JWTManager(),
        rate_limiter=_PassRateLimiter(),
...
def test_request_signer_rejects_missing_timestamp():
    signer = RequestSigner(secret_key="k")
    assert not signer.verify_signature(
        "deadbeef",
        "GET",
        "/x",
        headers={"X-Client": "no-ts"},
    )
...
def test_create_auth_middleware_factory_uses_explicit_config():
    middleware = create_auth_middleware(
        ...
        jwt_secret="factory-secret",
    )
    assert middleware.require_auth is False
    assert middleware.rate_limiter.requests_per_minute == 33
    assert middleware.jwt_manager.secret == "factory-secret"
