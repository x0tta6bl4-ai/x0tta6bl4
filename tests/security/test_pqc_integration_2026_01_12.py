...
class TestPQCLibOQSIntegration:
    ...
    async def test_kem_key_generation(self, pqc_backend):
        ...
        logger.info(
            f"✅ KEM keypair generated - public: {len(keypair.public_key)} bytes, private: {len(keypair.private_key)} bytes"
        )
    ...
    async def test_encapsulation_and_decapsulation(self, pqc_backend):
        ...
        logger.info(
            f"✅ KEM encapsulation/decapsulation successful - shared secret: {len(shared_secret_1)} bytes"
        )
    ...
    async def test_signing_and_verification(self, pqc_backend):
        ...
        is_valid_modified = pqc_backend.verify(
            modified_message, signature, keypair.public_key
        )
        assert not is_valid_modified, "Should reject signature for modified message"

        logger.info(
            f"✅ Digital signature test passed - signature: {len(signature)} bytes"
        )
...
class TestHybridEncryption:
    ...
    async def test_hybrid_keypair_generation(self, hybrid_encryption):
        ...
        logger.info(
            f"✅ Hybrid keypair generated - classical: {len(keypair.classical_public)}, PQ: {len(keypair.pq_public)}"
        )
    ...
    async def test_hybrid_encapsulation(self, hybrid_encryption):
        ...
        logger.info(
            f"✅ Hybrid encapsulation successful - ciphertext: {len(ciphertext)}, shared secret: {len(shared_secret)}"
        )
...
class TestPQCMeshIntegration:
    ...
    async def test_pq_mesh_key_exchange(self, pq_mesh_security):
        ...
        ciphertext_1to2, secret_1to2 = pq_mesh_security.hybrid_cipher.encapsulate(
            node2_keypair
        )

        # Node 2 encapsulates for Node 1
        ciphertext_2to1, secret_2to1 = pq_mesh_security.hybrid_cipher.encapsulate(
            node1_keypair
        )
    ...
    async def test_pq_secured_mesh_communication(self, pq_mesh_security):
        ...
        ciphertext, shared_secret = pq_mesh_security.hybrid_cipher.encapsulate(
            node2_keypair
        )
        ...
        ciphertext_msg = pq_mesh_security.hybrid_cipher.hybrid_encrypt(
            plaintext, shared_secret
        )

        # Node 2 receives and decrypts
        decrypted = pq_mesh_security.hybrid_cipher.hybrid_decrypt(
            ciphertext_msg, shared_secret
        )
...
class TestPQCMigrationPath:
    ...
    async def test_pqc_performance_acceptable(self, pqc_backend):
        ...
        logger.info(
            f"✅ PQC performance acceptable - keygen: {keygen_time*1000:.1f}ms, encap: {encap_time*1000:.1f}ms"
        )
