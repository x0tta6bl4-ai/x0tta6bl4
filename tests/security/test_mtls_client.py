...
from src.security.mtls_client import (
    MTLSConfig,
    CertificateInfo,
    MTLSClient,
    get_mtls_client,
...
class TestMTLSClient:
    ...
    async def test_client_initialization_no_certs(self, mock_config):
        ...
        with patch.object(client, '_load_spiffe_credentials', new_callable=AsyncMock) as mock_spiffe:
    ...
    async def test_client_context_manager(self):
        ...
        with patch.object(MTLSClient, '_initialize', new_callable=AsyncMock):
            with patch.object(MTLSClient, 'close', new_callable=AsyncMock) as mock_close:
    ...
    async def test_http_methods(self):
        ...
        mock_session.request.assert_called_with("POST", "https://test.local/api", json={"data": "test"})

        # Test PUT
        await client.put("https://test.local/api", json={"data": "update"})
        mock_session.request.assert_called_with("PUT", "https://test.local/api", json={"data": "update"})
...
class TestMTLSRequestContextManager:
    ...
    async def test_mtls_request_context_manager(self):
        ...
        with patch('src.security.mtls_client.get_mtls_client', new_callable=AsyncMock) as mock_get_client:
            mock_client = AsyncMock()
            mock_client.request = AsyncMock(return_value=mock_response)
            mock_get_client.return_value = mock_client

            async with mtls_request("https://test.local/api", method="POST", json={"data": "test"}) as response:
...
class TestCertificateRotation:
    ...
    async def test_rotation_loop_reloads_when_needed(self):
        ...
        client = MTLSClient(config=MTLSConfig(
        ...
        with patch.object(client, '_reload_certificates', new_callable=AsyncMock) as mock_reload:
            # Start rotation loop
