...
from src.network.proxy_metrics_collector import (MetricSeries, MetricType,
...
class TestMetricSeries:
    ...
    def test_get_latest(self):
        ...
        series = MetricSeries(
            name="test_metric", metric_type=MetricType.GAUGE, description="Test metric"
        )
...
class TestProxyMetricsCollector:
    ...
    async def test_register_metric(self, collector):
        ...
        collector.register_metric(
            "custom_metric", MetricType.GAUGE, "Custom test metric"
        )
    ...
    async def test_record_proxy_request_failure(self, collector):
        ...
        await collector.record_proxy_request(
            proxy_id="proxy-1", success=False, latency_ms=0.0
        )
    ...
    async def test_get_proxy_metrics(self, collector):
        ...
        for i in range(10):
            await collector.record_proxy_request(
                proxy_id="proxy-1", success=True, latency_ms=100.0 + i * 10
            )
    ...
    async def test_get_global_metrics(self, collector):
        ...
        for i in range(10):
            await collector.record_proxy_request(
                proxy_id=f"proxy-{i % 3}", success=True, latency_ms=100.0
            )
...
class TestPerformance:
    ...
    async def test_high_throughput_recording(self):
        ...
        tasks = [collector.record("load_test", float(i)) for i in range(10000)]
    ...
    async def test_concurrent_access(self):
        ...
        async def writer(proxy_id: str, count: int):
            for i in range(count):
                await collector.record("concurrent", float(i), {"proxy_id": proxy_id})

        # Run multiple concurrent writers
        await asyncio.gather(
            writer("proxy-1", 100), writer("proxy-2", 100), writer("proxy-3", 100)
        )
