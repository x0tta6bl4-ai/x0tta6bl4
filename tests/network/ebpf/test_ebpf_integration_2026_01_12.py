...
EBPF_PROGRAMS_DIR = (
    Path(__file__).parent.parent.parent / "network" / "ebpf" / "programs"
)
...
class TestEBPFCompilation:
    ...
    def test_compile_xdp_counter(self):
        ...
        try:
            ...
            logger.info(
                f"✅ xdp_counter.c compiled successfully ({obj_file.stat().st_size} bytes)"
            )
    ...
    def test_verify_object_format(self, ebpf_compiled_objects):
        ...
        for prog_name, obj_path in ebpf_compiled_objects.items():
            try:
                result = subprocess.run(
                    ["file", str(obj_path)], capture_output=True, text=True, timeout=5
                )
...
class TestEBPFWithMAKEPK:
    ...
    async def test_ebpf_metrics_collection(self):
        ...
        try:
            ...
            logger.info(
                f"✅ eBPF metrics exporter initialized: {len(metrics)} metric types"
            )
        except ImportError:
    ...
    async def test_ebpf_anomaly_detection(self):
        ...
        try:
            ...
            logger.info(
                f"✅ eBPF anomaly detector initialized: {len(anomalies)} anomalies detected"
            )
        except ImportError:
        ...
        try:
            ...
            try:
                try:
                    ...
                    logger.info(
                        f"✅ Proper error handling for invalid files: {type(e).__name__}"
                    )
            finally:
