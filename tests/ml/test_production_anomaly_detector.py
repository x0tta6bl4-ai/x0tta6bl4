...
from src.ml.production_anomaly_detector import (
    AdaptiveThresholdCalculator, AnomalySeverity, CorrelationAnalyzer,
    ProductionAnomalyDetector, SeasonalityDetector,
...
class TestCorrelationAnalyzer:
    def test_correlation_computation(self):
        ...
        for i in range(50):
            analyzer.update(
                {"metric1": float(i), "metric2": float(i * 2), "metric3": float(-i)}
            )
...
class TestProductionAnomalyDetector:
    ...
    def test_normal_metric_recording(self):
        detector = ProductionAnomalyDetector()
        for i in range(150):
            event = detector.record_metric(
                "api", "latency", 100.0 + np.random.normal(0, 5)
            )
            if event:
