...
class TestWCAGCompliance:
    ...
    def require_live_api(self, base_url):
        """Skip network checks when the live API is not running."""
        ...
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                ...
                if sock.connect_ex((host, port)) != 0:
                    pytest.skip(f"Live API is not available at {base_url}")
    ...
    async def client(self):
        """HTTP client isolated from host proxy environment."""
    ...
    async def test_api_supports_cors_headers(self, client, base_url):
        ...
        response = await client.options(
            f"{base_url}/health", headers={"Origin": "https://example.com"}
        )
    ...
    def test_json_structure_accessibility(self):
        ...
        sample_response = {"status": "ok", "version": "3.0.0", "components": {}}
