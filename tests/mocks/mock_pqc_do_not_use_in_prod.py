...
class SimplifiedNTRU:
    ...
    def _encrypt_message(self, message: bytes, public_key: bytes) -> bytes:
        ...
        encrypted = bytes(
            m ^ k ^ r for m, k, r in zip(message, extended_key, random_pad)
        )
    ...
    def _decrypt_message(self, ciphertext: bytes, private_key: bytes) -> bytes:
        ...
        decrypted = bytes(
            e ^ k ^ r for e, k, r in zip(encrypted, extended_key, random_pad)
        )
...
class HybridEncryption:
    ...
    def hybrid_encapsulate(
        ...
        classical_secret = hashlib.sha256(
            classical_random + classical_public_key
        ).digest()
        classical_ciphertext = self._classical_encrypt(
            classical_random, classical_public_key
        )
    ...
    def hybrid_decapsulate(
        ...
        classical_random = self._classical_decrypt(
            ciphertexts["classical"], classical_private_key
        )
...
class QuantumSafeKeyExchange:
    ...
    def respond_to_exchange(
        self, init_message: Dict, my_keypair: Dict
        ...
        shared_secret, ciphertexts = self.hybrid.hybrid_encapsulate(
            initiator_pq_pub, initiator_classical_pub
        )
    ...
    def complete_exchange(self, response_message: Dict) -> bytes:
        """
