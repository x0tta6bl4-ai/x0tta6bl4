...
with unittest.mock.patch("slowapi.Limiter") as MockLimiter:
    ...
    from fastapi import FastAPI, APIRouter # Import FastAPI and APIRouter
    from src.database import Base, get_db, User, Session as DB_Session # Import User and DB_Session models
    from src.api.users import router as users_router, users_db # Import users_router and users_db
...
async def client_fixture(db_session: Session, mocker):
    ...
    print(f"DEBUG: Type of client before yield: {type(client)}") # Debug print
    try:
        yield client
    finally:
...
async def test_get_current_user_exists(client: AsyncClient):
    ...
    await client.post(
        "/api/v1/users/register",
        json={"email": "current@example.com", "password": "securepassword", "full_name": "Current User"}
    )
...
async def test_get_user_stats_admin_success(client: AsyncClient):
    ...
    await client.post("/api/v1/users/register", json={"email": "user1@example.com", "password": "password"})
    await client.post("/api/v1/users/register", json={"email": "user2@example.com", "password": "password"})

    response = await client.get("/api/v1/users/stats", headers={"X-Admin-Token": "test_admin_token"})
    assert response.status_code == 200
...
async def test_get_user_stats_invalid_admin_token(client: AsyncClient):
    ...
    response = await client.get("/api/v1/users/stats", headers={"X-Admin-Token": "wrong_token"})
    assert response.status_code == 403
    assert "Admin access required" in response.json()["detail"]
