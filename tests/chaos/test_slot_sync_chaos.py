...
class SlotSyncChaosTester:
    ...
    def __init__(self, num_nodes: int = 50, slot_count: int = 10):
        ...
        for i in range(num_nodes):
            ...
            self.nodes[node_id] = MeshNode(
                node_id=node_id, slot_number=i % slot_count, neighbors=set()
            )
    ...
    async def run_chaos_test(
        ...
        """
        logger.info(
            f"Starting chaos test: {self.num_nodes} nodes, {duration}s duration"
        )
        ...
        test_passed = (
            slot_sync_success > 0.95
            and avg_recovery < 2.0
            and race_conditions < (duration * 0.05)
        )
    ...
    async def _simulate_network_partition(self):
        ...
        logger.debug(
            f"Network partitioned: {len(partition1)} + {len(partition2)} nodes"
        )
        ...
        for node in self.nodes.values():
            if node.state == NodeState.FAILED:
                ...
                if random.random() < 0.1:  # 10% recovery chance per tick
                    ...
                    logger.debug(
                        f"Node {node.node_id} recovered in {recovery_time:.3f}s"
                    )
    ...
    def generate_report(self) -> str:
        ...
        for result in self.test_results:
            ...
            report.append(
                f"    Slot Sync Success Rate: {result.slot_sync_success_rate:.2%}"
            )
            ...
            report.append("")
            report.append(
                f"  Result: {'✅ PASSED' if result.test_passed else '❌ FAILED'}"
            )
            report.append("-" * 60)
            report.append("")
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Chaos test slot-based synchronization"
    )
    ...
    parser.add_argument(
        "--failure-rate", type=float, default=0.1, help="Node failure rate per second"
    )
    parser.add_argument(
        "--partition-prob",
        type=float,
        default=0.05,
        help="Network partition probability",
    )
