...
class MTTRBenchmark:
    ...
    async def wait_for_recovery(
        self, max_wait: int = 60, check_interval: float = 0.5
    ) -> Optional[float]:
    ...
    async def run_mttr_suite(self, iterations: int = 5) -> Dict:
        ...
        mttr_stats = {
            "node_failure": {
                "mttr_seconds": (
                    statistics.mean(node_failure_times) if node_failure_times else None
                ),
            ...
            "link_failure": {
                "mttr_seconds": (
                    statistics.mean(link_failure_times) if link_failure_times else None
                ),
            ...
            "overall_mttr_seconds": (
                statistics.mean(node_failure_times + link_failure_times)
                if (node_failure_times or link_failure_times)
                else None
            ),
        ...
        if mttr_stats["node_failure"]["mttr_seconds"]:
            logger.info(
                f"Node Failure MTTR: {mttr_stats['node_failure']['mttr_seconds']:.2f}s"
            )
        if mttr_stats["link_failure"]["mttr_seconds"]:
            logger.info(
                f"Link Failure MTTR: {mttr_stats['link_failure']['mttr_seconds']:.2f}s"
            )
...
async def main():
    ...
    parser.add_argument(
        "--url", default="http://localhost:8080", help="Base URL for service"
    )
    parser.add_argument(
        "--output-dir",
