...
def benchmark_shaping(profile: TrafficProfile, iterations: int = 10000) -> Dict:
    ...
    for name, data in [("small_50b", test_data_small), 
        ...
        results[name] = {
            ...
            "overhead_percent": (int(statistics.mean(shaped_sizes)) - len(data)) / len(data) * 100,
        }
    
    return results


def benchmark_delay_generation(profile: TrafficProfile, iterations: int = 10000) -> Dict:
...
def benchmark_analyzer(iterations: int = 10000) -> Dict:
    ...
    return {
        ...
        "record_packet_p99_us": sorted(record_times)[int(len(record_times) * 0.99)] * 1_000_000,
...
def print_results(results: Dict):
    ...
    for size_key in ["small_50b", "medium_500b", "large_1400b"]:
        ...
        r = results[size_key]
        print(f"\n  {size_key}:")
        print(f"    Shape:   avg={r['shape_avg_ms']:.4f}ms, p99={r['shape_p99_ms']:.4f}ms")
        print(f"    Unshape: avg={r['unshape_avg_ms']:.4f}ms, p99={r['unshape_p99_ms']:.4f}ms")
        print(f"    Размер:  {r['original_size']}B -> {r['shaped_size']}B (+{r['padding_overhead']}B, {r['overhead_percent']:.1f}%)")
...
def main():
    ...
    for profile in TrafficProfile:
        ...
        print(f"  Диапазон: {delay_results['delay_min_ms']:.2f}ms - {delay_results['delay_max_ms']:.2f}ms")
    ...
    for profile in TrafficProfile:
        ...
        print(f"│ {profile.value:19} │ {r['shape_avg_ms']:10.4f} │ {r['overhead_percent']:10.1f} │ {delay['delay_avg_ms']:12.2f} │")
