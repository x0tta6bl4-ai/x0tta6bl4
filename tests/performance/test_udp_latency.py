...
from src.network.transport.udp_shaped import (
    ShapedUDPTransport,
    UDPPacket,
    PacketType
)
...
async def benchmark_packet_processing(iterations: int = 10000) -> Dict:
    ...
    for profile in profiles:
        ...
        results[profile] = {
            ...
            "total_avg_us": (statistics.mean(prepare_times) + statistics.mean(unpack_times)) * 1_000_000,
    ...
    for profile in profiles:
        ...
        if rtts:
            results[profile] = {
                ...
                "rtt_p99_ms": sorted(rtts)[int(len(rtts) * 0.99)] if len(rtts) > 100 else max(rtts),
...
async def benchmark_throughput(duration_seconds: float = 2.0) -> Dict:
    ...
    for profile in ["none", "gaming", "file_download"]:
        ...
        results[profile] = {
            ...
            "packet_loss_percent": (1 - received_packets / sent_packets) * 100 if sent_packets > 0 else 0,
        }
...
def print_processing_results(results: Dict):
    ...
    for profile, r in results.items():
        print(f"{profile:<20} {r['prepare_avg_us']:<15.2f} {r['unpack_avg_us']:<15.2f} {r['total_avg_us']:<15.2f} {r['packet_size']:<10}")
...
def print_rtt_results(results: Dict):
    ...
    for profile, r in results.items():
        if "error" in r:
            ...
            print(f"{profile:<15} {r['rtt_avg_ms']:<12.3f} {r['rtt_p50_ms']:<12.3f} {r['rtt_p99_ms']:<12.3f} {r['jitter_ms']:<12.3f} {r['packet_loss_percent']:<10.1f}%")
...
def print_throughput_results(results: Dict):
    ...
    for profile, r in results.items():
        print(f"{profile:<15} {r['throughput_pps']:<15.0f} {r['throughput_mbps']:<12.2f} {r['sent_packets']:<15} {r['received_packets']:<15} {r['packet_loss_percent']:<10.1f}%")
