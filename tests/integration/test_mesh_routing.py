...
@pytest.mark.skipif(
    not UDP_AVAILABLE, reason="UDP sockets unavailable in this environment"
)
@pytest.mark.asyncio
async def test_two_nodes_direct_communication():
    ...
    # Создаём два узла
    node_a = CompleteMeshNode(
        MeshConfig(node_id="alice", port=6001, enable_multicast=False)
    )
    node_b = CompleteMeshNode(
        MeshConfig(node_id="bob", port=6002, enable_multicast=False)
    )
...
async def test_three_nodes_multihop():
    ...
    node_a = CompleteMeshNode(
        MeshConfig(node_id="alice", port=6011, enable_multicast=False)
    )
    node_b = CompleteMeshNode(
        MeshConfig(node_id="bob", port=6012, enable_multicast=False)
    )
    node_c = CompleteMeshNode(
        MeshConfig(node_id="charlie", port=6013, enable_multicast=False)
    )
...
async def test_routing_stats():
    ...
    node = CompleteMeshNode(
        MeshConfig(node_id="stats-test", port=6020, enable_multicast=False)
    )
...
async def test_broadcast():
    ...
    node_a = CompleteMeshNode(
        MeshConfig(node_id="alice", port=6030, enable_multicast=False)
    )
    node_b = CompleteMeshNode(
        MeshConfig(node_id="bob", port=6031, enable_multicast=False)
    )
    node_c = CompleteMeshNode(
        MeshConfig(node_id="charlie", port=6032, enable_multicast=False)
    )
...
class TestRoutingPacket:
    ...
    def test_packet_serialization(self):
        ...
        from src.network.routing import PacketType, RoutingPacket
