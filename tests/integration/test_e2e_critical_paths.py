...
class E2ECriticalPathTests:
    ...
    def test_e2e_system_startup_and_initialization(self):
        ...
        print(
            f"  ✅ {active_count}/{total_count} components initialized ({active_count/total_count*100:.1f}%)"
        )
    ...
    def test_e2e_health_check_workflow(self):
        ...
        assert (
            success_count >= 45
        ), f"At least 45/50 concurrent checks should pass, got {success_count}"
    ...
    def test_e2e_error_handling_and_recovery(self):
        ...
        try:
            ...
            assert resp.status_code in [
                404,
                405,
            ], f"Should get 404/405, got {resp.status_code}"
        ...
        assert (
            recovery_rate >= 0.95
        ), f"Recovery rate should be >= 95%, got {recovery_rate*100:.1f}%"
    ...
    def test_e2e_performance_under_load(self):
        ...
        print(
            f"  Avg: {baseline_avg:.2f}ms, Min: {min(baseline_times):.2f}ms, Max: {max(baseline_times):.2f}ms"
        )
        ...
        if times:
            ...
            assert (
                concurrent_avg < 500
            ), f"Average response should be < 500ms, got {concurrent_avg:.2f}ms"
        ...
        if times:
            ...
            assert (
                successful >= 90
            ), "Should handle at least 90/100 requests under stress"
...
def test_e2e_critical_paths():
    ...
    healthy, info = tester.check_api_healthy()
    if not healthy:
        pytest.skip(
            f"requires local API at {tester.HEALTH_ENDPOINT} "
            f"(error={info.get('error', 'unavailable')})"
        )

    tests = [
        (
            "System Startup and Initialization",
            tester.test_e2e_system_startup_and_initialization,
        ),
