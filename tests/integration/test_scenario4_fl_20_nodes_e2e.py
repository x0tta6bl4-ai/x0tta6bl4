...
from src.federated_learning.consciousness_integration import \
    FLConsciousnessIntegration
...
class MockMeshRouter:
    ...
    def __init__(self, node_id: str):
        ...
        self._stats = {
            "peers": [
                {"latency": 0.05 + i * 0.01, "node_id": f"peer-{i}"} for i in range(3)
            ],
...
class MockMeshShield:
    ...
    def get_metrics(self) -> Dict:
        return {"quarantines": 0, "mttr_avg": 2.5, "failures_detected": 0}
...
async def test_register_20_nodes(fl_coordinator_20, mesh_nodes_20):
    ...
    integration = FLMeshIntegration(
        coordinator=fl_coordinator_20, mesh_nodes=mesh_nodes_20
    )
...
async def test_node_failure_recovery_20_nodes(fl_coordinator_20, mesh_nodes_20):
    ...
    assert result is not None
    assert result["participants"] >= fl_coordinator_20.config.min_participants - len(
        failed_nodes
    )
...
async def test_performance_20_nodes(fl_coordinator_20, mesh_nodes_20):
    ...
    assert collection_time < 12.0  # Includes deterministic sleep overhead in CI/sandbox
