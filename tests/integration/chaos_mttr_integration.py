...
from tests.chaos.test_slot_sync_chaos import SlotSyncChaosTester, ChaosTestResult
from tests.validation.mttr_validator import MTTRValidator, RecoveryType, MTTRValidationResult
...
class ChaosMTTRIntegration:
    ...
    def __init__(
        ...
        self.mttr_validator = MTTRValidator(
            target_mttr_p95=target_mttr_p95,
    ...
    async def run_integrated_test(
        ...
        combined_result = {
            ...
            "overall_passed": (
                chaos_result.test_passed and
                mttr_result.validation_passed
            )
        }
    ...
    def generate_combined_report(self, result: Dict) -> str:
        ...
        report.append(f"  Slot Sync Success Rate: {chaos['slot_sync_success_rate']:.1%}")
        ...
        report.append(f"  Validation Passed: {'✅ YES' if mttr['validation_passed'] else '❌ NO'}")
        report.append("")
        
        report.append("Overall Result:")
        report.append(f"  All Tests Passed: {'✅ YES' if result['overall_passed'] else '❌ NO'}")
        report.append("")
        report.append("=" * 60)
...
async def main():
    ...
    parser.add_argument("--failure-rate", type=float, default=0.1, help="Failure rate per second")
