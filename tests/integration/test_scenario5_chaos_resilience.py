...
from src.federated_learning.mesh_integration import (FLMeshIntegration,
                                                     MeshNodeInfo)
...
async def fl_coordinator_chaos():
    ...
    config = CoordinatorConfig(
        min_participants=3, target_participants=10, max_participants=20
    )
...
async def test_node_failure_recovery(
    chaos_engine, fl_coordinator_chaos, mesh_nodes_chaos
):
    """Test recovery from node failure."""
    integration = FLMeshIntegration(
        coordinator=fl_coordinator_chaos, mesh_nodes=mesh_nodes_chaos
    )
...
@pytest.mark.asyncio
async def test_byzantine_attack_detection(
    chaos_engine, fl_coordinator_chaos, mesh_nodes_chaos
):
...
async def test_high_load_resilience(
    ...
    for i in [2, 4, 6]:
        await chaos_engine.inject_high_load(
            f"node-{i:02d}", duration=5.0, load_percent=0.95
        )
...
@pytest.mark.asyncio
async def test_multiple_chaos_events(
    chaos_engine, fl_coordinator_chaos, mesh_nodes_chaos
):
