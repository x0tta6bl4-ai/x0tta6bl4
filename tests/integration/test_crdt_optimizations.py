...
try:
    from src.data_sync.crdt_optimizations import (CRDTDelta, CRDTSyncOptimizer,
                                                  get_crdt_optimizer)
    from src.data_sync.crdt_sync import Counter, LWWRegister, ORSet
...
@pytest.mark.skipif(
    not CRDT_OPTIMIZATIONS_AVAILABLE, reason="CRDT optimizations not available"
)
class TestCRDTOptimizationsIntegration:
    ...
    def test_delta_application(self, optimizer):
        ...
        )  # May fail if operation doesn't match
