...
class TesteBPFToolchain:
    ...
    def test_clang_has_bpf_target(self):
        ...
        try:
            result = subprocess.run(
                ["clang", "-target", "bpf", "-c", "-x", "c", "/dev/null", "-o", "/dev/null"],
                capture_output=True,
                text=True
            )
            # clang may complain about missing input, but should support bpf target
            assert "unsupported target" not in result.stderr.lower() or result.returncode == 0
        except FileNotFoundError:
            pytest.skip("clang not installed")
...
class TesteBPFCompilation:
    ...
    def ebpf_dir(self):
        ...
        dir_path = Path(__file__).parent.parent.parent / "src" / "network" / "ebpf" / "programs"
...
class TesteBPFObjectFormat:
    ...
    def test_compile_xdp_counter(self):
        ...
        ebpf_dir = Path(__file__).parent.parent.parent / "src" / "network" / "ebpf" / "programs"
        ...
        try:
            result = subprocess.run(
                ["clang", "-O2", "-target", "bpf", "-c", str(source), "-o", "/tmp/test_xdp_counter.o"],
                capture_output=True,
                text=True,
                cwd=str(ebpf_dir)
            )
            ...
            if result.returncode == 0:
                ...
                elif "file not found" in result.stderr.lower() or "asm/types.h" in result.stderr:
                    # Missing kernel headers - expected in non-CI environments
                    pytest.skip("Kernel headers not installed (expected in non-CI environments)")
                else:
...
class TesteBPFKernelCompatibility:
    ...
    def test_kernel_version_parsing(self):
        ...
        assert version_tuple >= minimum_version or \
               version_tuple[0] > minimum_version[0], \
            ...
            if config_path.exists():
                try:
                    if config_path.name == "config.gz":
                        # Compressed config
                        pytest.skip("BPF syscall check requires reading compressed config")
                    else:
...
class TesteBPFDocumentation:
    ...
    def test_readme_has_requirements(self):
        ...
        if readme.exists():
            ...
            assert "kernel" in content.lower(), "README should mention kernel requirements"
...
class TesteBPFIntegration:
    ...
    def test_makefile_help_target(self):
        ...
        try:
            ...
            if result.returncode == 0:
                assert "all" in result.stdout.lower(), "Help doesn't mention 'all' target"
                assert "clean" in result.stdout.lower(), "Help doesn't mention 'clean' target"
        except FileNotFoundError:
