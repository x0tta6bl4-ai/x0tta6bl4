    ...
    def test_graphsage_analyze_success(self):
        ...
        with patch("src.api.v3_endpoints.V3_AVAILABLE", True):
            with patch("src.api.v3_endpoints.get_v3_integration") as mock_get:
                ...
                mock_integration.analyze_with_graphsage = AsyncMock(
                ...
                request_data = {
                    ...
                    "node_topology": {"node-1": ["node-2"], "node-2": ["node-1"]},
                }
    ...
    def test_graphsage_analyze_no_topology(self):
        ...
        with patch("src.api.v3_endpoints.V3_AVAILABLE", True):
            with patch("src.api.v3_endpoints.get_v3_integration") as mock_get:
                ...
                request_data = {
                    "node_features": {"node-1": {"cpu": 0.5, "memory": 0.5}}
                }
    ...
    def test_graphsage_analyze_failure(self):
        ...
        with patch("src.api.v3_endpoints.V3_AVAILABLE", True):
            with patch("src.api.v3_endpoints.get_v3_integration") as mock_get:
                ...
                request_data = {"node_features": {"node-1": {"cpu": 0.5}}}
    ...
    def test_stego_encode_success(self):
        ...
        with patch("src.api.v3_endpoints.V3_AVAILABLE", True):
            with patch("src.api.v3_endpoints.get_v3_integration") as mock_get:
                ...
                mock_integration.encode_packet_stego.return_value = (
                    b"encoded_packet_data_with_steganography"
                )
                mock_get.return_value = mock_integration

                payload = base64.b64encode(b"secret message").decode()
                request_data = {"payload": payload, "protocol_mimic": "http"}
    ...
    def test_stego_encode_icmp_protocol(self):
        ...
        with patch("src.api.v3_endpoints.V3_AVAILABLE", True):
            with patch("src.api.v3_endpoints.get_v3_integration") as mock_get:
                ...
                request_data = {"payload": payload, "protocol_mimic": "icmp"}
    ...
    def test_stego_encode_dns_protocol(self):
        ...
        with patch("src.api.v3_endpoints.V3_AVAILABLE", True):
            with patch("src.api.v3_endpoints.get_v3_integration") as mock_get:
                ...
                request_data = {"payload": payload, "protocol_mimic": "dns"}
        ...
        with patch("src.api.v3_endpoints.V3_AVAILABLE", True):
            with patch("src.api.v3_endpoints.get_v3_integration") as mock_get:
                ...
                response = client.post(
                    "/api/v3/stego/decode", params={"encoded_packet": encoded_packet}
                )
                assert response.status_code == 200
                data = response.json()
...
class TestChaosEndpoints:
    ...
    def test_chaos_run_node_down(self):
        ...
        with patch("src.api.v3_endpoints.V3_AVAILABLE", True):
            with patch("src.api.v3_endpoints.get_v3_integration") as mock_get:
                mock_integration = Mock()
                mock_integration.run_chaos_test = AsyncMock(
                    return_value={
                        "scenario": "node_down",
                        "status": "completed",
                        "duration": 60.0,
    ...
    def test_chaos_run_link_failure(self):
        ...
        with patch("src.api.v3_endpoints.V3_AVAILABLE", True):
            with patch("src.api.v3_endpoints.get_v3_integration") as mock_get:
                ...
                request_data = {"scenario": "link_failure", "intensity": 0.5}
    ...
    def test_chaos_run_ddos(self):
        ...
        with patch("src.api.v3_endpoints.V3_AVAILABLE", True):
            with patch("src.api.v3_endpoints.get_v3_integration") as mock_get:
                ...
                request_data = {"scenario": "ddos", "intensity": 0.8, "duration": 30.0}
    ...
    def test_chaos_run_byzantine(self):
        ...
        with patch("src.api.v3_endpoints.V3_AVAILABLE", True):
            with patch("src.api.v3_endpoints.get_v3_integration") as mock_get:
                ...
                request_data = {"scenario": "byzantine", "intensity": 0.2}
    ...
    def test_chaos_run_failure(self):
        ...
        with patch("src.api.v3_endpoints.V3_AVAILABLE", True):
            with patch("src.api.v3_endpoints.get_v3_integration") as mock_get:
                ...
                request_data = {"scenario": "node_down", "intensity": 0.5}
    ...
    def test_chaos_default_values(self):
        ...
        with patch("src.api.v3_endpoints.V3_AVAILABLE", True):
            with patch("src.api.v3_endpoints.get_v3_integration") as mock_get:
                mock_integration = Mock()
                mock_integration.run_chaos_test = AsyncMock(
                    return_value={"scenario": "node_down", "status": "completed"}
                )
                mock_get.return_value = mock_integration

                # Only scenario is required
                request_data = {"scenario": "node_down"}
...
class TestAuditTrailEndpoints:
    ...
    def test_get_audit_records_filtered(self):
        ...
        with patch("src.api.v3_endpoints.V3_AVAILABLE", True):
            with patch("src.api.v3_endpoints.get_audit_trail") as mock_get:
                ...
                response = client.get(
                    "/api/v3/audit/records", params={"record_type": "security_event"}
                )
                assert response.status_code == 200
                data = response.json()
                assert data["total"] == 3
    ...
    def test_audit_trail_unavailable(self):
        ...
        with patch("src.api.v3_endpoints.V3_AVAILABLE", False):
            request_data = {"record_type": "test", "data": {}}
            response = client.post("/api/v3/audit/add", json=request_data)
            assert response.status_code == 503
