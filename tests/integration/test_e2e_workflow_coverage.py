...
class WorkflowExecutor:
    ...
    async def execute_workflow(
        self, workflow_name: str, context: Dict[str, Any] = None
    ) -> WorkflowContext:
            ...
            try:
                ...
                try:
                    result = await asyncio.wait_for(
                        step.handler(context or {}), timeout=step.timeout_seconds
                    )
    ...
    def get_execution_metrics(self, workflow_name: str) -> Dict[str, Any]:
        ...
        executions = [
            h for h in self.execution_history if h["workflow_name"] == workflow_name
        ]
...
class TestDataRetrievalWorkflow:
    ...
    async def test_retrieval_with_timeout(self):
        ...
        steps = [WorkflowStep("slow_fetch", slow_fetch, timeout_seconds=0.1)]
...
class TestDecisionExecutionWorkflow:
    ...
    async def test_analyze_and_decide_workflow(self):
        ...
        async def analyze_system(ctx):
            await asyncio.sleep(0.01)
            return {"health_score": 0.8, "anomalies": []}

        async def make_decision(ctx):
            await asyncio.sleep(0.01)
            return {"action": "scale_up", "confidence": 0.95}

        async def execute_action(ctx):
            await asyncio.sleep(0.01)
            return {"status": "completed", "resources_scaled": 2}
...
class TestPerformanceOptimizationWorkflow:
    ...
    async def test_performance_optimization_workflow(self):
        ...
        async def verify_improvement(ctx):
            await asyncio.sleep(0.01)
            return {"latency_ms": 50, "improvement_percent": 66.67}
...
class TestDataConsistencyWorkflow:
    ...
    async def test_data_sync_workflow(self):
        ...
        steps = [
            ...
            WorkflowStep(
                "check_replicas", check_replicas, dependencies=["read_primary"]
            ),
    ...
    async def test_consensus_workflow(self):
        ...
        steps = [
            ...
            WorkflowStep(
                "determine", determine_consensus, dependencies=["collect_votes"]
            ),
...
class TestWorkflowMetrics:
    ...
    async def test_workflow_execution_metrics(self):
        ...
        steps = [WorkflowStep("step1", simple_step), WorkflowStep("step2", simple_step)]
    ...
    async def test_workflow_failure_metrics(self):
        ...
        steps = [WorkflowStep("flaky_step", failing_step)]
