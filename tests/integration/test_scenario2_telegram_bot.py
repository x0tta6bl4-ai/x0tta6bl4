...
def _require_udp_bind_capability():
    """Skip in sandbox environments where UDP bind/send is not permitted."""
    sock = None
    try:
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        sock.bind(("127.0.0.1", 0))
    except OSError as exc:
        pytest.skip(f"UDP sockets unavailable in this environment: {exc}")
    finally:
        if sock is not None:
            sock.close()
...
async def test_launch_node_already_running(node_manager):
    ...
    assert (
        "уже запущен" in result2["error"].lower()
        or "already" in result2["error"].lower()
    )
...
async def test_get_network_status_node_not_launched(node_manager):
    ...
    assert (
        "не запущен" in status["error"].lower()
        or "not launched" in status["error"].lower()
    )
...
async def test_close_connection_all(node_manager):
    ...
    node_manager.connections["conn-3"] = {
        "user_id": 99999,
        "type": "test",
    }  # Другой пользователь
...
async def test_close_connection_no_connections(node_manager):
    ...
    assert (
        "нет активных" in result["error"].lower()
        or "no active" in result["error"].lower()
    )
