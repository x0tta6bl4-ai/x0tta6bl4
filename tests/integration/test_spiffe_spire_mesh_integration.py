...
try:
    from src.security.spiffe.controller.spiffe_controller import (
        AttestationStrategy, SPIFFEController)
...
try:
    from src.network.batman.topology import MeshNode, MeshTopology, NodeState

    MESH_AVAILABLE = True
except ImportError:
    MESH_AVAILABLE = False

SPIRE_BINARIES_AVAILABLE = bool(
    shutil.which("spire-agent") and shutil.which("spire-server")
)

if not SPIRE_BINARIES_AVAILABLE:
    pytestmark = pytest.mark.skip(
        reason="SPIRE binaries unavailable in this environment (spire-agent/spire-server)"
    )
...
@pytest.mark.skipif(
    not (SPIFFE_AVAILABLE and MESH_AVAILABLE),
...
@pytest.mark.skipif(
    not (SPIFFE_AVAILABLE and PQC_AVAILABLE),
...
class TestSPIFFEMapEKIntegration:
    ...
    def spiffe_mapek_loop(self):
        ...
        loop = SPIFFEMapEKLoop(
            trust_domain="x0tta6bl4.mesh", renewal_threshold=0.5, check_interval=60
        )
        return loop
    ...
    def test_mapek_identity_anomaly_detection(self, spiffe_mapek_loop):
        ...
        is_expired = datetime.utcnow() > expired_identity.get(
            "expires_at", datetime.utcnow() + timedelta(hours=1)
        )
...
@pytest.mark.skipif(
    not (SPIFFE_AVAILABLE and MESH_AVAILABLE and PQC_AVAILABLE),
...
class TestSPIFFEFullStackIntegration:
    ...
    def test_identity_recovery_after_node_failure(self, full_stack_scenario):
        ...
        # Verify recovery
        assert (
            topology.nodes["recoverable-node"].spiffe_id
            == "spiffe://x0tta6bl4.mesh/node/recoverable-renewed"
        )
    ...
    def test_trusted_peer_identification(self, full_stack_scenario):
        ...
        for node_id, node_info in topology.nodes.items():
            assert node_info.spiffe_id is not None
            assert (
                "trusted" in node_id or "secure" in node_id or "recoverable" in node_id
            )
