...
@pytest.mark.skipif(
    not SPIFFE_MTLS_AVAILABLE, reason="SPIFFE mTLS components not available"
)
class TestTLS13Enforcement:
    ...
    @pytest.mark.asyncio
    @patch(
        "src.security.spiffe.mtls.mtls_controller_production.tempfile.NamedTemporaryFile"
    )
    @patch(
        "src.security.spiffe.mtls.mtls_controller_production.ssl.create_default_context"
    )
    async def test_mtls_controller_setup_enforces_tls13(
        self, mock_ssl_context, mock_temp_file
    ):
    ...
    def test_tls_config_default_min_version(self):
        ...
        config = TLSConfig(cert_pem=b"cert", key_pem=b"key", ca_bundle=b"ca")
    ...
    def test_cipher_suite_strength(self):
        ...
        assert (
            "aNULL" not in context.get_ciphers()
...
-----END CERTIFICATE-----"""
        ...
        with patch("cryptography.x509.load_pem_x509_certificate") as mock_load:
            mock_cert = MagicMock()
            mock_san = MagicMock()
            mock_san.value.get_values_for_type.return_value = [
            ...
            result = await controller.verify_peer_spiffe_id(
                cert_pem, expected_spiffe_id="spiffe://x0tta6bl4.mesh/node/worker-1"
            )
    ...
    async def test_verify_peer_spiffe_id_mismatch(self, mock_ssl_context):
        ...
        with patch("cryptography.x509.load_pem_x509_certificate") as mock_load:
            ...
            result = await controller.verify_peer_spiffe_id(
                cert_pem, expected_spiffe_id="spiffe://x0tta6bl4.mesh/node/worker-2"
            )
    ...
    async def test_verify_peer_invalid_trust_domain(self, mock_ssl_context):
        ...
        with patch("cryptography.x509.load_pem_x509_certificate") as mock_load:
            ...
            mock_san.value.get_values_for_type.return_value = [
                "spiffe://untrusted.domain/node/worker-1"
            ]
            mock_cert.extensions.get_extension_for_oid.return_value = mock_san
            mock_load.return_value = mock_cert
...
class TestOCSPRevocationValidation:
    ...
    def test_certificate_validator_ocsp_enabled(self):
        ...
        validator = CertificateValidator(check_revocation=True, check_ocsp=True)
    ...
    def test_certificate_validator_crl_enabled(self):
        ...
        validator = CertificateValidator(check_revocation=True, check_crl=True)
    ...
    def test_certificate_chain_validation(self, mock_load):
        ...
        with patch.object(validator, "_validate_certificate_chain", return_value=True):
            is_valid, spiffe_id, error = validator.validate_certificate(
                b"cert_pem", trust_bundle=trust_bundle
            )
...
class TestMTLSHTTPClientIntegration:
    ...
    def test_spiffe_http_client_initialization(self):
        ...
        client = SPIFFEHttpClient(
            expected_peer_id="spiffe://x0tta6bl4.mesh/api", verify_peer=True
        )
...
class TestMTLSAutoRotation:
    ...
    async def test_auto_rotation_interval(self, mock_ssl_context):
        ...
        controller = MTLSControllerProduction(
            workload_api_client=mock_workload_api, rotation_interval=3600  # 1 hour
        )
    ...
    async def test_rotation_on_expiry(self, mock_ssl_context):
        ...
        mock_workload_api = AsyncMock()
        controller = MTLSControllerProduction(
            workload_api_client=mock_workload_api, rotation_interval=1800  # 30 minutes
        )
