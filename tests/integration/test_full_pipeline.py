...
class TestFullPipeline:
    ...
    def test_faketls_plus_voice_profile(self):
        ...
        for packet in voice_packets:
            ...
            assert len(shaped) >= 202, f"Размер должен быть минимум 202, got {len(shaped)}"
    ...
    def test_dao_governance_full_cycle(self):
        ...
        proposal = engine.create_proposal(
            ...
            actions=[{"type": "config_change", "key": "heartbeat_interval", "value": 60}]
        )
        
        # Голосуем от нескольких узлов
        voters = ["node-1", "node-2", "node-3", "node-4", "node-5"]
        votes = [VoteType.YES, VoteType.YES, VoteType.YES, VoteType.NO, VoteType.ABSTAIN]
...
class TestPerformanceIntegration:
    ...
    def test_throughput_benchmark(self):
        ...
        # Реалистичный порог для полного pipeline с crypto
        # XOR быстрый, но общий pipeline имеет overhead
        assert throughput_pps > 1000, f"Throughput слишком низкий: {throughput_pps} pps"
        assert throughput_mbps > 10, f"Throughput должен быть > 10 Mbps, got {throughput_mbps:.2f}"
