...
class DatabaseResilienceTests:
    ...
    def test_database_stop_and_recovery(self):
        ...
        subprocess.run(
            f"docker stop {self.DB_CONTAINER}", shell=True, capture_output=True
        )
        time.sleep(2)
        ...
        subprocess.run(
            f"docker start {self.DB_CONTAINER}", shell=True, capture_output=True
        )
    ...
    def test_database_persistence(self):
        ...
        subprocess.run(
            f"docker restart {self.DB_CONTAINER}", shell=True, capture_output=True
        )
...
def test_database_resilience_integration():
    ...
    if shutil.which("docker") is None:
        pytest.skip("requires docker CLI for DB stop/start resilience checks")

    db_running, db_state = tester.get_container_status(tester.DB_CONTAINER)
    if not db_running:
        pytest.skip(
            f"requires running DB container '{tester.DB_CONTAINER}' "
            f"(state='{db_state}')"
        )
