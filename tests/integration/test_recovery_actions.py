...
@pytest.mark.skipif(not RECOVERY_ACTIONS_AVAILABLE, reason="Recovery actions not available")
class TestRecoveryActionsIntegration:
    ...
    async def test_restart_service(self, executor):
        ...
        with patch('asyncio.create_subprocess_shell') as mock_subprocess:
            ...
            mock_process = Mock()
            mock_process.communicate = asyncio.coroutine(lambda: (b"deployment restarted", b""))
    ...
    async def test_scale_up(self, executor):
        ...
        with patch('asyncio.create_subprocess_shell') as mock_subprocess:
            ...
            result = await executor.scale_up("test-deployment", replicas=5, namespace="default")
