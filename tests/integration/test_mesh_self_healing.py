...
def mesh_running():
    ...
    subprocess.run(
        ["docker", "compose", "start", "node-a", "node-b", "node-c"], check=False
    )
...
def test_node_failure_recovery(mesh_running, wait_for_convergence):
    ...
    for node_id in ["node-a", "node-c"]:
        ...
        assert (
            peer_count == 1
        ), f"{node_id} should see 1 peer after node-b failure, got {peer_count}"
    ...
    for node_id, url in NODES.items():
        ...
        assert (
            peer_count == 2
        ), f"{node_id} expected 2 peers after recovery, got {peer_count}"
...
def test_network_partition(mesh_running, wait_for_convergence):
    ...
    for node_id in ["node-a", "node-b"]:
        ...
        assert (
            peer_count == 1
        ), f"{node_id} should see 1 peer after partition, got {peer_count}"
...
def test_self_healing_metrics(mesh_running):
    ...
    for node_id, base_url in NODES.items():
        ...
        assert (
            "mapek_cycle_duration_seconds" in metrics_text
