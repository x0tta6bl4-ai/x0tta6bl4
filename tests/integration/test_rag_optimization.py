...
from src.rag.semantic_cache import SemanticCache, CachedResult, CachedRAGPipeline
from src.rag.batch_retrieval import (
    BatchRetriever, 
    BatchQuery, 
    BatchResult,
    AdaptiveBatchRetriever
)
...
class TestBatchRetriever:
    ...
    def mock_pipeline(self):
        ...
        async def mock_retrieve(query, k=10, use_cache=False):
            ...
            return {
                'results': [{'id': f'result_{i}', 'text': f'Result {i}'} for i in range(k)],
...
class TestRAGIntegration:
    ...
    async def test_full_rag_pipeline_optimization(self):
        ...
        async def mock_retrieve(query, k=10, use_cache=False):
            ...
            return {
                'results': [{'id': f'result_{i}', 'score': 0.9 - i*0.1} for i in range(k)],
                'scores': [0.9 - i*0.1 for i in range(k)]
            }
        ...
        cached_pipeline = CachedRAGPipeline(
            mock_pipeline,
            cache_config={'max_size': 100, 'similarity_threshold': 0.85}
        )
        
        batch_retriever = BatchRetriever(cached_pipeline.rag_pipeline, max_workers=4)
        
        queries = [
            BatchQuery(id=f"q_{i}", query=f"Query {i % 5}")
            for i in range(20)
        ]
