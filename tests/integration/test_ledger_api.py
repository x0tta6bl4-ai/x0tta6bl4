...
pytestmark = pytest.mark.skip(
    "ledger API integration tests are unstable in this sandbox (TestClient deadlock)"
)
...
class TestLedgerAPI:
    ...
    def test_ledger_search_post(self):
        ...
        response = client.post(
            ...
            json={"query": "Какие метрики у нас хуже targets?", "top_k": 5},
        )
    ...
    def test_ledger_search_get(self):
        ...
        # Затем ищем
        response = client.get(
            "/api/v1/ledger/search", params={"q": "Какие метрики?", "top_k": 3}
        )
    ...
    def test_ledger_search_empty_query(self):
        ...
        response = client.post("/api/v1/ledger/search", json={"query": "", "top_k": 5})
    ...
    def test_ledger_search_multiple_queries(self):
        ...
        queries = ["Какие метрики?", "Какие компоненты готовы?", "Что изменилось?"]

        for query in queries:
            response = client.post(
                "/api/v1/ledger/search", json={"query": query, "top_k": 3}
            )
            assert response.status_code == 200
