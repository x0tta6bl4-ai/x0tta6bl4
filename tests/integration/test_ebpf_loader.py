...
class TestEBPFRealProgram:
    ...
    def test_load_real_program_if_exists(self, ebpf_loader):
        ...
        if not xdp_counter_path.exists():
            pytest.skip("xdp_counter.o not found. Compile it first: make -C src/network/ebpf/programs")
        ...
        try:
            ...
            pytest.skip(f"Failed to load eBPF program: {e}. May require root privileges or kernel support.")
    
    def test_attach_detach_cycle_if_program_exists(self, ebpf_loader_impl, test_interface):
        ...
        try:
            # Load program
            program_id = ebpf_loader_impl.load_program("xdp_counter.o", EBPFProgramType.XDP)
            ...
            pytest.skip(f"eBPF operation failed: {e}. May require root privileges or kernel support.")
