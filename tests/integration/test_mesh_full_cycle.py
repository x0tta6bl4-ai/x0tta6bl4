...
@pytest.mark.skipif(not MESH_COMPONENTS_AVAILABLE, reason="Mesh components not available")
class TestMeshFullCycle:
    ...
    async def test_node_discovery_and_connection(self):
        ...
        # Add peer
        node_manager.add_peer(peer_node["node_id"], peer_node["address"], peer_node["port"])
        
        # Verify peer is added
        assert peer_node["node_id"] in node_manager.peers
        assert node_manager.peers[peer_node["node_id"]]["address"] == peer_node["address"]
    ...
    async def test_self_healing_full_cycle(self):
        ...
        assert "High CPU" in result or "High Memory" in result or "Network Loss" in result
