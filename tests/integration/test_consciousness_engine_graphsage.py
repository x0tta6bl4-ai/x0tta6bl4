...
class MockGraphSAGEAnomalyDetectorInTest:
    def predict(self, node_id, node_features, neighbors):
        ...
        if latency > 100.0:
            return MagicMock(is_anomaly=True, anomaly_score=0.9, confidence=0.95, node_id=node_id, features=node_features, inference_time_ms=1.0)
        else:
            return MagicMock(is_anomaly=False, anomaly_score=0.518, confidence=0.99, node_id=node_id, features=node_features, inference_time_ms=1.0)
...
async def test_consciousness_engine_graphsage_anomaly_detection(mock_create_graphsage_detector_for_mapek_func):
    ...
    print(f"DEBUG: engine.graphsage_detector.predict type: {type(engine.graphsage_detector.predict)}")
    ...
    anomaly_pred = engine.graphsage_detector.predict(anomaly_metrics['node_id'], anomaly_metrics, [])
    print(f"DEBUG ANOMALY: anomaly_prediction.anomaly_score: {anomaly_pred.anomaly_score}")
    ...
    assert metrics_anomaly.state in [ConsciousnessState.CONTEMPLATIVE, ConsciousnessState.MYSTICAL]
