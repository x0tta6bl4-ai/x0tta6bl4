...
NODES = {
    "node-alpha": {
        "container": "mesh-node-alpha",
        "ip": "172.31.0.10",
        "port": 5000,
        "host_port": 5001,
    },
    "node-beta": {
        "container": "mesh-node-beta",
        "ip": "172.31.0.11",
        "port": 5000,
        "host_port": 5002,
    },
    "node-gamma": {
        "container": "mesh-node-gamma",
        "ip": "172.31.0.12",
        "port": 5000,
        "host_port": 5003,
    },
    "node-delta": {
        "container": "mesh-node-delta",
        "ip": "172.31.0.13",
        "port": 5000,
        "host_port": 5004,
    },
...
def mesh_running():
    ...
    if not os.path.exists(COMPOSE_FILE):
        pytest.skip(f"mesh compose file not found: {COMPOSE_FILE}")
    ...
    try:
        ...
        pytest.skip(f"socket operations not permitted in this environment: {e}")
        ...
        try:
            ...
            pytest.skip(f"unable to start mesh docker stack: {start_err}")
...
def test_container_status(mesh_running):
    ...
    result = subprocess.run(
        ["docker", "compose", "-f", COMPOSE_FILE, "ps"], capture_output=True, text=True
    )
    ...
    for node_id, config in NODES.items():
        assert (
            config["container"] in output
        ), f"{config['container']} not in container list"
        assert (
            "Up" in output.split(config["container"])[1].split("\n")[0]
...
def test_enhanced_metrics(mesh_running):
    ...
    for node_id, config in NODES.items():
        ...
        if (
            "ebpf" in node_id.lower() or node_id == "node-alpha"
        ):  # Alpha is likely to have eBPF
            assert (
                "x0tta6bl4_mesh_connected_peers" in output
            ), f"{node_id} missing eBPF mesh metrics"
            assert (
                "x0tta6bl4_node_info" in output
            ), f"{node_id} missing node info metric"
            assert (
                "x0tta6bl4_uptime_seconds" in output
            ), f"{node_id} missing uptime metric"

        # Check for health check metrics
        assert (
            "x0tta6bl4_health_check" in output
            ...
            try:
                ...
                result = subprocess.run(
                    [
                        "docker",
                        "exec",
                        source_config["container"],
                        "curl",
                        "-s",
                        f"http://{target_config['ip']}:5000/health",
                    ],
                    capture_output=True,
                    text=True,
                    timeout=10,
                ...
                output = result.stdout
                assert (
                    "status" in output and "ok" in output.lower()
                ...
                pytest.fail(
                    f"{source_node} -> {target_node} communication timed out (10 seconds)"
                )
            except Exception as e:
...
def test_container_connectivity(mesh_running):
    ...
    for source_node in all_node_ids:
        for target_node in all_node_ids:
            ...
            print(
                f"Testing {source_node} -> {target_config['container']} communication..."
            )
            ...
            try:
                ...
                result = subprocess.run(
                    [
                        ...
                        "curl",
                        "-s",
                        f"http://{target_config['container']}:5000/health",
                    ],
                    capture_output=True,
                    text=True,
                    timeout=5,
                ...
                print(
                    f"Warning: {source_node} -> {target_config['container']} failed: {e}"
                )
...
def test_port_mapping(mesh_running):
    ...
    for node_id, config in NODES.items():
        ...
        try:
            ...
            assert (
                "HTTP" in response and "200" in response
            ...
            pytest.fail(
                f"Port {config['host_port']} for {node_id} is not accessible: {e}"
            )
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "curl",
                    "-s",
                    "http://localhost:5000/mesh/status",
                ],
                capture_output=True,
                text=True,
                timeout=10,
