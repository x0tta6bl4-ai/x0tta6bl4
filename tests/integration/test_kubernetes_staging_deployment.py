...
if shutil.which("kubectl") is None:
    pytestmark = pytest.mark.skip("kubectl is not available in this environment")
else:
    try:
        _cluster_check = subprocess.run(
            ["kubectl", "cluster-info"], capture_output=True, text=True, timeout=10
        )
        _cluster_unreachable = _cluster_check.returncode != 0
    except Exception:
        _cluster_unreachable = True
    if _cluster_unreachable:
        pytestmark = pytest.mark.skip("kubernetes cluster is not reachable")
...
class TestKubernetesClusterReadiness:
    ...
    def test_required_namespaces_exist(self):
        ...
        required_namespaces = ["default", "kube-system", "kube-public"]

        for ns in required_namespaces:
            result = subprocess.run(
                ["kubectl", "get", "namespace", ns], capture_output=True, text=True
            )
    ...
    def test_crd_support(self):
        ...
        result = subprocess.run(
            ["kubectl", "api-versions"], capture_output=True, text=True
        )
        assert result.returncode == 0
...
"""
        ...
        result = subprocess.run(
            ["kubectl", "get", "svc", "-n", k8s_namespace, "x0tta6bl4-test-service"],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 0
        ...
        # Cleanup
        subprocess.run(
            ["kubectl", "delete", "-f", "/tmp/test-service.yaml"], capture_output=True
        )
...
"""
        ...
        # Cleanup
        subprocess.run(
            ["kubectl", "delete", "-f", "/tmp/test-pods.yaml"], capture_output=True
        )
...
"""
        ...
        # Cleanup
        subprocess.run(
            ["kubectl", "delete", "-f", "/tmp/test-cm.yaml"], capture_output=True
        )
...
"""
        ...
        # Cleanup
        subprocess.run(
            ["kubectl", "delete", "-f", "/tmp/test-secret.yaml"], capture_output=True
        )
...
"""
        ...
        # Cleanup
        subprocess.run(
            ["kubectl", "delete", "-f", "/tmp/test-netpolicy.yaml"], capture_output=True
        )
...
"""
        ...
        # Cleanup
        subprocess.run(
            ["kubectl", "delete", "-f", "/tmp/test-role.yaml"], capture_output=True
        )
...
"""
        ...
        # Cleanup
        subprocess.run(
            ["kubectl", "delete", "-f", "/tmp/test-hpa.yaml"], capture_output=True
        )
...
"""
        ...
        # Cleanup
        subprocess.run(
            ["kubectl", "delete", "-f", "/tmp/test-pdb.yaml"], capture_output=True
        )
...
"""
        ...
        # Cleanup
        subprocess.run(
            ["kubectl", "delete", "-f", "/tmp/test-quota.yaml"], capture_output=True
        )
...
"""
        ...
        # Cleanup
        subprocess.run(
            ["kubectl", "delete", "-f", "/tmp/test-ingress.yaml"], capture_output=True
        )
