    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "beacon_throughput": (
                    self.total_beacons / self.duration_seconds
                    if self.duration_seconds > 0
                    else 0
                ),
                "pqc_throughput": (
                    self.total_pqc_ops / self.duration_seconds
                    if self.duration_seconds > 0
                    else 0
                ),
            },
            ...
            "scaling_analysis": {
                "latency_per_thousand_nodes": (beacon_stats["p99"] / self.node_count)
                * 1000,
                "efficiency_percent": (
                    (100 / (1 + (beacon_stats["p99"] - beacon_stats["p50"])))
                    if beacon_stats["p50"] > 0
                    else 0
                ),
            },
        }
...
class TrafficPattern:
    ...
    def __init__(
        self,
        name: str,
        beacon_rate: float = 1.0,
        pqc_ops_rate: float = 100,
        identity_rotation_interval: int = 600,
    ):
        self.name = name
...
class FailurePattern:
    ...
    def network_partition(start_time: float) -> "FailurePattern":
        ...
        return FailurePattern(
            "network_partition", start_time, duration=60.0, intensity=0.5
        )
...
class PerformanceAnalyzer:
    ...
    def identify_bottleneck(result: LoadTestResult) -> str:
        ...
        latency_degradation = (
            beacon_stats["p99_ms"] - beacon_stats["p50_ms"]
        ) / beacon_stats["p50_ms"]
    ...
    def predict_scaling(results_by_count: Dict[int, LoadTestResult]) -> Dict[str, Any]:
        ...
        if len(node_counts) >= 2:
            ...
            numerator = sum(
                (node_counts[i] - x_mean) * (latencies[i] - y_mean)
                for i in range(len(node_counts))
            )
            denominator = sum(
                (node_counts[i] - x_mean) ** 2 for i in range(len(node_counts))
            )
            ...
            return {
                "slope": slope,
                "intercept": intercept,
                "r_squared": PerformanceAnalyzer._calculate_r_squared(
                    node_counts, latencies, slope, intercept
                ),
    ...
    def _calculate_r_squared(
        x_vals: List[float], y_vals: List[float], slope: float, intercept: float
    ) -> float:
        """Calculate R² for linear fit"""
        y_mean = statistics.mean(y_vals)
        ss_res = sum(
            (y_vals[i] - (slope * x_vals[i] + intercept)) ** 2
...
class DistributedLoadGenerator:
    ...
    def run_steady_state_test(
        self, pattern: Optional[TrafficPattern] = None
    ) -> LoadTestResult:
        ...
        while elapsed < self.duration_seconds:
            ...
            for i in range(beacon_count):
                ...
                if beacon:
                    ...
                    for j in range(
                        min(10, self.node_count)
                    ):  # Process at 10 random nodes
            ...
            if iteration % 60 == 0:
                ...
                logger.info(
                    f"Progress: {elapsed:.1f}s / {self.duration_seconds}s "
        ...
        logger.info(
            f"✅ Test complete: {self.result.total_beacons} beacons, "
    ...
    def run_failure_injection_test(
        ...
        logger.info(
            f"Starting failure injection test with {len(failure_patterns)} patterns"
        )
        self.spawn_mesh_nodes()
        ...
        for failure_pattern in failure_patterns:
            active_failures[failure_pattern.failure_type] = (
                failure_pattern.start_time + failure_pattern.duration
            )
                ...
                if (
                    ...
                    for i in range(affected_count):
                        if i < len(self.nodes):
                            ...
                            if i == 0:  # Log once
                                logger.info(
                                    f"Injected {failure_pattern.failure_type} at {elapsed:.1f}s"
                                )
                else:
                    # Recover from failure
                    if failure_pattern.start_time + failure_pattern.duration <= elapsed:
                        affected_count = int(
                            self.node_count * failure_pattern.intensity
                        )
                        for i in range(affected_count):
                            if i < len(self.nodes):
                                if (
                                    self.nodes[i].is_failed
                                    or self.nodes[i].is_byzantine
                                ):
                                    ...
                                    if i == 0:  # Log once
                                        logger.info(
                                            f"Recovered from {failure_pattern.failure_type} "
...
def main():
    ...
    logger.info(
        f"Feasible for 1000 nodes: {scaling_analysis.get('feasible_for_1000', False)}"
    )
    logger.info(
        f"Feasible for 2000 nodes: {scaling_analysis.get('feasible_for_2000', False)}"
    )
    ...
    with open(results_file, "w") as f:
        output = {k: v.generate_report() for k, v in all_results.items()}
