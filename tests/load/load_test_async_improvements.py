...
async def make_request(client: httpx.AsyncClient, endpoint: str) -> Dict[str, float]:
    ...
    try:
        ...
        return {"latency": latency, "status": 0, "success": False}
...
async def load_test_endpoint(endpoint: str) -> Dict[str, float]:
    ...
    tasks = [
        run_client(client_id, endpoint) for client_id in range(CONCURRENT_REQUESTS)
    ]
...
def print_metrics(metrics: Dict[str, float]):
    ...
    print(
        f"Successful:         {metrics['successful']} ({metrics['success_rate']:.2f}%)"
    )
...
async def main():
    ...
    if blocking_results["blocking_detected"]:
        print(
            f"  ⚠️ Event loop blocking detected: {blocking_results['max_blocking_time_ms']:.2f}ms"
        )
    else:
