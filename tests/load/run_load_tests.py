...
from tests.load.load_testing_framework import (LoadTestConfig, LoadTester,
                                               StressTestConfig, StressTester)
...
class Phase6LoadTestRunner:
    ...
    async def run_baseline_test(self):
        ...
        config = LoadTestConfig(
            duration_seconds=30, target_throughput=50, concurrent_tasks=5
        )
    ...
    async def run_throughput_test(self):
        ...
        async def autonomic_loop():
            await system.autonomic_loop_iteration(
                {"cpu": 0.5, "memory": 0.6, "latency_ms": 50},
                ["scale_up", "optimize", "restart"],
            )

        config = LoadTestConfig(
            duration_seconds=60, target_throughput=100, concurrent_tasks=10
        )
    ...
    async def run_high_concurrency_test(self):
        ...
        config = LoadTestConfig(
            duration_seconds=45, target_throughput=200, concurrent_tasks=50
        )
    ...
    async def run_stress_test(self):
        ...
        config = StressTestConfig(
            start_throughput=50, max_throughput=300, increment_per_minute=50
        )
        ...
        print(
            f"  Max Stable Throughput: {summary['max_stable_throughput']:.1f} ops/sec"
        )
    ...
    async def run_latency_consistency_test(self):
        ...
        for load_level in [50, 100, 150, 200]:
            ...
            print(
                f"    Mean: {stats['latency_stats']['mean_ms']:.2f}ms, "
    ...
    def generate_summary_report(self):
        ...
        if "latency_consistency" in self.results:
            ...
            summary["results"]["latency_consistency"] = self.results[
                "latency_consistency"
            ]
