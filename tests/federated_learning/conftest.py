...
def simple_weights() -> ModelWeights:
    ...
    return ModelWeights(
        layer_weights={"layer1": [1.0, 2.0, 3.0, 4.0], "layer2": [5.0, 6.0]},
        layer_biases={"layer1": [0.1, 0.2], "layer2": [0.3]},
    )
...
def random_weights_factory():
    ...
    def create_weights(
        num_layers: int = 2, layer_size: int = 10, seed: int = None
    ) -> ModelWeights:
        if seed is not None:
            random.seed(seed)
...
def sample_updates(model_update_factory) -> List[ModelUpdate]:
    ...
    return [
        model_update_factory(
            node_id=f"node-{i:03d}", round_number=1, num_samples=100 + i * 10, seed=i
        )
        for i in range(5)
    ]
...
def byzantine_updates(model_update_factory) -> List[ModelUpdate]:
    ...
    normal_updates = [
        model_update_factory(node_id=f"honest-{i}", round_number=1, seed=i)
        for i in range(4)
    ]
...
def coordinator(coordinator_config) -> FederatedCoordinator:
    ...
    coord = FederatedCoordinator(
        coordinator_id="test-coordinator", config=coordinator_config
    )
    yield coord
    # Cleanup
    coord.stop()
...
def initial_global_model(simple_weights) -> GlobalModel:
    ...
    return GlobalModel(
        version=0, round_number=0, weights=simple_weights, aggregation_method="initial"
    )
