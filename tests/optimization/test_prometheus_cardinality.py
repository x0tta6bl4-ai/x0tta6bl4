...
class TestLabelAggregator:
    ...
    def test_regex_aggregation(self):
        ...
        assert (
            agg.aggregate_label("metric", "pod_name", "pod_456_worker") == "pod_generic"
        )
...
class TestCardinalityTracker:
    ...
    def test_label_aggregation_reduces_cardinality(self):
        tracker = CardinalityTracker()
        tracker.aggregator.add_aggregation_rule(
            "metric", "node_id", "node_1", "region_a"
        )
        tracker.aggregator.add_aggregation_rule(
            "metric", "node_id", "node_2", "region_a"
        )
...
class TestIntegration:
    def test_complete_cardinality_control_workflow(self):
        ...
        for i in range(100):
            tracker.record_metric(
                "requests",
                {"endpoint": f"/api/v1/endpoint_{i}", "method": "GET", "status": "200"},
            )
    ...
    def test_multiple_metrics_with_different_cardinalities(self):
        ...
        for i in range(5):
            for j in range(20):
                optimizer.record_metric_sample(
                    "high_cardinality", {"label": f"value_{j}"}
                )

        report = optimizer.get_cardinality_report()
        assert report["total_unique_metrics"] == 11
        assert "high_cardinality" in [
            m["metric"] for m in report["top_10_high_cardinality"]
        ]
