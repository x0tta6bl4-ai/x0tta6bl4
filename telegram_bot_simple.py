...
YOOMONEY_POLL_INTERVAL_SECONDS = int(os.getenv("YOOMONEY_POLL_INTERVAL_SECONDS", "120"))
...
def build_payment_success_menu() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="Подключить", callback_data="config")
    builder.button(text="Мои устройства", callback_data="devices")
    builder.button(text="Главное меню", callback_data="menu")
    builder.adjust(1, 1, 1)
    return builder.as_markup()
...
def build_account_menu(user_id: int) -> InlineKeyboardMarkup:
    ...
    if state == "new":
        ...
        egress_mode = get_user_egress_mode(user_id)
        if egress_mode == "warp":
            builder.button(text="🚀 Режим: WARP", callback_data="egress:direct")
        else:
            builder.button(text="🌐 Режим: Прямой", callback_data="egress:warp")
        builder.button(text="Помощь", callback_data="repair")
    if is_admin(user_id):
        builder.button(text="Оператор", callback_data="admin")
    builder.button(text="Главное меню", callback_data="menu")
...
def _generate_reality_link_for_port(
    ...
    *,
    ...
    return (
        f"vless://{user_uuid}@{endpoint}:{port}"
        "?security=reality"
        "&encryption=none"
        f"&flow={profile['flow']}"
        f"&type={profile['type']}"
...
def generate_xhttp_link(user_uuid: str, label: str | None = None) -> str:
    ...
    return (
        f"vless://{user_uuid}@{PROFILE_VPN_SERVER}:8443"
        "?security=tls"
        "&encryption=none"
        f"&type={profile['type']}"
        f"&path={path}"
...
def resolve_user_preferred_transport(
    ...
    preferred_transport = (health_policy or {}).get("preferred_transport", "main")
...
def build_android_stealth_bundle(
    ...
    return {
        ...
        "routing": {
            ...
            "fallback_path": f"{NL_BETA_VPN_SERVER}:{NL_BETA_VPN_PORT}",
        },
        "entries": [
...
def render_subscription_text(user: dict) -> str:
    ...
    egress_mode = ((user.get("egress_mode") or "direct") if user else "direct").strip().lower()
    mode_text = "WARP" if egress_mode == "warp" else "Прямой"
    mode_note = (
        f"В подписке резерв {SECONDARY_VPN_PORT} идёт первым."
        if egress_mode == "warp"
        else f"В подписке сначала основной {VPN_PORT}, затем резерв {SECONDARY_VPN_PORT}."
    )
    return (
        f"{BOT_BRAND}\n\n"
...
async def handle_callback(callback: CallbackQuery) -> None:
    ...
    elif callback.data.startswith("egress:"):
        if not is_user_active(callback.from_user.id):
            await callback.answer("Только для активных подписчиков")
            return
        mode = callback.data.split(":", 1)[1]
        if mode in {"direct", "warp"}:
            set_user_egress_mode(callback.from_user.id, mode)
            log_activity(callback.from_user.id, f"egress_changed:{mode}")
            if mode == "warp":
                text = (
                    f"{BOT_BRAND}\n\n"
                    "✅ Режим WARP включен\n\n"
                    f"В подписке резерв {SECONDARY_VPN_PORT} теперь идёт первым. "
                    "Используй этот режим, если основной профиль режется оператором."
                )
            else:
                text = (
                    f"{BOT_BRAND}\n\n"
                    "✅ Режим Прямой включен\n\n"
                    f"В подписке основной {VPN_PORT} снова идёт первым. "
                    "Это режим по умолчанию, если основной маршрут работает стабильно."
                )
            await safe_edit(
                callback,
                text,
...
async def notify_new_device(bot, user_id: int, access: dict, parsed: dict, token: str) -> None:
    ...
    device_count = count_subscription_accesses(user_id)
    if device_count <= 1:
...
def _build_stub_profiles(messages: list[str]) -> str:
    ...
    for msg in messages:
        ...
        lines.append(f"vless://{stub_uuid}@0.0.0.0:1?security=&type=tcp#{fragment}")
