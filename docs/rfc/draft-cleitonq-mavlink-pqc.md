# CleitonQ: Post-Quantum Cryptography Extension for MAVLink v2 Protocol

    draft-x0tta6bl4-cleitonq-mavlink-pqc-00
...
This document specifies CleitonQ, a backward-compatible extension to the
...
vulnerable to pre-key compromise and lack forward secrecy.  CleitonQ
introduces three new message types — CLEITONQ_SESSION_INIT,
CLEITONQ_SIGNED_CMD, and CLEITONQ_CHUNK — encapsulating post-quantum
...
3.  Protocol Overview
4.  Message Types
    4.1  CLEITONQ_SESSION_INIT (MSG_ID 50001)
    4.2  CLEITONQ_SIGNED_CMD (MSG_ID 50002)
    4.3  CLEITONQ_CHUNK (MSG_ID 50000)
5.  CLEITONQ_CHUNK Fragmentation
...
orbital and deep-sea assets.  CleitonQ addresses these gaps by
...
CleitonQ operates in three phases during a MAVLink session:

   Phase 1 — Session Key Establishment
       The GCS generates an ML-KEM-1024 keypair and sends the public
       key encapsulated as a CLEITONQ_SESSION_INIT message.  The FCS
   ...
   Phase 2 — Ongoing Command Authentication
       ...
       signature is transmitted as a CLEITONQ_SIGNED_CMD message
   ...
   Phase 3 — Transparent Relay via Chunking
       ...
       MAVLink v2 payload limit of 280 bytes.  CLEITONQ_CHUNK
...
All CleitonQ messages use MAVLink v2 framing with the following
common header fields:

    magic:       0xFD (MAVLink v2 start byte)
    incompat:    0x00  (CleitonQ set compat, not incompat)
...
**Purpose:** Fragment a large CleitonQ message (SESSION_INIT or
...
Fragmentation is REQUIRED for any CleitonQ message whose payload
...
The replay protection model is built around two mechanisms:
   ...
   1.  **Seq ordering**: CLEITONQ_CHUNK frames carry a unique seq
...
discarded and a CLEITONQ_NACK (deferred to future specification) MAY
be emitted.
...
fragmented via CLEITONQ_CHUNK if the MAVLink frame cannot
...
-   **Signature size**: Variable, nominally 2429 bytes.
    MUST be fragmented via CLEITONQ_CHUNK.
...
A CleitonQ session proceeds as follows:

Step 1 — GCS generates an ML-KEM-1024 keypair (ek, dk).

Step 2 — GCS encapsulates: (ct, ss) = ML-KEM-1024.Encaps(ek).

Step 3 — GCS frames ct as CLEITONQ_SESSION_INIT (MSG_ID 50001),
    possibly fragmented via CLEITONQ_CHUNK (MSG_ID 50000) if ct
    exceeds MTU.

Step 4 — FCS receives all CLEITONQ_CHUNK frames, reassembles, and
    decapsulates: ss = ML-KEM-1024.Decaps(dk, ct).

Step 5 — Both sides derive three keys from ss via HKDF-SHA3-256
    [RFC5869]:

        K_cmd  = HKDF(ss, "cleitonq-cmd-key")   — for ML-DSA
        K_auth = HKDF(ss, "cleitonq-auth-key")  — for HMAC-SHA3-256
        K_enc  = HKDF(ss, "cleitonq-enc-key")   — reserved

Step 6 — FCS sends CLEITONQ_SESSION_ACK (provisional MSG_ID 50003,
...
MAV_CMD_COMPONENT_ARM_DISARM, MAV_CMD_NAV_WAYPOINT), it MUST:
   ...
   2.  Transmit a CLEITONQ_SIGNED_CMD (MSG_ID 50002) message that
       references the command by its MAVLink seq number and carries
       an ML-DSA-87 signature over (cmd_seq || session_id || nonce ||
       original_payload).

   3.  Optionally fragment the SIGNED_CMD via CLEITONQ_CHUNK.

The receiving FCS:

   1.  Buffers the command message upon reception.

   2.  Reassembles the SIGNED_CMD from CLEITONQ_CHUNK frames, if
       fragmented.
...
A malicious relay MAY strip appended authentication frames.  CleitonQ
mitigates this by using CLEITONQ_CHUNK fragmentation with
...
Each CLEITONQ_SIGNED_CMD contains a unique nonce and a session_id.
...
from the MAVLink reserved range (50000-65535):
    ...
    +-----------+---------------------+------------+
    | 50000     | CLEITONQ_CHUNK      | Section 4.3|
    | 50001     | CLEITONQ_SESSION_INIT | Section 4.1|
    | 50002     | CLEITONQ_SIGNED_CMD  | Section 4.2|
    +-----------+---------------------+------------+
