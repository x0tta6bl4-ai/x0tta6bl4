...
- Production lifecycle definition: `src/core/production_lifespan.py` exists, but current `src/core/app.py` does not bind it as the FastAPI lifespan.
...
- Warning-as-error API check on 2026-05-28: `PYTHONWARNINGS=error python3 -m pytest tests/unit/api --no-cov -q` passed 1491 tests.
- Focused map checks: component registry, namespace convergence, canonical import, evidence/runtime bridge, policy enforcement, event/control-plane, and lifecycle readiness each have dedicated drift-checks listed below.
...
- Runtime readiness flags: `/edge/health` and `/events/health` expose `startup_hook_completed`; route-only APIs expose backing-state fields such as `write_db_ready`, `stripe_config_ready`, `control_plane_ready`, `persistent_supply_chain_ready`, `analytics_runtime_ready`, `playbook_control_plane_ready`, `agent_mesh_runtime_ready`, `policy_runtime_ready`, `node_runtime_ready`, `telemetry_runtime_ready`, `vpn_runtime_ready`, `users_runtime_ready`, `swarm_runtime_ready`, `ledger_runtime_ready`, `swarm_orchestration_ready`, `vision_runtime_ready`, and `service_identity_runtime_ready`.
...
Production lifecycle definition (not app-bound by current src/core/app.py)
...
|---|---|---|
| API shell -> production intelligence runtime definition | `src/core/app.py:46-53`, `src/core/production_lifespan.py:102-182`, `docs/architecture/CURRENT_LIFECYCLE_READINESS_MAP.json` | Production intelligence startup/shutdown is defined, but current `src/core/app.py` constructs `FastAPI(...)` without `lifespan=production_lifespan`. Route import must not be promoted into proof that DB schema checks, mesh state, metrics, zero-trust, consciousness, PARL, FL, MAPE-K, edge, or event-sourcing startup hooks ran. |
| API shell -> route surface | `src/core/app.py:86-95`, `src/api/maas/endpoints/combined.py:61-170`, `docs/architecture/CURRENT_LIFECYCLE_READINESS_MAP.json` | Current MaaS routing is centered on `get_combined_router()`, with only `billing-v1`, `dashboard`, `edge-computing`, and `event-sourcing` still registered through direct `_include_maas_router(...)` calls. Router registration order remains behavior, but current evidence should use the lifecycle readiness map rather than the older individual-router include model. |
| Governance API -> local control plane | `src/api/maas/endpoints/governance.py:26-38`, `src/api/maas/endpoints/governance.py:243-362` | Governance execution uses EventBus, service identity, normalized policy evaluation, and SafeActuator before applying actions. |
...
| `src.libx0t` canonical imports -> absent top-level compatibility surface | `docs/architecture/CURRENT_CANONICAL_IMPORT_MAP.json`, `docs/architecture/CURRENT_NAMESPACE_CONVERGENCE_MAP.json`, `tests/unit/test_canonical_import_map_unit.py`, `src/libx0t/core/production_checks.py:34`, `src/security/pqc_identity.py:15`, `src/network/discovery/protocol.py:329` | Current `src/libx0t` imports its own `src.libx0t.*` modules when they exist, runtime PQC callers prefer `src.security.pqc`, and the top-level `libx0t` source tree is absent. Compatibility imports of `libx0t.*` depend on `src/libx0t` being on `sys.path`, not on a mirrored top-level source tree. |
...
| Billing webhook -> EventBus -> Ledger trace | `src/api/maas_billing.py:20`, `src/api/maas_billing.py:131-177`, `src/api/maas_billing.py:604-645`, `scripts/ops/smoke_ledger_event_trace_citation.py:354-403` | Route-only Stripe webhook handling now emits a local billing trace citation for DB/audit mutation and optional X0T bridge intent, with an explicit boundary that it does not prove live Stripe settlement or chain finality. |
...
| EventBus traces -> Ledger API -> RAG memory | `src/services/service_event_trace.py:159-209`, `src/ledger/rag_search.py:237-247`, `src/ledger/rag_search.py:364-460`, `src/api/ledger_endpoints.py:311-366`, `scripts/ops/smoke_ledger_event_trace_citation.py:138-826` | Redacted EventBus history can now be indexed as runtime evidence. Search citations can carry `event_id`, `event_type`, `source_agent`, registered `service_name`, `layer`, `entrypoint`, and `redacted=true`; the smoke script exercises the API route and search citation path across swarm consensus, commerce settlement, route-only marketplace API, route-only governance API, route-only billing webhook, DAO executor blocked-action, self-healing recovery-action, self-healing PQC identity, mesh network reward, share-to-earn network usage reward, MPTCP network control, SPIRE security identity, and PQC security service layers. |
...
| EventBus + service identity + actuator -> cross-layer control plane | `docs/architecture/CURRENT_EVENT_CONTROL_PLANE_MAP.json`, `src/coordination/events.py:18-69`, `src/services/service_event_identity.py:76-103`, `src/integration/spine.py:123-186` | 136 current `src` files touch the event/control surface. API, DAO, network, security, self-healing, server, swarm, commerce, reward code, billing webhook traces, trace filtering, and Ledger/RAG event-trace indexing share one event vocabulary and identity envelope. |
| Redacted service identity status -> event-producing services | `src/services/service_identity_registry.py:16-149`, `src/api/service_identity_status.py:21-33`, `src/core/app.py:250` | 89 current services that call `service_event_identity()` are now visible through `/api/v1/service-identity/status` without exposing SPIFFE IDs, DIDs, or wallet values. |
...
5. `EventBus` is the hidden control plane. `get_event_bus()` is used by governance, billing webhook traces, DAO, token bridge, MPTCP, SPIFFE, self-healing, ghost server, swarm, reward events, and marketplace events. The project already has a common event vocabulary, even where modules look separate. The current event/control-plane map tracks 136 `src` files, 32 `EventType` values, 89 services that resolve canonical event identity, and 193 trace-visible services/APIs.
...
7. Production lifecycle and API route inclusion are not the same thing. `src/core/app.py` includes `src.edge.api` and `src.event_sourcing.api` routers in all modes, but their startup/shutdown hooks are only called by `src/core/production_lifespan.py` when production lifespan is active.
   ...
   Current lifecycle map details: 27 router surfaces are tracked through the combined MaaS router plus direct legacy/v1 includes. `edge-computing` and `event-sourcing` are the only tracked routers with explicit `production_lifespan` startup/shutdown hooks, but current `src/core/app.py` does not bind `production_lifespan` as the FastAPI lifespan, so those hooks are defined rather than proven by app import. Their health payloads expose `startup_hook_completed` so operators can separate route reachability from startup completion. Route-only marketplace, billing, governance, supply-chain, analytics, playbooks, policies, nodes, telemetry, and VPN APIs expose runtime readiness fields so operators can separate route availability from database-backed writes, Stripe configuration, local control-plane dispatch readiness, durable SBOM/attestation evidence readiness, analytics DB/Redis telemetry readiness, signed command dispatch readiness, policy route-precedence/DB readiness, node route-precedence/telemetry/healing readiness, telemetry route-precedence/Redis/settlement-uptime readiness, and VPN x-ui/config/ZKP/production-env readiness.

8. The old parallel package-surface picture is gone in the current worktree. Current namespace map shows 0 top-level `libx0t` Python files, 186 `src/libx0t` Python files, 0 common paths, 0 byte-identical common paths, and 186 `src/libx0t`-only Python paths. The repo import scan currently sees 48 direct `libx0t` imports and 113 direct `src.libx0t` imports. `src/libx0t` is the current compatibility source surface; direct top-level `libx0t.*` runtime imports are not allowed while the top-level source tree is absent.

   Concrete import hole found and fixed: `src.libx0t.core.production_lifespan` now imports `src.libx0t.security.zero_trust`. Top-level `libx0t/security` source files are absent, so compatibility import tests must resolve through `src/libx0t` on `sys.path` instead of deleted top-level shims.
   ...
   Additional import hole fixed: `src/libx0t/core/production_checks.py` no longer imports missing `libx0t.security.post_quantum_liboqs`; it uses `src.security.pqc.LIBOQS_AVAILABLE`. `src/security/pqc_identity.py` and `src/network/discovery/protocol.py` now prefer the canonical `src.security.pqc` module instead of deprecated `src.libx0t.security.post_quantum` or top-level `libx0t.security.post_quantum`.
...
15. Service identity is now inspectable without leaking identity values. `src/services/service_identity_registry.py` enumerates all 89 current `service_event_identity()` callers, and `/api/v1/service-identity/status` reports only configured/source/env-var metadata, not the SPIFFE/DID/wallet values themselves.
...
| Namespace compatibility drift | Namespace map shows the top-level `libx0t` source tree is absent: 0 top-level Python files, 186 `src/libx0t` Python files, 0 common paths, and 186 `src/libx0t`-only paths. Canonical import map tracks 0 direct top-level imports inside `src/libx0t`; repo-wide direct import counts are 44 `libx0t` and 101 `src.libx0t` statements. | Treat `src/libx0t` as the compatibility source surface, keep direct runtime top-level `libx0t.*` imports out of `src`, and keep the namespace/canonical import drift checks aligned when public compatibility tests change. |
| Runtime route without app-bound lifecycle | Lifecycle map tracks 27 router surfaces through the combined router and direct legacy/v1 includes. `edge-computing` and `event-sourcing` have explicit production hooks, but current `src/core/app.py` does not bind `production_lifespan`; their health payloads report `startup_hook_completed`. `maas-marketplace` remains route-import-only and reports `write_db_ready` at `/api/v1/maas/marketplace/status`. `maas-billing` remains route-import-only and reports `stripe_config_ready` at `/api/v1/maas/billing/readiness`. `maas-governance` remains route-import-only and reports `control_plane_ready` at `/api/v1/maas/governance/readiness`. `maas-supply-chain` remains route-import-only and reports `persistent_supply_chain_ready` at `/api/v1/maas/supply-chain/readiness`. `maas-analytics` remains route-import-only and reports `analytics_runtime_ready` at `/api/v1/maas/analytics/readiness`. `maas-playbooks` remains route-import-only and reports `playbook_control_plane_ready` at `/api/v1/maas/playbooks/readiness`. `maas-agent-mesh` remains route-import-only, always registered, and reports `agent_mesh_runtime_ready` plus local health bot, auth dependency, status payload, history, and non-dry-run execution guard readiness at `/api/v1/maas/agents/health/status`. `maas-policies` remains route-import-only and reports `policy_runtime_ready` plus the legacy GET/POST shadowing boundary at `/api/v1/maas/policies/readiness`. `maas-nodes` remains route-import-only and reports `node_runtime_ready` plus legacy node-route shadowing, telemetry bridge, token signing, audit-log, and healing-service readiness at `/api/v1/maas/nodes/readiness`. `maas-telemetry` remains route-import-only and reports `telemetry_runtime_ready` plus legacy heartbeat/topology shadowing, Redis persistence, fallback cache, reputation, metrics, and settlement uptime readiness at `/api/v1/maas/telemetry/readiness`. `vpn` remains route-import-only and reports `vpn_runtime_ready` plus fixed-prefix route precedence, lazy x-ui, local User DB, config generator, cache, MaaS auth, legacy admin-token, ZKP legacy DB, and production VPN env readiness at `/vpn/readiness`. `users` remains route-import-only and reports `users_runtime_ready` plus fixed-prefix route precedence, SQLAlchemy User/Session model readiness, DB session capability, bcrypt, token helpers, slowapi limiter, admin-token, request identity propagation, and the test-only users_db boundary at `/api/v1/users/readiness`. `swarm-api` remains route-import-only and reports `swarm_runtime_ready` plus fixed-prefix route precedence, in-memory registry/lock readiness, admin-token, slowapi limiter, lazy orchestrator/task imports, vision-engine import readiness, and active swarm/agent counts at `/api/v3/swarm/health`. `ledger-api` remains route-import-only and reports `ledger_runtime_ready` plus fixed-prefix route precedence, RAG search/index/query surface readiness, CONTINUITY file readiness, verification evidence readiness, EventBus trace indexing readiness, and trace dependency readiness at `/api/v1/ledger/status`. `swarm-orchestration` remains route-import-only and reports `swarm_orchestration_ready` plus fixed-prefix route precedence, import-time orchestrator readiness, in-memory status/agents/tasks/metrics readiness, asyncio scheduler readiness, and the accepted-task-vs-live-agent boundary at `/api/v1/swarm/readiness`. `vision-analytics` remains route-import-only and reports `vision_runtime_ready` plus import-time vision component readiness, local image backend readiness, upload surface readiness, app rate-limit boundary, and the no-image-processing readiness boundary at `/api/v1/vision/readiness`. `service-identity-status` remains route-import-only, always registered, and reports `service_identity_runtime_ready` plus registry, trace filter/history, EventBus, EventType, and redacted-payload readiness at `/api/v1/service-identity/status`. | Keep the route-only readiness map under focused checks, then broaden to full-suite warning cleanup. |
...
| Background service identity drift | 89 services call `service_event_identity()` and are now registered in `src/services/service_identity_registry.py`. | Keep the registry drift-check aligned with new service identity callers, and extend the status payload only with redacted metadata. |
| Event/control-plane drift | Event map tracks 136 `src` files touching EventBus, service identity, event helpers, SafeActuator, AsyncSafeActuator, identity status, service trace filters, billing webhook traces, or EventBus-to-Ledger indexing. | Keep `tests/unit/test_event_control_plane_map_unit.py` as the guard when adding new event publishers, action executors, trace filters, or trace-memory bridges. |
...
3. Lifecycle readiness map:
   ...
   - Done: drift-check verifies all 27 tracked router surfaces are covered, separates combined-router registration from direct legacy/v1 includes, and records that `production_lifespan` hooks are defined but not app-bound by current `src/core/app.py`.
   ...
   - Done: `/api/v1/maas/agents/health/status` exposes `agent_mesh_runtime_ready`, `health_bot_surface_ready`, `health_bot_config_ready`, `auth_dependency_ready`, `status_payload_ready`, `non_dry_run_guard_ready`, and local-only/execution-token boundary metadata for always-registered MaaS agent mesh routes.
   - Done: `/api/v1/maas/policies/readiness` exposes `policy_runtime_ready`, `policy_db_ready`, `acl_policy_model_ready`, `rbac_dependency_ready`, and the legacy GET/POST shadowing boundary for DB-backed ACL policy routes.
   - Done: `/api/v1/maas/nodes/readiness` exposes `node_runtime_ready`, `node_db_ready`, `node_model_ready`, `node_rbac_ready`, `token_signer_ready`, `audit_log_ready`, `telemetry_bridge_ready`, `healing_service_ready`, and the legacy node-route shadowing boundary for DB-backed node runtime routes.
   - Done: `/api/v1/maas/telemetry/readiness` exposes `telemetry_runtime_ready`, `telemetry_db_ready`, `redis_persistence_ready`, `fallback_cache_ready`, `uptime_tracker_ready`, `settlement_uptime_ready`, `reputation_system_ready`, `metrics_export_ready`, and the legacy heartbeat/topology shadowing boundary for telemetry helper routes.
   - Done: `/vpn/readiness` exposes `vpn_runtime_ready`, `vpn_db_ready`, `config_generators_ready`, `xui_client_factory_ready`, `cache_ready`, `auth_dependency_ready`, `legacy_admin_token_ready`, `zkp_legacy_db_ready`, `zkp_attestor_ready`, `production_env_ready`, and fixed-prefix route-precedence metadata for VPN routes.
   - Done: `/api/v1/users/readiness` exposes `users_runtime_ready`, `users_db_ready`, `user_model_ready`, `session_model_ready`, `password_hashing_ready`, `token_generation_ready`, `rate_limiter_ready`, `admin_token_ready`, and fixed-prefix/test-store boundary metadata for user auth/profile/admin routes.
   - Done: `/api/v3/swarm/health` exposes `swarm_runtime_ready`, `registry_ready`, `admin_token_ready`, `rate_limiter_ready`, `orchestrator_ready`, `task_model_ready`, `vision_engine_ready`, active swarm/agent counts, and fixed-prefix/in-memory runtime boundary metadata for swarm routes.
   - Done: `/api/v1/ledger/status` exposes `ledger_runtime_ready`, `rag_surface_ready`, `continuity_file_ready`, `verification_evidence_ready`, `event_trace_index_ready`, `event_trace_dependencies_ready`, and fixed-prefix/local RAG boundary metadata for ledger routes.
   - Done: `/api/v1/swarm/readiness` exposes `swarm_orchestration_ready`, `swarm_components_ready`, `orchestrator_surface_ready`, `orchestrator_state_ready`, `task_scheduler_ready`, `agents_ready`, active agent/task counts, and the accepted-task-vs-live-agent boundary for legacy swarm orchestration routes.
   - Done: `/api/v1/vision/readiness` exposes `vision_runtime_ready`, `vision_components_ready`, `processor_surface_ready`, `topology_surface_ready`, `correction_surface_ready`, `upload_surface_ready`, `local_image_backend_ready`, external-provider metadata, and the no-image-processing readiness boundary for vision analytics routes.
   - Done: `/api/v1/service-identity/status` exposes `service_identity_runtime_ready`, registry, trace filter/history, EventBus, EventType, redacted-payload readiness, and the no-live-identity-proof boundary for always-registered service identity routes.
   - Next: keep expanding warning-as-error coverage from focused route checks toward broader API/runtime suites.

4. Namespace convergence:
   - Done: namespace convergence map captures file counts, import counts, bridges, and hotspots.
   - Done: top-level `libx0t` source files are absent in the current worktree; public compatibility imports resolve through `src/libx0t` when `src` is on `sys.path`.
   ...
   - Done: `src/libx0t` runtime network imports use `src.libx0t.network.*`, and selected Byzantine/eBPF compatibility bridge modules delegate to canonical `src.network.*` modules instead of deleted top-level files.
...
6. Event/control plane:
   ...
   - Done: drift-check verifies source refs, current 32-value EventType vocabulary, and current 136-file event surface coverage.
   - Done: redacted `/api/v1/service-identity/status` reports configured identity source metadata for all 89 current service identity callers without exposing identity values.
   ...
   - Done: route-only `maas-marketplace` events are now trace-registered separately from background `service_event_identity()` services and covered by the Ledger event-trace smoke path.
   - Done: route-only `maas-governance` now reaches the Ledger event-trace smoke through its real `_execute_action()` path, including policy evaluation, SafeActuator, redacted service identity fields, and a `pipeline.stage_end` citation.
   - Done: route-only `maas-billing` now reaches the Ledger event-trace smoke through its real `stripe_webhook()` path, including local Stripe webhook handling, DB/audit state, optional X0T bridge intent, and an explicit no-live-settlement claim boundary.
   - Next: extend trace-memory smoke coverage to another route-only module with external backing state, such as supply-chain attestations or telemetry-backed settlement readiness.
