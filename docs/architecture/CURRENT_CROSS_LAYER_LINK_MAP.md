...
- Warning-as-error map check on 2026-05-28: component registry, namespace convergence, canonical import, evidence/runtime bridge, policy enforcement, event/control-plane, and lifecycle readiness drift-checks passed 58 tests.
...
3. Lifecycle readiness map:
   ...
   - Done: `/api/v1/users/readiness` exposes `users_runtime_ready`, `users_db_ready`, `user_model_ready`, `session_model_ready`, `password_hashing_ready`, `token_generation_ready`, `rate_limiter_ready`, `admin_token_ready`, and fixed-prefix/test-store boundary metadata for full-mode-only user auth/profile/admin routes.
   - Done: `/api/v3/swarm/health` exposes `swarm_runtime_ready`, `registry_ready`, `admin_token_ready`, `rate_limiter_ready`, `orchestrator_ready`, `task_model_ready`, `vision_engine_ready`, active swarm/agent counts, and fixed-prefix/in-memory runtime boundary metadata for full-mode-only swarm routes.
   - Done: `/api/v1/ledger/status` exposes `ledger_runtime_ready`, `rag_surface_ready`, `continuity_file_ready`, `verification_evidence_ready`, `event_trace_index_ready`, `event_trace_dependencies_ready`, and fixed-prefix/local RAG boundary metadata for full-mode-only ledger routes.
   - Done: `/api/v1/swarm/readiness` exposes `swarm_orchestration_ready`, `swarm_components_ready`, `orchestrator_surface_ready`, `orchestrator_state_ready`, `task_scheduler_ready`, `agents_ready`, active agent/task counts, and the accepted-task-vs-live-agent boundary for full-mode-only legacy swarm orchestration routes.
   - Done: `/api/v1/vision/readiness` exposes `vision_runtime_ready`, `vision_components_ready`, `processor_surface_ready`, `topology_surface_ready`, `correction_surface_ready`, `upload_surface_ready`, `local_image_backend_ready`, external-provider metadata, and the no-image-processing readiness boundary for full-mode-only vision analytics routes.
...
6. Event/control plane:
   ...
   - Done: drift-check verifies source refs, current 31-value EventType vocabulary, and current 40-file event surface coverage.
