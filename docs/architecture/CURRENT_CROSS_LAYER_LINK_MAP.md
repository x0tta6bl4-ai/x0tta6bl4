...
| EventBus + service identity + actuator -> cross-layer control plane | `docs/architecture/CURRENT_EVENT_CONTROL_PLANE_MAP.json`, `src/coordination/events.py:18-69`, `src/services/service_event_identity.py:76-103`, `src/integration/spine.py:123-186` | 137 current `src` files touch the event/control surface. API, DAO, network, security, self-healing, server, swarm, commerce, reward code, billing webhook traces, trace filtering, and Ledger/RAG event-trace indexing share one event vocabulary and identity envelope. |
| Redacted service identity status -> event-producing services | `src/services/service_identity_registry.py:16-149`, `src/api/service_identity_status.py:21-33`, `src/core/app.py:250` | 88 current services that call `service_event_identity()` are now visible through `/api/v1/service-identity/status` without exposing SPIFFE IDs, DIDs, or wallet values. |
...
8. The old parallel package-surface picture is gone in the current worktree. Current namespace map shows 0 top-level `libx0t` Python files, 186 `src/libx0t` Python files, 0 common paths, 0 byte-identical common paths, and 186 `src/libx0t`-only Python paths. The repo import scan currently sees 44 direct `libx0t` imports and 101 direct `src.libx0t` imports. `src/libx0t` is the current compatibility source surface; direct top-level `libx0t.*` runtime imports are not allowed while the top-level source tree is absent.
...
15. Service identity is now inspectable without leaking identity values. `src/services/service_identity_registry.py` enumerates all 88 current `service_event_identity()` callers, and `/api/v1/service-identity/status` reports only configured/source/env-var metadata, not the SPIFFE/DID/wallet values themselves.
...
| Background service identity drift | 88 services call `service_event_identity()` and are now registered in `src/services/service_identity_registry.py`. | Keep the registry drift-check aligned with new service identity callers, and extend the status payload only with redacted metadata. |
| Event/control-plane drift | Event map tracks 137 `src` files touching EventBus, service identity, event helpers, SafeActuator, AsyncSafeActuator, identity status, service trace filters, billing webhook traces, or EventBus-to-Ledger indexing. | Keep `tests/unit/test_event_control_plane_map_unit.py` as the guard when adding new event publishers, action executors, trace filters, or trace-memory bridges. |
...
6. Event/control plane:
   ...
   - Done: drift-check verifies source refs, current 32-value EventType vocabulary, and current 137-file event surface coverage.
   - Done: redacted `/api/v1/service-identity/status` reports configured identity source metadata for all 88 current service identity callers without exposing identity values.
