...
NODE_CONFIGS = {
    "nl": {
        "server": os.getenv("NL_SERVER_IP", "89.125.1.107"),
        "port": int(os.getenv("NL_SERVER_PORT", "443")),
        "public_key": os.getenv("NL_PUBLIC_KEY", "xMwVfOuehQZwVHPodTvo3TJEGUYUbxmGTeAxMUBWpww"),
        "remark": os.getenv("NL_REMARK", "x0tta6bl4_NL_Master")
    },
    "ru": {
        "server": os.getenv("RU_SERVER_IP", "TBD_RU_IP"),
        "port": int(os.getenv("RU_SERVER_PORT", "443")),
        "public_key": os.getenv("RU_PUBLIC_KEY", "TBD_RU_PUB_KEY"),
        "remark": os.getenv("RU_REMARK", "x0tta6bl4_RU_Entry")
    },
    "us": {
        "server": os.getenv("US_SERVER_IP", "TBD_US_IP"),
        "port": int(os.getenv("US_SERVER_PORT", "443")),
        "public_key": os.getenv("US_PUBLIC_KEY", "TBD_US_PUB_KEY"),
        "remark": os.getenv("US_REMARK", "x0tta6bl4_US_Exit")
    }
}
...
def generate_vless_link(
    ...
    *,
    ...
    final_server = server if server is not None else node["server"]
    final_port = port if port is not None else node["port"]
    public_key = node["public_key"]
    final_remark = remark if remark is not None else node["remark"]
    ...
    vless_link = (
        f"vless://{user_uuid}@{final_server}:{final_port}"
        f"?type=tcp"
        f"&encryption=none"
        f"&security=reality"
        f"&pbk={public_key}"
        ...
        f"#{urllib.parse.quote(final_remark)}"
    )
    return vless_link
...
User ID: {user_id}
Node: {node_id.upper()} ({final_server})
