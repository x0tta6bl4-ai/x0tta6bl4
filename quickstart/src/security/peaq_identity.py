...
class PeaqIdentityAdapter:
    ...
    def sign_telemetry(self, data: bytes) -> Dict[str, Any]:
        ...
        sig_str = signature.signature.hex() if hasattr(signature.signature, "hex") else str(signature.signature)
        mhash = getattr(signature, "message_hash", None) or getattr(signature, "messageHash", b"")
        mhash_str = mhash.hex() if hasattr(mhash, "hex") else str(mhash)
        return {
            "did": self.did,
            "signature": sig_str,
            "message_hash": mhash_str,
            "v": getattr(signature, "v", 27),
            "r": hex(signature.r) if hasattr(signature, "r") else "0x" + "0"*64,
            "s": hex(signature.s) if hasattr(signature, "s") else "0x" + "0"*64,
        }
