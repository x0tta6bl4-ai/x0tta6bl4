...
class KimiK25Integration:
    ...
    def __init__(
        ...
        # Auto-detect local Ollama/OpenAI-compatible stack if no Kimi API key is provided
        is_local = False
        if api_endpoint is None:
            if not os.environ.get("KIMI_API_KEY") and not api_key:
                # Default to local Ollama
                self.api_endpoint = "http://localhost:11434/v1"
                self._model = model or "qwen2.5-coder:1.5b"
                is_local = True
            else:
                self.api_endpoint = self._DEFAULT_ENDPOINT
        ...
        # Optimize default timeout for local Ollama deployments to 5.0s
        if is_local and timeout_s == 30.0:
            self._timeout_s = 5.0
        else:
    ...
    def _get_api_key(self) -> str:
        import os
        key = self._api_key or os.environ.get("KIMI_API_KEY", "")
        # If no key, but endpoint is local, supply a stub key to avoid skipping the call
        if not key and ("localhost" in self.api_endpoint or "127.0.0.1" in self.api_endpoint):
            return "local-stub-key"
        return key
    ...
    async def enhance_decision(
        ...
        for attempt in range(self._max_retries + 1):
            try:
                # trust_env=False: bypass system SOCKS proxy (ALL_PROXY / HTTPS_PROXY)
                # to avoid requiring the optional 'socksio' package.  This also
                # disables other env-based HTTP settings (SSL_CERT_FILE, NO_PROXY).
                # If you need corporate proxy support for remote endpoints, set
                # proxy explicitly via httpx.AsyncClient(proxy=...) instead.
                async with httpx.AsyncClient(timeout=self._timeout_s, trust_env=False) as client:
                ...
                try:
                    ...
                    MetricsRegistry.maas_llm_inference_latency_seconds.labels(
                        model=self._model,
                        status="success"
                    ).observe(elapsed_ms / 1000.0)
                except Exception as m_exc:
                    logger.debug("Failed to record LLM latency metric: %s", m_exc)
        ...
        try:
            ...
            logger.debug("Failed to record LLM error latency metric: %s", m_exc)
