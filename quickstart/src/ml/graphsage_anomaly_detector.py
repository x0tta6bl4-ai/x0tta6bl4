...
class GraphSAGEAnalyzer(GraphSAGEAnomalyDetector):
    """Backward compatibility wrapper for GraphSAGEAnomalyDetector used in MAPE-K integration."""
    def __init__(
        self,
        in_channels: int = 8,
    ...
    def analyze_anomaly_sync(
        ...
        target_node = next(iter(node_features.keys())) if node_features else "default"
        target_features = node_features.get(target_node, {})
        
        neighbors = []
        if node_topology and target_node in node_topology:
            for neighbor in node_topology[target_node]:
                if neighbor in node_features:
                    neighbors.append((neighbor, node_features[neighbor]))
        ...
        failure_type = FailureType.RESOURCE_EXHAUSTION if prediction.is_anomaly else FailureType.UNKNOWN
        recommended_action = "clear_cache" if prediction.is_anomaly else "none"
        severity = prediction.anomaly_score
        ...
        return AnomalyAnalysis(
            ...
            severity=severity,
            affected_nodes=[target_node] if prediction.is_anomaly else []
        )
    ...
    async def analyze_anomaly(
        ...
        return self.analyze_anomaly_sync(node_features, node_topology)
