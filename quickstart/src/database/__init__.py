...
def get_alembic_head_gaps() -> List[str]:
    """Check if the database version matches the alembic head version."""
    try:
        ...
        if not alembic_ini.exists() or not alembic_dir.exists():
            return ["Alembic config or migration directory missing"]
        ...
        if "alembic_version" not in inspector.get_table_names():
            return [f"Database has no alembic_version table. Expected head: {', '.join(heads)}"]
        ...
        # If current rev is not in heads, we have a gap
        if current_rev not in heads:
            expected = ", ".join(heads)
            return [f"Database version {current_rev} does not match alembic head: {expected}"]
            
        return []
    except Exception as exc:
        logger.warning(f"Failed to check alembic head status: {exc}")
        return [f"Alembic head check failed: {exc}"]
