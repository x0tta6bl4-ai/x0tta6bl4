...
from src.core.thread_safe_stats import AtomicCounter, AtomicFloat, ThreadSafeMetrics
...
class ThreadSafeMAPEKLoop:
    ...
    async def execute_cycle(
        self, system_metrics: dict[str, float], context: dict[str, Any]
    ) -> MAPEKState:
        """
        ...
        async with self._cycle_lock:
            ...
            try:
                ...
                knowledge_updates = await self._knowledge_phase(
                ...
                logger.info(
                    f"MAPE-K cycle #{cycle_id} completed in {cycle_duration:.3f}s"
                )
    ...
    async def _analyze_phase(
        self, monitor_data: dict[str, Any], context: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        self.metrics.set_gauge(
            "analysis_severity",
            0.0 if severity == "NORMAL" else 1.0 if severity == "WARNING" else 2.0,
        )
    ...
    async def _plan_phase(
        self, analysis_data: dict[str, Any], context: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def _execute_phase(
        self, plan_data: dict[str, Any], context: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "actions_executed": len(
                [a for a in actions if not a.startswith("No action")]
            ),
        }

    async def _knowledge_phase(
        self, execute_data: dict[str, Any], context: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def get_current_state(self) -> dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                ...
                if (self.successful_cycles.get() + self.failed_cycles.get()) > 0
                else 0.0
            ),
        }
