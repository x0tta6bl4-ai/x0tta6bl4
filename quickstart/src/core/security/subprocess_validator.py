...
def _validate_run_kwargs(kwargs: dict) -> None:
    if kwargs.get("shell", False):
        raise ValueError("Use of shell execution flag is not allowed")  # nosec
...
def safe_run(cmd: list[str], **kwargs) -> subprocess.CompletedProcess:
    ...
    """
    # Resolve non-string parameter objects (like FastAPI Query) to their default values
    resolved_cmd = []
    for arg in cmd:
        if not isinstance(arg, str) and hasattr(arg, "default") and isinstance(getattr(arg, "default"), str):
            resolved_cmd.append(arg.default)
        else:
            resolved_cmd.append(arg)

    validate_command(resolved_cmd)
    validate_arguments(resolved_cmd)
    _validate_run_kwargs(kwargs)
    safe_kwargs = dict(kwargs)
    safe_kwargs["shell"] = False
    resolved_cmd[0] = _resolve_trusted_command(resolved_cmd[0])
