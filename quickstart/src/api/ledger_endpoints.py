...
def _extract_citations(results: list[Dict[str, Any]]) -> list[Dict[str, Any]]:
    ...
    for result in results:
        ...
        redacted = metadata["redacted"] if "redacted" in metadata else result.get("redacted")
        ...
        citation = _compact_citation(
            {
                ...
                "source_agent": metadata.get("source_agent") or result.get("source_agent"),
...
def _ledger_continuity_file_available(ledger_rag: Any) -> bool:
    ...
    return continuity_file is not None and callable(getattr(continuity_file, "exists", None))
...
def _ledger_event_trace_dependencies_available() -> bool:
    return (
        callable(EventBus)
        and callable(service_event_trace_history)
        and bool(getattr(EventType, "__members__", None))
    )
...
def _ledger_readiness_status(ledger_rag: Any) -> Dict[str, Any]:
    ...
    return {
        ...
        "backing_state": {
            ...
            "event_trace_index": (
                "Runtime EventBus traces are indexed through ledger_rag event-trace methods."
            ),
...
async def index_ledger(force: bool = False, include_verification: bool = False):
    ...
    try:
        ...
        if success and include_verification:
            verification_success = await ledger_rag.index_verification_evidence(force=force)
        ...
        if success:
            ...
            raise HTTPException(status_code=500, detail="Не удалось проиндексировать ledger")
...
async def index_verification_evidence(
    ...
    try:
        ...
        logger.error(f"Ошибка при индексировании verification evidence: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def verification_evidence_status():
    ...
    try:
        ...
        logger.error(f"Ошибка при получении статуса verification evidence: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def index_event_traces(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Не удалось проиндексировать EventBus traces")
...
async def event_trace_status():
    ...
    try:
        ...
        logger.error(f"Ошибка при получении статуса EventBus traces: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
