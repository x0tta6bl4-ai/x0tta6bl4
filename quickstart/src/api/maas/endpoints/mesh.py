...
def _mesh_read_list_claim_boundary_summary(surface: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "local_audit_log_observation_claim_allowed": (
            headers.get("X-X0TTA6BL4-Local-Audit-Log-Observation-Claim-Allowed")
            or headers.get("X-x0tta6bl4-Local-Audit-Log-Observation-Claim-Allowed")
        )
        == "true",
        "local_mapek_event_observation_claim_allowed": (
            headers.get("X-X0TTA6BL4-Local-MAPE-K-Event-Observation-Claim-Allowed")
            or headers.get("X-x0tta6bl4-Local-MAPE-K-Event-Observation-Claim-Allowed")
        )
        == "true",
...
async def deploy_mesh(
    ...
    """
    # Check plan escalation
    PLAN_RANK = {"free": 0, "starter": 1, "pilot": 1, "pro": 2, "enterprise": 3}
    user_plan = getattr(user, "plan", "starter") or "starter"
    if user_plan == "free":
        user_plan = "starter"
    requested_plan = request.billing_plan or "starter"
    if requested_plan == "free":
        requested_plan = "starter"

    if PLAN_RANK.get(requested_plan, 1) > PLAN_RANK.get(user_plan, 1):
        raise HTTPException(
            status_code=402,
            detail="Plan escalation blocked. Please upgrade your subscription first.",
        )
