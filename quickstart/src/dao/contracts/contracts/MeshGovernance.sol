...
contract MeshGovernance is Ownable, ReentrancyGuard {
    ...
    // SPIRE SVID mapping for Sybil protection
    mapping(address => string) public nodeSvid;
    mapping(string => address) public svidToWallet;
    ...
    event NodeSvidRegistered(address indexed nodeAddress, string svid);
    ...
    function createProposal(
        ...
        require(bytes(nodeSvid[msg.sender]).length > 0, "Address not registered with unique SPIRE SVID");
    ...
    /**
     * @notice Register unique SPIRE SVID for node address (Sybil protection)
     */
    function registerNodeSvid(address nodeAddress, string memory svid) external onlyOwner {
        require(bytes(svid).length > 0, "SVID cannot be empty");
        require(svidToWallet[svid] == address(0) || svidToWallet[svid] == nodeAddress, "SVID already registered to another address");
        
        string memory oldSvid = nodeSvid[nodeAddress];
        if (bytes(oldSvid).length > 0) {
            delete svidToWallet[oldSvid];
        }
        
        nodeSvid[nodeAddress] = svid;
        svidToWallet[svid] = nodeAddress;
        emit NodeSvidRegistered(nodeAddress, svid);
    }
