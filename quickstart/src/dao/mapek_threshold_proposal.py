            ...
            if action["type"] == "update_mapek_threshold":
                ...
                if self.threshold_manager:
                    try:
                        ...
                        logger.info(
                            f"✅ Updated threshold {parameter} = {value} via ThresholdManager"
                        )
                    except Exception as e:
                        logger.error(
                            f"❌ Failed to update threshold via ThresholdManager: {e}"
                        )
                        return False
                else:
                    logger.warning(
                        f"⚠️ ThresholdManager not available, threshold {parameter} = {value} not applied"
                    )
    ...
    def get_current_thresholds(self) -> Dict[str, float]:
        ...
        if self.threshold_manager:
            try:
                ...
                logger.debug(
                    f"📊 Loaded thresholds from ThresholdManager: {thresholds}"
                )
                return thresholds
            except Exception as e:
                logger.warning(
                    f"⚠️ Failed to load thresholds from ThresholdManager: {e}"
                )
