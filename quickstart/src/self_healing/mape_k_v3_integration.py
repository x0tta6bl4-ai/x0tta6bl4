...
class MAPEKV3Integration:
    ...
    async def analyze_with_graphsage(
        ...
        """
        Анализ аномалий с помощью GraphSAGE (async).
        """
    ...
    def analyze_with_graphsage_sync(
        self,
        node_features: Dict[str, Dict[str, float]],
        node_topology: Optional[Dict[str, List[str]]] = None,
    ) -> Optional[AnomalyAnalysis]:
        """
        Анализ аномалий с помощью GraphSAGE (sync).
        """
        if not self.graphsage_analyzer:
            return None

        try:
            analysis = self.graphsage_analyzer.analyze_anomaly_sync(
                node_features=node_features, node_topology=node_topology
            )

            logger.info(
                f"GraphSAGE Sync Analysis: {analysis.failure_type.value} "
                f"(confidence={analysis.confidence:.2f}, severity={analysis.severity})"
            )

            return analysis
        except Exception as e:
            logger.error(f"GraphSAGE sync analysis failed: {e}", exc_info=True)
            return None
...
def integrate_v3_into_mapek(mapek_cycle, enable_graphsage=True, enable_stego=False):
    ...
    if v3_integration.graphsage_analyzer and hasattr(mapek_cycle, "analyzer"):
        ...
        def enhanced_analyze(metrics, node_id="unknown", event_id=None):
            # Вызываем оригинальный анализ
            result = original_analyze(metrics, node_id, event_id)

            # Добавляем GraphSAGE анализ если доступны node_features
            if isinstance(metrics, dict) and "node_features" in metrics:
                graphsage_result = v3_integration.analyze_with_graphsage_sync(
                    node_features=metrics["node_features"],
                    node_topology=metrics.get("node_topology"),
                )

                if graphsage_result and graphsage_result.failure_type.value != "unknown":
                    # Map FailureType to AI Strategy category
                    category_map = {
                        "node_failure": "Network Link Failure",
                        "link_failure": "Network Link Failure",
                        "byzantine_fault": "Byzantine Attack",
                        "resource_exhaustion": "Resource Exhaustion",
                    }
                    category = category_map.get(graphsage_result.failure_type.value, "Network Link Failure")
                    
                    # Override the result to trigger AI strategy planning
                    result = f"AI-Analysis ({category}): confidence={graphsage_result.confidence:.2f}, recommended action={graphsage_result.recommended_action}"
                    logger.info(f"🤖 GNN/GraphSAGE Anomaly Classified: {result}")
