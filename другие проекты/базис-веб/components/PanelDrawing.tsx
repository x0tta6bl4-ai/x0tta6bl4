...
const PanelDrawing: React.FC<PanelDrawingProps> = ({ panel, width, height, detailed = false }) => {
  ...
  const renderEdgeLine = (x1: number, y1: number, x2: number, y2: number, type: EdgeThickness) => {
      if (type === 'none') return null;
      const color = type === '0.4' ? '#2563eb' : '#dc2626'; 
      const strokeWidth = type === '0.4' ? 2 : 4; 
      return <line x1={x1} y1={y1} x2={x2} y2={y2} stroke={color} strokeWidth={strokeWidth} strokeLinecap="square" />;
  };
    ...
    <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} className="font-mono text-xs select-none bg-white">
        ...
        {(panel.hardware || []).map((hw, i) => {
            ...
            <g key={hw.id || i}>
                ...
                {detailed && (
                    <>
                        <line x1={startX - 10} y1={cy} x2={cx} y2={cy} stroke="#ccc" strokeDasharray="2,2" strokeWidth="0.5"/>
                        <text x={startX - 25} y={cy + 3} textAnchor="end" fontSize="9" fill="#666">{hw.y}</text>
                        
                        <line x1={cx} y1={startY + drawH + 10} x2={cx} y2={cy} stroke="#ccc" strokeDasharray="2,2" strokeWidth="0.5"/>
                        <text x={cx} y={startY + drawH + 25} textAnchor="middle" fontSize="9" fill="#666">{hw.x}</text>
                    </>
                )}
            </g>
        )})}

        {renderEdgeLine(startX, startY, startX + drawW, startY, panel.edging.top)}
        {renderEdgeLine(startX, startY + drawH, startX + drawW, startY + drawH, panel.edging.bottom)}
        {renderEdgeLine(startX, startY, startX, startY + drawH, panel.edging.left)}
        {renderEdgeLine(startX + drawW, startY, startX + drawW, startY + drawH, panel.edging.right)}

        {panel.groove && panel.groove.enabled && (
            <g>
                {panel.groove.side === 'top' && (
                    <>
                    <line x1={startX} y1={startY + panel.groove.offset * scale} x2={startX + drawW} y2={startY + panel.groove.offset * scale} stroke="#2563eb" strokeWidth="1" strokeDasharray="3,3"/>
                    <line x1={startX} y1={startY + (panel.groove.offset + panel.groove.width) * scale} x2={startX + drawW} y2={startY + (panel.groove.offset + panel.groove.width) * scale} stroke="#2563eb" strokeWidth="1" strokeDasharray="3,3"/>
                    </>
                )}
                {panel.groove.side === 'bottom' && (
                    <>
                    <line x1={startX} y1={startY + drawH - panel.groove.offset * scale} x2={startX + drawW} y2={startY + drawH - panel.groove.offset * scale} stroke="#2563eb" strokeWidth="1" strokeDasharray="3,3"/>
                    <line x1={startX} y1={startY + drawH - (panel.groove.offset + panel.groove.width) * scale} x2={startX + drawW} y2={startY + drawH - (panel.groove.offset + panel.groove.width) * scale} stroke="#2563eb" strokeWidth="1" strokeDasharray="3,3"/>
                    </>
                )}
                {panel.groove.side === 'left' && (
                    <>
                    <line x1={startX + panel.groove.offset * scale} y1={startY} x2={startX + panel.groove.offset * scale} y2={startY + drawH} stroke="#2563eb" strokeWidth="1" strokeDasharray="3,3"/>
                    <line x1={startX + (panel.groove.offset + panel.groove.width) * scale} y1={startY} x2={startX + (panel.groove.offset + panel.groove.width) * scale} y2={startY + drawH} stroke="#2563eb" strokeWidth="1" strokeDasharray="3,3"/>
                    </>
                )}
                {panel.groove.side === 'right' && (
                    <>
                    <line x1={startX + drawW - panel.groove.offset * scale} y1={startY} x2={startX + drawW - panel.groove.offset * scale} y2={startY + drawH} stroke="#2563eb" strokeWidth="1" strokeDasharray="3,3"/>
                    <line x1={startX + drawW - (panel.groove.offset + panel.groove.width) * scale} y1={startY} x2={startX + drawW - (panel.groove.offset + panel.groove.width) * scale} y2={startY + drawH} stroke="#2563eb" strokeWidth="1" strokeDasharray="3,3"/>
                    </>
                )}
            </g>
        )}

        <g transform={`translate(0, -${detailed ? 20 : 15})`}>
            ...
            <rect x={startX + drawW/2 - 20} y={startY - 8} width="40" height="16" fill="white"/>
            <text x={startX + drawW/2} y={startY + 3} textAnchor="middle" fill="#333" fontWeight="bold" fontSize="12">{panel.width}</text>
        </g>

        <g transform={`translate(-${detailed ? 20 : 15}, 0)`}>
            ...
            <rect x={startX - 8} y={startY + drawH/2 - 10} width="16" height="20" fill="white"/>
            <text x={startX} y={startY + drawH/2 + 3} textAnchor="middle" transform={`rotate(-90, ${startX}, ${startY + drawH/2})`} fill="#333" fontWeight="bold" fontSize="12">{panel.height}</text>
        </g>
        ...
        {detailed && (
            <g transform={`translate(20, ${height - 100})`}>
                ...
                <g transform="translate(0, 30)">
                    ...
                    <path d="M20,0 L17,-2 L17,2 Z" fill="#ef4444"/>
