...
import { Panel, ProductionStage, ProductionNorms, Material } from '../types';
import { 
    CheckCircle2, Clock, PlayCircle, AlertTriangle, 
    Scissors, Layers, Factory, Hammer, PackageCheck,
    FileDown, Printer, ArrowRight, ChevronRight,
    Box, FileText, CheckSquare, Scan, LayoutList, QrCode,
...
const PipelineStepper: React.FC<{ currentStage: ProductionStage, onStageSelect: (s: ProductionStage) => void }> = ({ currentStage, onStageSelect }) => {
    const stages: { id: ProductionStage, label: string, icon: React.ElementType }[] = [
        ...
        { id: 'drilling', label: 'Присадка', icon: CircleDot },
...
const ProductionPipeline: React.FC<ProductionPipelineProps> = ({ materialLibrary }) => {
    ...
    const renderDrillingStage = () => (
        <div className="h-full flex flex-col">
             <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div>
                     <h2 className="text-xl font-bold text-white flex items-center gap-2"><CircleDot className="text-purple-500"/> Участок Присадки (ЧПУ)</h2>
                        ...
                        {panels.filter(p => p.hardware.length > 0 || p.groove?.enabled).map((p, i) => (
                            <tr key={p.id} className="hover:bg-slate-800/50">
                                ...
                                <td className="p-3 font-bold">
                                    {p.name}
                                    <div className="sm:hidden text-xs font-normal text-slate-500">{p.hardware.length} отв.</div>
                                </td>
    ...
    const renderAssemblyStage = () => (
         <div className="h-full flex flex-col">
             <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={handleStageComplete} className="px-4 py-2 bg-emerald-600 text-white font-bold rounded flex items-center gap-2 hover:bg-emerald-700 shadow-[0_0_20px_rgba(5,150,105,0.4)]">
    ...
    const renderDashboard = () => (
        <div className="h-full overflow-y-auto no-scrollbar p-6 space-y-6">
            <h2 className="text-2xl font-bold text-white mb-4 flex items-center gap-3"><BarChart3 size={32} className="text-blue-500"/> MES Аналитика</h2>
            
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* 1. Material Usage Chart */}
                ...
                <div className="bg-[#1a1a1a] p-4 rounded-xl border border-slate-700 shadow-xl">
                    <h3 className="text-sm font-bold text-slate-400 mb-4 uppercase">Статус Выполнения ({currentGlobalStage})</h3>
            ...
            {/* 3. Activity Log (Simulated) */}
            <div className="bg-[#1a1a1a] p-6 rounded-xl border border-slate-700 shadow-xl">
                <h3 className="text-sm font-bold text-slate-400 mb-4 uppercase flex items-center gap-2"><Activity size={16}/> Лог Активности</h3>
                <div className="space-y-4">
                    {[1, 2, 3].map((_, i) => (
                        <div key={i} className="flex gap-4 border-l-2 border-slate-700 pl-4 py-1 relative">
                            ...
                            <div>
                                <div className="text-sm text-white">Оператор завершил этап <span className="text-blue-400 font-bold">{['Раскрой', 'Кромление', 'Присадка'][i]}</span> для партии #1024-{i+1}</div>
                                <div className="text-xs text-slate-500">Участок {i+1} • Пользователь: Master</div>
                            </div>
                        </div>
                    ))}
                </div>
        ...
        <div className="h-full flex flex-col bg-[#0f0f0f] text-slate-300">
            ...
            <div className="flex bg-[#1a1a1a] border-b border-slate-700">
                <button 
                    ...
                    className={`px-6 py-4 text-sm font-bold uppercase tracking-wider border-b-2 transition ${activeView === 'pipeline' ? 'border-blue-500 text-blue-400 bg-slate-800' : 'border-transparent text-slate-500 hover:text-slate-300'}`}
                >
                    Производство
                </button>
                <button 
                    onClick={() => setActiveView('dashboard')}
                    className={`px-6 py-4 text-sm font-bold uppercase tracking-wider border-b-2 transition ${activeView === 'dashboard' ? 'border-blue-500 text-blue-400 bg-slate-800' : 'border-transparent text-slate-500 hover:text-slate-300'}`}
                >
                    MES Дашборд
                </button>
            </div>
            ...
            {activeView === 'dashboard' ? (
                ...
                <>
                    ...
                    <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
                        ...
                        {/* RIGHT SIDEBAR (STATS) - Hidden on Print, Collapsible/Bottom on mobile */}
