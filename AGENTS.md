...
### Разделение статусов (3-Tier Taxonomy)

Каждый компонент или утверждение должно быть помечено строго одной из категорий:
- **✅ VERIFIED**: Подтверждено непосредственно автотестом или бенчмарком с тихим выходом Exit Code 0 и артефактом.
- **🟡 VALIDATED IN LAB**: Подтверждено на тестовом окружении / интеграционном стенде (CI, Docker Compose, local testbed).
- **⚪ TARGET**: Целевая характеристика или инженерная гипотеза, планируемая к подтверждению.

### Правила терминологии

- **Запрещено слово "физически":** Не использовать выражения типа "физически прогнан", если тесты проводились в Docker, CI или эмуляторе. Корректно: *"выполнен в тестовом окружении"* или *"validated on an integration testbed"*.
- **Главное позиционирование платформы:** Проект позиционируется через проверяемое поведение: *"Система умеет обнаруживать отказ, автоматически восстанавливаться и подтверждать соблюдение инвариантов после восстановления"*, а не через абстрактный список аббревиатур или наносекунд.
