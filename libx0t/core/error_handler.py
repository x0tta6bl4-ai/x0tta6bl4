...
# Optional alerting
try:
    from src.monitoring.alerting import AlertManager, AlertSeverity, send_alert
...
try:
    ...
    if "x0tta6bl4_errors_total" not in [
        c.describe()[0].name
...
class ErrorHandler:
    ...
    async def handle_error(
        ...
        if severity == ErrorSeverity.CRITICAL:
            ...
            logger.debug(
                f"🔍 ERROR in {context}: {error_type}: {error_message}", extra=log_data
            )

        # Alert for critical/high errors - Full integration with AlertManager
        if ALERTING_AVAILABLE and severity in [
            ...
            try:
                ...
                await send_alert(
                    ...
                    ),  # Skip rate limit for critical errors
                )
        ...
        if _error_counter is not None:
            try:
                _error_counter.labels(
                    error_type=error_type, context=context, severity=severity.value
                ).inc()
            except Exception:
    ...
    def handle_error_sync(
        ...
        if ALERTING_AVAILABLE and severity in [
            ...
            try:
                ...
                alert_manager = getattr(
                    ErrorHandler.handle_error_sync, "_alert_manager", None
                )
...
def handle_error_decorator(
    context: str, severity: ErrorSeverity = ErrorSeverity.MEDIUM
):
    """
