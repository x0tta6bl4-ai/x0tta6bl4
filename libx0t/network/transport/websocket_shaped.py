...
class ShapedWebSocketClient:
        ...
        try:
            if name == "xor":
                ...
                return TransportManager.create(
                    "faketls", sni=key or "www.cloudflare.com"
                )
    ...
    async def _reconnect(self) -> bool:
        ...
        while self._reconnect_attempts < self.max_reconnect_attempts:
            ...
            logger.info(
                f"Переподключение через {delay:.1f}с (попытка {self._reconnect_attempts})"
            )
        ...
        logger.error(
            f"Не удалось переподключиться после {self.max_reconnect_attempts} попыток"
        )
    ...
    async def send(self, data: bytes) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Отправлено: {msg.original_size}B -> {msg.shaped_size}B, delay={msg.delay_ms:.1f}ms"
            )
            return True
...
class ShapedWebSocketServer:
    ...
    def __init__(
        ...
        self._obfuscation_key = obfuscation_key or os.getenv(
            "OBFUSCATION_KEY", "x0tta6bl4"
        )
    ...
    def _create_transport(self) -> Optional[ObfuscationTransport]:
        ...
        try:
            if self._obfuscation == "xor":
                ...
                return TransportManager.create(
                    "shadowsocks", password=self._obfuscation_key
                )
        except Exception:
            pass
        return None
...
async def example_echo_server():
    ...
    server = ShapedWebSocketServer(
        port=8765,
        obfuscation="xor",
        obfuscation_key=os.getenv("OBFUSCATION_KEY", "demo-key"),
