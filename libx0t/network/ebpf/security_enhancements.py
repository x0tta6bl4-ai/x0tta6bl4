...
class SecurityEnhancements:
    ...
    def configure_noise(
        ...
        logger.info(
            f"Noise injection configured: {level.value} ({self.noise_config.min_ns}-{self.noise_config.max_ns}ns)"
        )
    ...
    def check_lru_eviction(self, map_name: str, current_entries: int) -> bool:
        ...
        threshold = int(
            self.lru_config.max_entries * self.lru_config.eviction_threshold
        )
    ...
    def get_security_status(self) -> Dict[str, any]:
        ...
        return {
            ...
            "map_stats": {name: stats for name, stats in self._map_usage_stats.items()},
        }
