...
# Optional imports
try:
    from .loader import EBPFLoader, EBPFProgramType
...
class CiliumLikeIntegration:
    ...
    def _export_flow(self, flow_event: FlowEvent):
        ...
        try:
            ...
            logger.debug(
                f"Flow exported: {flow_event.source_ip} -> {flow_event.destination_ip}"
            )
        except Exception as e:
            ...
            self.metrics_exporter.export_metrics(
                {
                    ...
                    "cilium_packets_processed_total": metrics[
    ...
    def get_hubble_like_flows(
        ...
        for flow in flows:
            hubble_flow = {
                ...
                "source": {"ip": flow.source_ip, "port": flow.source_port},
                "destination": {
