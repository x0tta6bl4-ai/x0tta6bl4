...
class FakeTLSSocket(socket.socket):
    ...
    def __init__(self, sock: socket.socket, transport: "FakeTLSTransport"):
        ...
        # FakeTLSSocket acts as a wrapper, delegating all operations to self._sock.
        # It does NOT re-initialize super() with fileno, as that could cause issues
        # with socket ownership and is not the intended wrapper pattern.
    ...
    def _recv_exact(self, size: int, flags=0) -> bytes:
        # Helper to ensure exactly 'size' bytes are read, or return empty if not possible
        buffer = b""
        while len(buffer) < size:
            packet = self._sock.recv(size - len(buffer), flags)
            if not packet:
                # Connection closed or no more data available
                return b""
            buffer += packet
        return buffer

    def recv(self, bufsize: int, flags=0) -> bytes:
        # This recv is simplified and expects complete TLS records.
        # It does NOT handle partial records spanning multiple recv calls.

        if not self._handshake_received:
            # Attempt to read 5-byte header for ServerHello handshake record
            ...
            if content_type == 0x16:  # Handshake
                # Read and ignore the ServerHello body
                sh_body = self._recv_exact(length, flags)
                if not sh_body:
                    return b"" # Incomplete ServerHello body
                self._handshake_received = True
            else:
                # Unexpected content type for initial record.
                # For this simplified implementation, we treat it as an error/malformed stream.
                return b""
        ...
        if content_type == 0x17:  # Application Data
            data = self._recv_exact(length, flags)
            if not data:
                return b"" # Incomplete Application Data body
            return self._transport.deobfuscate(data)
        else:
            # Unexpected content type for application data record.
            return b""
