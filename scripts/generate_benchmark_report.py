...
def generate_markdown_report(data: Dict[str, Any]) -> str:
    ...
    report.append(
        f"**Environment:** {data['environment'].get('os', 'Unknown')} {data['environment'].get('os_version', '')}\n"
    )
    report.append("\n---\n")
...
def generate_html_report(data: Dict[str, Any]) -> str:
    ...
    html.append(
        f"<p><strong>Environment:</strong> {data['environment'].get('os', 'Unknown')}</p>"
    )
...
def main():
    ...
    parser.add_argument("input_file", type=Path, help="Input JSON benchmark file")
    parser.add_argument(
        "--format",
