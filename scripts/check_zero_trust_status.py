...
def format_trust_score(score) -> str:
    ...
    return score_map.get(score.value if hasattr(score, 'value') else score, "❓ UNKNOWN")
    ...
    if args.peer:
        ...
        if args.json:
            output = {
                ...
                "trust_score": result.trust_score.value if hasattr(result.trust_score, 'value') else str(result.trust_score),
                "reason": result.reason,
                "isolation_level": result.isolation_level.value if result.isolation_level and hasattr(result.isolation_level, 'value') else None
            }
        ...
        if args.json:
            ...
            if args.detailed:
                ...
                print(f"Avg Sync Duration: {stats.get('avg_sync_duration_ms', 0):.2f}ms")
