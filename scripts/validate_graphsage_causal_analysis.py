...
from src.ml.causal_analysis import (CausalAnalysisEngine, CausalAnalysisResult,
                                    RootCause)
from src.ml.graphsage_anomaly_detector import (AnomalyPrediction,
                                               GraphSAGEAnomalyDetector)
from src.monitoring.alerting import AlertSeverity, send_alert
...
async def validate_graphsage_causal_analysis():
    ...
    anomalous_nodes_no_anomaly = [
        n_id for n_id, pred in predictions_no_anomaly.items() if pred.is_anomaly
    ]
    ...
    if not anomalous_nodes_no_anomaly:
        ...
        print(
            f"❌ Scenario 1: Incorrectly identified anomalies: {anomalous_nodes_no_anomaly}"
        )
        await send_alert(
    ...
    anomalous_nodes_anomaly = [
        n_id for n_id, pred in predictions_anomaly.items() if pred.is_anomaly
    ]

    if anomaly_target_node in anomalous_nodes_anomaly:
        print(
            f"✅ Scenario 2: Correctly identified anomaly in node {anomaly_target_node}."
        )
        ...
        try:
            causal_graph: CausalGraph = causal_analysis_engine.analyze(
                node_status=[
                    NodeStatus(node_id=nf["node_id"], status=pred.anomaly_score > 0.5)
                    for nf, pred in zip(
                        node_features_anomaly, predictions_anomaly.values()
                    )
                ],
                edge_status=[
                    EdgeStatus(source=u, destination=v, status=True)
                    for u, v in edges_anomaly
                ],  # Assume all edges healthy for simplicity
            )
            print(
                f"✅ Causal analysis completed for node {anomaly_target_node}. Root causes: {causal_graph.get_root_causes()}"
            )
            if causal_graph.get_root_causes():
                print(
                    f"    - Example root cause: {causal_graph.get_root_causes()[0].description}"
                )
            else:
                print(
                    "    - No specific root causes identified (might be expected for simple anomaly)."
                )
        except Exception as e:
            logger.error(
                f"❌ Causal analysis failed for node {anomaly_target_node}: {e}"
            )
            await send_alert(
                ...
                annotations={
                    "runbook_url": "/docs/ml/causal_analysis_troubleshooting.md"
                },
            )
    else:
        print(
            f"❌ Scenario 2: Failed to identify anomaly in node {anomaly_target_node}. Anomalies found: {anomalous_nodes_anomaly}"
        )
        await send_alert(
