...
def detect_anomalies(metrics: List[Dict[str, float]], metric_name: str) -> List[Dict[str, Any]]:
...
def analyze_memory_trend(metrics: List[Dict[str, float]]) -> Dict[str, Any]:
    ...
    memory_values = [m.get('container_memory_usage_bytes', 0) for m in metrics if 'container_memory_usage_bytes' in m]
    ...
    if len(memory_values) > 3:
        ...
        diffs = [memory_values[i+1] - memory_values[i] for i in range(len(memory_values)-1)]
...
def generate_report(metrics_dir: Path, output_file: Path):
    ...
    metrics_files = sorted(metrics_dir.glob("*.json")) + sorted(metrics_dir.glob("metrics_*.txt"))
    ...
    with open(output_file, 'w') as f:
        ...
        if memory_analysis.get('status') != 'insufficient_data':
            ...
            f.write(f"- Growth: {memory_analysis.get('growth', 0):,.0f} bytes ({memory_analysis.get('growth_percent', 0):.2f}%)\n")
            
            if memory_analysis.get('status') == 'leak_detected':
                f.write(f"- ⚠️ **Memory Leak Detected:** Linear growth pattern detected\n")
            else:
...
def main():
    ...
    parser.add_argument("--metrics-dir", type=str, default="./stability_test_metrics",
                       help="Directory containing metrics files")
    parser.add_argument("--output", type=str, default="./STABILITY_METRICS_ANALYSIS.md",
