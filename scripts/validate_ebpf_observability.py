...
async def validate_ebpf_observability(
    ...
    try:
        ...
        compile_cmd = [
            ...
            "bpf",
            "-I",
            kernel_headers_path,  # Include kernel headers
            "-c",
            program_c_path,
            "-o",
            program_o_path,
    ...
    try:
        ...
        success = ebpf_loader.attach_to_interface(
            program_id, interface, EBPFAttachMode.SKB
        )
        if success:
            logger.info(
                f"✅ eBPF program {program_id} attached to {interface} successfully."
            )
        else:
        ...
        try:
            ...
            if "xdp" in result:
                logger.info(
                    f"✅ 'xdp' found in 'ip link show dev {interface}' output. Program likely attached."
                )
            else:
                logger.warning(
                    f"⚠️ 'xdp' NOT found in 'ip link show dev {interface}' output. Program might not be active."
                )
        except Exception as e:
        ...
        if ebpf_loader and program_id:
            ...
            try:
                if ebpf_loader.detach_from_interface(program_id, interface):
                    logger.info(
                        f"✅ eBPF program {program_id} detached from {interface}."
                    )
                else:
                    logger.warning(
                        f"⚠️ Failed to detach eBPF program {program_id} from {interface}."
                    )
                ...
                await send_alert(
                    ...
                    annotations={
                        "runbook_url": "/docs/monitoring/ebpf_troubleshooting.md"
                    },
                )
