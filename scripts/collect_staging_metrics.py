...
def collect_kubectl_metrics(namespace: str) -> Dict:
    ...
    try:
        ...
        logger.info(f"✅ Собрано метрик: {len(metrics['pods'])} pods, {len(metrics['services'])} services")
...
def collect_api_metrics(url: str) -> Dict:
    ...
    try:
        ...
        if response.status_code == 200:
            metrics["endpoints"]["prometheus_metrics"] = response.text[:10000]  # Первые 10KB
...
def main():
    ...
    parser.add_argument("--namespace", default="x0tta6bl4-staging", help="Kubernetes namespace")
