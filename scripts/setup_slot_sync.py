...
async def setup_slot_sync(num_nodes: int, num_slots: int) -> Dict:
    ...
    """
    logger.info(
        f"Setting up slot synchronization: {num_nodes} nodes, {num_slots} slots"
    )
    ...
    for i in range(num_nodes):
        ...
        sync = SlotSynchronizer(
            node_id=node_id, total_slots=num_slots, beacon_interval=1.0
        )
    ...
    avg_resync = (
        sum(s["resync_attempts"] for s in statuses) / len(statuses) if statuses else 0
    )
...
async def main():
    ...
    if args.validate:
        ...
        if results["success"]:
            ...
            print(
                f"   Synchronization rate: {results['synchronization_rate']:.1%} (target: ≥95%)"
            )
            exit(1)
    else:
