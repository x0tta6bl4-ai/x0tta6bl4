...
class ResourceMonitor:
    ...
    def get_summary(self) -> Dict[str, float]:
        ...
        return {
            ...
            "memory_percent_mean": (
                mean(memory_percent_values) if memory_percent_values else 0
            ),
        }
...
class ApiLoadTester:
    ...
    async def test_endpoint(
        self, method: str = "GET", path: str = "/health/ready"
    ) -> bool:
    ...
    async def run_load_test(
        ...
        logger.info(
            f"📊 Running load test ({duration_seconds}s, {concurrent_requests} concurrent)..."
        )
        ...
        logger.info(
            f"   Requests: {self.request_count} ({self.success_count} OK, {self.error_count} failed)"
        )
        logger.info(f"   Throughput: {throughput_rps:.2f} req/s")
        logger.info(
            f"   Latency P50: {latency_stats['p50_ms']:.2f}ms, P95: {latency_stats['p95_ms']:.2f}ms, P99: {latency_stats['p99_ms']:.2f}ms"
        )
...
class DatabaseBenchmark:
    ...
    def __init__(self):
        self.connection_string = os.getenv(
            "DATABASE_URL", "postgresql://postgres:postgres@localhost:5432/x0tta6bl4"
        )
    ...
    async def get_connection_pool_stats(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if result:
                return {"active_connections": result[0]["active_connections"]}
    ...
    def compare_to_previous(self) -> Dict[str, Any]:
        ...
        try:
            ...
            comparison = self._compare_metrics(
                current["metrics"][0], previous["metrics"][0]
            )
