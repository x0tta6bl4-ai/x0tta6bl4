...
def compare_metrics(baseline: Dict[str, Any], current: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if latency_regression:
        regressions.append(f"Latency P95 increased by {latency_diff:.2f}ms ({latency_diff/baseline_latency*100:.1f}%)")
    ...
    if memory_regression:
        regressions.append(f"Memory increased by {memory_diff:.2f}MB ({memory_diff/baseline_memory*100:.1f}%)")
