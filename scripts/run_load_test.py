...
from tests.load.production_load_test import (LoadTestConfig,
                                             ProductionLoadTester)
...
async def main():
    ...
    print("\n🎯 CRITERIA CHECK:")
    print(
        f"  Success Rate >= 99%: {'✅' if summary['success_rate_percent'] >= 99.0 else '❌'} ({summary['success_rate_percent']:.2f}%)"
    )
    print(
        f"  Latency P95 <= 100ms: {'✅' if summary['latency_p95_ms'] <= 100.0 else '❌'} ({summary['latency_p95_ms']:.2f}ms)"
    )
    print(
        f"  Max Memory <= 2.4GB: {'✅' if summary['max_memory_mb'] <= 2400.0 else '❌'} ({summary['max_memory_mb']:.2f}MB)"
    )
    print(
        f"  Throughput >= 6,800 req/sec: {'✅' if summary['avg_throughput_per_sec'] >= 6800.0 else '❌'} ({summary['avg_throughput_per_sec']:.2f} req/sec)"
    )
    print()
