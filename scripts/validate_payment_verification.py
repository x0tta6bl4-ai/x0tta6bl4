...
from src.sales.telegram_bot import confirm_payment, Config, TokenGenerator # Import necessary functions
...
async def simulate_payment_verification():
    ...
    if Config.USDT_TRC20_WALLET == "TYourWalletAddressHere" and Config.TON_WALLET == "UQYourTonWalletAddressHere":
        print("⚠️ Warning: Wallet addresses are default placeholders. Please set USDT_TRC20_WALLET and TON_WALLET env vars.")
        return
    ...
    with patch('src.sales.payment_verification.requests.get') as mock_get:
        ...
        result = verifier.verify_payment(Config.USDT_TRC20_WALLET, amount_usdt, test_order_id)
        
        if result['verified']:
            print(f"✅ TronScanVerifier direct test: Payment verified! TX: {result['transaction_hash']}")
        else:
            print(f"❌ TronScanVerifier direct test: Verification failed. Error: {result['error']}")
    ...
    with patch('src.sales.payment_verification.requests.get') as mock_get:
        ...
        if result['verified']:
            print(f"✅ TONVerifier direct test: Payment verified! TX: {result['transaction_hash']}")
        else:
            print(f"❌ TONVerifier direct test: Verification failed. Error: {result['error']}")
    ...
    with patch('src.sales.telegram_bot.TronScanVerifier') as MockTronScanVerifier, \
        ...
        if mock_tronscan_result['verified'] or mock_ton_result['verified']:
            ...
            if "ПЛАТЕЖ ПОДТВЕРЖДЁН!" in message:
                print("✅ Telegram bot simulation: Payment confirmed message displayed.")
            else:
                print(f"❌ Telegram bot simulation: Unexpected message on payment confirmed: {message}")
        else:
            mock_update.callback_query.edit_message_text.assert_called_once()
            args, kwargs = mock_update.callback_query.edit_message_text.call_args
            message = args[0]
            if "ПЛАТЕЖ НЕ НАЙДЕН" in message:
                print("✅ Telegram bot simulation: Payment not found message displayed.")
            else:
                print(f"❌ Telegram bot simulation: Unexpected message on payment not found: {message}")
