...
def benchmark_kyber(iterations: int = 100) -> dict:
    ...
    return {
        ...
        "total_ms": statistics.mean(keygen_times)
        + statistics.mean(encap_times)
        + statistics.mean(decap_times),
...
def benchmark_rsa(iterations: int = 100) -> dict:
    ...
    return {
        ...
        "total_ms": statistics.mean(keygen_times)
        + statistics.mean(encrypt_times)
        + statistics.mean(decrypt_times),
...
def print_comparison(kyber: dict, rsa: dict):
    ...
    print(
        f"{'Key Generation':<20} {kyber['keygen_ms']:>12.3f}ms {rsa['keygen_ms']:>12.3f}ms {speedup:>12.1f}x faster"
    )

    # Encapsulation
    speedup = rsa["encap_ms"] / kyber["encap_ms"]
    print(
        f"{'Encapsulation':<20} {kyber['encap_ms']:>12.3f}ms {rsa['encap_ms']:>12.3f}ms {speedup:>12.1f}x faster"
    )

    # Decapsulation
    speedup = rsa["decap_ms"] / kyber["decap_ms"]
    print(
        f"{'Decapsulation':<20} {kyber['decap_ms']:>12.3f}ms {rsa['decap_ms']:>12.3f}ms {speedup:>12.1f}x faster"
    )
    ...
    print("-" * 70)
    print(
        f"{'TOTAL':<20} {kyber['total_ms']:>12.3f}ms {rsa['total_ms']:>12.3f}ms {speedup:>12.1f}x faster"
    )

    print(f"\n{'Key Sizes':<20} {'Kyber768':>15} {'RSA-2048':>15}")
    print("-" * 70)
    print(
        f"{'Public Key':<20} {kyber['public_key_bytes']:>12} B {rsa['public_key_bytes']:>12} B"
    )
    print(
        f"{'Private Key':<20} {kyber['private_key_bytes']:>12} B {rsa['private_key_bytes']:>12} B"
    )
    print(
        f"{'Ciphertext':<20} {kyber['ciphertext_bytes']:>12} B {rsa['ciphertext_bytes']:>12} B"
    )
