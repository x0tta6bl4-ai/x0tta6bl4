...
async def test_action(executor: RecoveryActionExecutor, action_type: str, **kwargs) -> bool:
...
def main():
    ...
    parser.add_argument("--service", type=str, help="Service name (for restart/clear-cache)")
    ...
    parser.add_argument("--primary-region", type=str, help="Primary region (for failover)")
    parser.add_argument("--fallback-region", type=str, help="Fallback region (for failover)")
    parser.add_argument("--cache-type", type=str, default="all", help="Cache type (for clear-cache)")
    ...
    if not args.action:
        print("❌ Error: --action is required (use --list-actions to see available actions)")
        sys.exit(1)
    ...
    if args.action == "Restart service":
        ...
        if not args.deployment or not args.replicas:
            ...
            sys.exit(1)
        kwargs = {"deployment_name": args.deployment, "replicas": args.replicas, "namespace": args.namespace}
    elif args.action == "Switch route":
        if not args.old_route or not args.new_route:
            print("❌ Error: --old-route and --new-route are required for switch-route action")
            sys.exit(1)
        ...
        if not args.primary_region or not args.fallback_region:
            print("❌ Error: --primary-region and --fallback-region are required for failover action")
            sys.exit(1)
        kwargs = {"service_name": args.service or "default-service", "primary_region": args.primary_region, "fallback_region": args.fallback_region}
