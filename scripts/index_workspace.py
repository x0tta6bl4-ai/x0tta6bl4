...
IGNORED_DIRS = {'.git', '.Trash-1000', '.venv', 'System Volume Information', '__pycache__', 'node_modules'}

# Basic extension categories
EXT_CATEGORIES: Dict[str, str] = {
    # code
    '.py': 'code', '.js': 'code', '.ts': 'code', '.java': 'code', '.c': 'code', '.h': 'code', '.hpp': 'code', '.go': 'code', '.rs': 'code', '.php': 'code', '.html': 'code', '.css': 'code', '.json': 'code', '.yml': 'code', '.yaml': 'code', '.sh': 'code', '.sql': 'code', '.rb': 'code', '.md': 'code', '.toml': 'code', '.ini': 'code', '.cfg': 'code', '.txt': 'code', '.jsx': 'code', '.tsx': 'code', '.gradle': 'code', '.xml': 'code', '.pl': 'code', '.pm': 'code', '.swift': 'code', '.kt': 'code', '.dart': 'code', '.lua': 'code', '.cmake': 'code', '.make': 'code', '.mk': 'code', '.dockerfile': 'code', '.env': 'code', '.gitignore': 'code',
    # media
    '.jpg': 'media', '.jpeg': 'media', '.png': 'media', '.gif': 'media', '.bmp': 'media', '.tif': 'media', '.tiff': 'media', '.svg': 'media', '.webp': 'media', '.psd': 'media', '.ico': 'media', '.mp4': 'media', '.mov': 'media', '.avi': 'media', '.mkv': 'media', '.webm': 'media', '.mp3': 'media', '.wav': 'media', '.flac': 'media', '.aac': 'media', '.ogg': 'media', '.3gp': 'media', '.heic': 'media',
    # archive
    '.gz': 'archive', '.zip': 'archive', '.rar': 'archive', '.7z': 'archive', '.tar': 'archive', '.xz': 'archive', '.bz2': 'archive', '.zst': 'archive', '.lz4': 'archive', '.tgz': 'archive', '.jar': 'archive',
    # cache/build
    '.pyc': 'cache', '.so': 'cache', '.o': 'cache', '.obj': 'cache', '.dll': 'cache', '.class': 'cache', '.map': 'cache', '.lock': 'cache', '.log': 'cache', '.tmp': 'cache', '.part': 'cache', '.swp': 'cache',
    # documents
    '.pdf': 'doc', '.doc': 'doc', '.docx': 'doc', '.xls': 'doc', '.xlsx': 'doc', '.ppt': 'doc', '.pptx': 'doc', '.odt': 'doc', '.ods': 'doc', '.odp': 'doc', '.rtf': 'doc',
    # 3D/CAD/graphics
    '.f3d': 'cad', '.cdr': 'cad', '.plt': 'cad', '.dxf': 'cad', '.dwg': 'cad', '.max': 'cad', '.obj': 'cad', '.stl': 'cad', '.blend': 'cad', '.ps': 'cad'
}
    ...
    def walk(current: str, depth: int) -> DirNode:
        ...
        for e in entries:
            ...
            if os.path.isdir(full):
                ...
                files.append(e)
        return DirNode(name=os.path.basename(current) or current, path=current, dirs=dirs, files=files)
    return walk(root, 0)


def scan_all(root: str, collect_largest: int = 0) -> Tuple[Dict[str, Dict[str, int]], int, int, List[Tuple[str,int]]]:
...
def aggregate_categories(rows: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for cat, d in acc.items():
        out.append({'category': cat, 'count': d['count'], 'bytes': d['bytes'], 'human': human_size(d['bytes'])})
    out.sort(key=lambda r: r['bytes'], reverse=True)
    return out

def detect_previous_diff(previous_index_path: str, current_files: List[Tuple[str,int]], threshold_bytes: int) -> Dict[str, List[Dict[str, Any]]]:
    try:
        with open(previous_index_path, 'r', encoding='utf-8') as f:
            prev = json.load(f)
    except Exception:
        return {}
    prev_largest = {item['path']: item['bytes'] for item in prev.get('largest_files', []) if 'path' in item}
    ...
    for p, s in curr_map.items():
        if p not in prev_largest:
            ...
            if s > prev_size and s - prev_size >= threshold_bytes:
                grown_files.append({'path': p, 'old_bytes': prev_size, 'new_bytes': s, 'delta_bytes': s - prev_size, 'delta_human': human_size(s - prev_size)})
...
def main():
    ...
    ap.add_argument('--largest', type=int, default=500, help='Collect top N largest files')
    ap.add_argument('--previous', default=None, help='Path to previous index for diff detection')
    ap.add_argument('--diff-threshold-mb', type=int, default=100, help='Size or growth threshold in MB for diff reporting')
    args = ap.parse_args()

    root = os.path.abspath(args.root)
    tree = build_tree(root, args.depth)
    ext_stats_raw, total_files, total_bytes, largest_files = scan_all(root, args.largest)
    ...
    largest_serialized: List[Dict[str, Any]] = [
        {
            ...
            'bytes': s,
            'human': human_size(s)
        } for p, s in largest_files
    ]
    diff = {}
    if args.previous:
        diff = detect_previous_diff(args.previous, largest_files, args.diff_threshold_mb * 1024 * 1024)
    ...
    print(f"Index written to {args.output} (files={total_files}, size={human_size(total_bytes)}); largest saved to {top_largest_path}")
