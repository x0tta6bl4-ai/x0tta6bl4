...
def main():
    ...
    try:
        ...
        print(f"   {'✅' if SENTENCE_TRANSFORMERS_AVAILABLE else '❌'} SENTENCE_TRANSFORMERS_AVAILABLE: {SENTENCE_TRANSFORMERS_AVAILABLE}")
    ...
    # RAG Pipeline
    try:
        from rag.pipeline import RAGPipeline, CROSS_ENCODER_AVAILABLE
        
        print(f"   ✅ RAGPipeline импортирован")
        print(f"   {'✅' if CROSS_ENCODER_AVAILABLE else '❌'} CROSS_ENCODER_AVAILABLE: {CROSS_ENCODER_AVAILABLE}")
    ...
    critical = ['hnswlib', 'sentence-transformers', 'SentenceTransformer', 'VectorIndex', 'RAGPipeline', 'LedgerRAGSearch']
