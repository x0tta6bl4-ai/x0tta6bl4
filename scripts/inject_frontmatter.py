...
def detect_status(filepath: Path, content: str) -> str:
    ...
    if 'draft' in path_str or 'wip' in path_str or 'todo' in content_lower[:500]:
        return "Draft"
    elif 'deprecated' in path_str or 'legacy' in path_str or 'deprecated' in content_lower[:500]:
        return "Deprecated"
...
def detect_criticality(filepath: Path, content: str) -> str:
    ...
    critical_keywords = ['critical', 'security', 'authentication', 'authorization', 'production']
...
def inject_frontmatter_into_file(filepath: Path, dry_run: bool = False) -> bool:
    ...
    try:
        ...
        if dry_run:
            ...
            print(f"     Tags: {', '.join(frontmatter['tags']) if frontmatter['tags'] else 'None'}")
        else:
