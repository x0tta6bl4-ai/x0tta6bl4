...
def show_profile(conn: sqlite3.Connection, inbound_id: int) -> dict:
    ...
    if profile["security"] == "reality":
        ...
        target_domain = target.split(":")[0] if ":" in target else target
        preferred_snis = _preferred_candidates(port, PORT_PREFERRED_SERVER_NAMES)
        if target_domain and target_domain not in preferred_snis:
            preferred_snis = preferred_snis + [target_domain]
        preferred_snis = preferred_snis
