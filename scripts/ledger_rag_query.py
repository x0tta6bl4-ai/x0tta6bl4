...
def main():
    if len(sys.argv) < 2:
        ...
        print("\nПримеры:")
        print('  python scripts/ledger_rag_query.py "Какие метрики у нас хуже targets?"')
        print('  python scripts/ledger_rag_query.py "Какие issues нужно решить в первую очередь?"')
        sys.exit(1)
