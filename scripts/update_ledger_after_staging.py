...
def load_validation_results(results_dir: Path) -> Optional[Dict]:
    ...
    result_files = sorted(
        results_dir.glob("validation_staging_complete_*.json"),
        reverse=True
    )
...
def update_continuity_file(results: Dict):
    ...
    if pqc_result or anomaly_result or graphsage_result:
        ...
        if benchmarks_section in content:
            ...
            if mttd_result:
                updated_metrics += f"- **MTTD:** {mttd_result.get('mean', 'N/A')}s ✅ (target: <20s)\n"
            if mttr_result:
...
def main():
    parser = argparse.ArgumentParser(description="Обновление CONTINUITY.md после staging deployment")
    parser.add_argument("--results-dir", default="benchmarks/results", help="Директория с результатами")
    ...
    if args.dry_run:
        ...
        logger.info(f"Найдены результаты для метрик: {list(results.get('results', {}).keys())}")
        return 0
