...
def generate_ghost_ws_443_link(user_uuid: str, label: str | None = None) -> str:
    """WS via 443 fallback (Xray -> nginx:8444 -> Xray WS)."""
    host = PROFILE_VPN_SERVER
    path = quote(GHOST_HTTPS_WS_PATH, safe="/")
    fragment = quote(label or "x0tta6bl4-Access-WS-443", safe="")
    return (
...
def generate_ghost_xhttp_443_link(user_uuid: str, label: str | None = None) -> str:
    """XHTTP via 443 fallback (Xray -> nginx:8444 -> Xray XHTTP)."""
    host = PROFILE_VPN_SERVER  # IP: 89.125.1.107
    path = quote(GHOST_XHTTP_PATH, safe="/")
    fragment = quote(label or "x0tta6bl4-Access-XHTTP-443", safe="")
    return (
        f"vless://{user_uuid}@{host}:443"
        "?security=tls"
        "&encryption=none"
...
def generate_fallback_links(user_uuid: str, label: str | None = None) -> list[str]:
    ...
    # Port 8443 XHTTP (legacy)
    if ENABLE_GHOST_XHTTP_FALLBACK:
        links.append(generate_ghost_xhttp_link(user_uuid, label=label))
    # Secondary Reality
    if ENABLE_SECONDARY_REALITY_FALLBACK:
        links.append(generate_secondary_reality_link(user_uuid, label=f"{label} Reality-2083" if label else None))
    # Legacy XHTTP
    if ENABLE_XHTTP_FALLBACK:
        links.append(generate_xhttp_link(user_uuid, label=f"{label} Legacy-XHTTP" if label else None))
    return links
...
async def main() -> None:
    ...
    try:
        ...
        logger.warning(f"Failed to set_my_commands: {e}")
